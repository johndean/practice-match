...
def row(tid,name,done,state,label,now=None):
    ...
    for i,s in enumerate(STAGES):
        ...
        dots.append(f'<span class="{klass}" title="{s}"></span>')
    fill=int(done/len(STAGES)*100)
    return (f'<li class="brow {state}"><span class="tid">{tid}</span><span class="bname">{name}</span>'
            f'<span class="bdots" aria-hidden="true"><i class="bfill" style="width:{min(fill,100)}%"></i>{"".join(dots)}</span>'
            f'<span class="blabel">{label}</span></li>')
board="\n".join([
 ...
 row("0.1.23","ESRI-ZOOM — zoom to 20, never the “Map data not yet available” tile; Satellite credits Esri&rsquo;s current text",7,"done","Live on QA + production"),
 ...
 row("0.1.23","HOUSEKEEPING-B — the ledger proves its own integrity; the documented gate is CI&rsquo;s",7,"done","Live (docs, tests, tooling)"),
 ...
 row("0.1.24","SNAP-METRO — the Census&rsquo;s published metro figure as the AREA headline where one exists",1,"next","Briefed · starts when ONE-VOCABULARY merges"),
 ...
 row("admin","Users · Data Sources · Listings · Requests tabs — real rows, every action wired (A36&ndash;A39)",0,"wait","WAITING ON YOU: compose from V3&rsquo;s elements, or a new design export"),
])
STATE_LABEL={"done":"Live","active":"In progress","blocked":"Waiting on John"}
def rnode(ver,state,frac,blurb,letter):
    return (f'<div class="rnode {state}"><span class="rdot" aria-hidden="true" style="display:flex;align-items:center;justify-content:center">{letter}</span>'
            f'<span class="rtext"><span class="rstate">{STATE_LABEL[state]}</span><span class="rfrac">{ver} · {frac}</span><span class="rblurb">{blurb}</span></span></div>')
rail=('<div style="display:flex;justify-content:space-between;gap:6px;padding:8px 0 4px">'
 ...
 + '</div>')
tile=lambda title,big,sub: (f'<div style="border:1px solid var(--rule);border-radius:8px;padding:12px 14px"><div class="rstate" style="color:var(--ok)">{title}</div>'
                            f'<div style="font-size:26px;font-weight:700;color:var(--ink-strong)">{big}</div><div class="blabel">{sub}</div></div>')
...
v8 += '<li><b>2 &middot; Practice Detail panel</b> &mdash; Median Income <b>$94K &middot; +19% vs US &middot; approximate</b> (was a hard-coded +25%). Same $94K as the strip: the two agree on purpose.<br>' + img("qa-0123-panel-def","The docked panel&rsquo;s Median Income tile on QA 0.1.23") + '</li>\n'
...
for found,color,state in rows:
    v8 += f'<tr><td>{found}</td><td><b style="color:var(--{color})">{state}</b></td></tr>\n'
