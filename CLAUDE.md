...
**Still there, deliberately.** The four prototype props stay **declared** and are never passed (D-I8-2): `frontend/tests/app-generated.test.ts` requires `app.setup.js` to declare every prop the design declares, so dropping one fails that gate. `prototypeBar`, `startScreen` and `startGate` are the REFERENCE's way into a state — `frontend/tests/reference-server.mjs` injects them per request from `?props=` — and the app never passes any of them. `startViewport` the app takes from the URL only (`?viewport=mobile`), which is how the prototype's 390×800 phone frame stays reachable until a responsive design exists (D-I8-7; the toggle that used to set it lived in the jump bar). Fixture data in `logic.js` (`P`, `MARKETS`, `VETS`, `ECON_K`, `sellerListings`, `requests`, admin rows, `state.me`) also stays — keep field names; the UI reads them — until the listings API replaces it (Seed Listings plan, D6).
