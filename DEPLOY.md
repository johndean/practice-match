...
Two refusals were added with it. **exit 66** — SOURCE_DIR has uncommitted changes to tracked files: the upload is HEAD, so those edits would silently not ship; commit them, or pass a SOURCE_DIR that is committed (untracked files are fine and are simply left out). **exit 67** — the deployment did not reach `SUCCESS`. `railway up --ci` streams build logs, and that stream can time out (`reqwest error … operation timed out`) *after* the upload succeeded while the deployment carries on, which would otherwise abort the run with the api deployed and the worker not; so a non-zero `up` is resolved by polling `railway deployment list --service <svc> --environment <env> --json` for the **newest** deployment — `SUCCESS` continues to the next service, `FAILED`/`CRASHED` exits 67, and so does a deployment still unsettled after 15 minutes (`DEPLOY_POLL_INTERVAL` / `DEPLOY_POLL_TIMEOUT` override the 10 s / 900 s bounds; the shell tests use them). Residual: if `railway up` fails at *upload* time no new deployment appears, the poll sees the previous one's `SUCCESS` and the run continues — `verify-deploy.sh`'s `EXPECT_SHA`/`EXPECT_VERSION` is the backstop that then fails the deploy.
