...
5. **Seller — the identifiable image content switch, and the per-photograph review it governs** (John, 2026-09-09, image-identifiability protection sub-project, Task P11: "The seller should have ONE simple listing-level control: IDENTIFIABLE IMAGE CONTENT [ SHOW ] [ NOT SHOW ] ... DEFAULT: NOT SHOW. The safest privacy state is the default."). Composed here from the SAME controls this document already asks for — the fourth entry in step 7's disclosure-switch list, the step-6 tile's own thumbnail-plus-caption idiom for the photograph, a small state pill in the design's own rounded-badge shape — rather than invented: a fourth switch beside "Show revenue as a range" and "Share floor plans and documents on request only", its off-state copy "Not shown to buyers — recommended" and its on-state copy "Shown to buyers — signage, logos and names may be visible" (Task P11, amendments A20.1-A20.9). Rev 3 owns what this composition cannot reach without inventing UI: a per-photograph REVIEW dialog (Task P12) where the seller sees each detected region the automatic pass found and confirms, edits or clears it one photograph at a time, and the states the visual oracle needs for it — a tile mid-processing, a tile awaiting review with its regions drawn, a tile confirmed, and a tile whose processing failed.
