...
**17. Editing this plan is itself under test.** `frontend/tests/cross-plan-deltas.test.ts` reads this file in nine cases. Adding A-C0 must not introduce the tokens `market_access`, `007_license_audit`, `pricePin`, `migrations/016_census_registry.sql`, `` `010`–`059` ``, `VIN Foundation decision pending (Platform spec §9)`, nor any table row matching `| bubble` or `| community bubble`; and must not remove `Depends(require("market.read"))`, `community mosaic shading`, `Average Practice Payroll`, `Avg. payroll per practice`, `` `practicePin(label, selected)` ``, `migrations/017_census_registry.sql`, `migrations/020_license_audit.sql`, `` `020` is taken ``, `` `003`–`009` are unassigned ``, `one decision record`, or the V13 typography sentence. Run `cd frontend && npx vitest run tests/cross-plan-deltas.test.ts` after the edit.
