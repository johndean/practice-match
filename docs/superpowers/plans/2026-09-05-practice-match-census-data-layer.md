...
- **Derived formulas (§8), `formula_version = 'v1'`:** `pet_households_est = households × 0.57` (documented national placeholder — **superseded 2026-09-15 by Task PET-RATE-PROVENANCE: the rate is 0.586, the AVMA 2025 Sourcebook's share of United States households owning at least one pet, and it lives with its whole provenance in `app/census/pet_rate.py`. Every `0.57` below this line is the historical record of what this plan proposed, not what the product computes.**) · `population_growth_pct = (pop_2023 − pop_2018) / pop_2018 × 100` · `vets_per_10k_households = establishments / (households / 10000)` · `income_index_vs_us = (local_median − us_median) / us_median × 100` · `revenue_per_establishment = payroll_k × 1000 / establishments` (surfaced as "Avg. payroll per practice" under the layer "Average Practice Payroll"; the metric key is unchanged) · `drive_catchment` V1 = straight-line buffers 8 km (`drive_10`) and 16 km (`drive_20`), method `euclidean_buffer_v1` · `opportunity_score = 40·min(income/140000, 1) + 35·min(growth/40, 1) + 25·max(0, 1 − vets_per_10k/3)`, clamped 0–100, rounded, always rendered with its three components, never in a price context.
- **Data quality (§14):** suppress any ACS value with CV `(moe / 1.645) / estimate > 0.30` → `suppressed = true, suppress_reason = 'high_moe'` (row kept). Summed counts: combined MOE `sqrt(Σ moe²)`, same CV test. Medians never sum: household-weighted average, flagged approximate. **Amended 2026-09-12 (Task INCOME-MEDIAN): a household-weighted MEDIAN of tract medians, interpolated; the weighted average was measured 10.5 % high at GHI (Austin) against 64 tracts.** CBP noise flags stored verbatim; noise-flagged counts are estimates; suppress employment/payroll where Census suppressed. Every rendered figure shows dataset + vintage; two vintages never appear in one ratio.
...
- **Migration numbering:** Sub-project 3 Phase A uses `017`–`059` less the seller lifecycle's `030`–`039` (A-SL5) and the image identifiability sub-project's `040`–`049` (A-C12, 2026-09-09 — spec `2026-09-09-image-identifiability-protection-design.md`); Sub-project 2 owns `010`–`015` and the Seed Listings plan owns `016`; Sub-project 3 Phase B (listing-dependent) uses `060`+. **Amended 2026-09-09 (controller, A-C10/A-C11/A-C12):** the seller lifecycle plan carved `030`–`039` out of SP3-A on 2026-09-08 (A-SL5: `030_listing_owner_and_status.sql`, `031_listing_asset.sql`, `032_listing_submitted_at.sql`), so SP3-A holds `017`–`029` and `050`–`059`; image identifiability protection added `040`–`049`; `080`–`089` are the map engines; and `090`–`099` are reserved for platform and hotfix migrations that land on `main` between releases (first: `090_listing_photo_captions.sql`, seed hotfix A-L11/A-L12).
...
| D3 | **Tract-level choropleth vector tiles deferred to Phase C.** Browse V3 shipped *community mosaic shading* — client-side `L.rectangle` cells over community areas from the market-data payload — until A-C33 and John's rulings D-C34–D-C37 replaced it with **community boundary shading**: real Census polygons at ZCTA, place and county (spec 2026-09-10, amendment A24). Tract polygons remain deferred, now gated on the band-ambiguity measurement rather than on a date. The word "choropleth" is reserved in this programme for the Phase C server-generated MVT tile pipeline (spec §7/§10), which still has no consumer. | YAGNI; flagged for the VIN Foundation with the basemap licence question. |
...
| D11 | **ZIP Code Business Patterns (`zbp`, `2022/zbp`, NAICS2017) is the community-level competition source**, aggregated over ZCTAs by area overlap; county CBP is the benchmark. Register it as a new cleared public-domain dataset (VIN Foundation to approve the addition to the spec's §2 table). *Amended 2026-09-09 (controller amendment A-C6): `2022/zbp` does not exist -- the standalone `zbp` dataset ends at vintage 2018. From 2019, ZIP-level Business Patterns are served by the CBP endpoint's own `zip code` geography instead, so `dataset_registry`'s `zbp` row's `api_dataset_id` is `2022/cbp`, not `2022/zbp`.* | County CBP cannot render per-community competition and produced incoherent ratios (red-team C1). ZIPs ≈ ZCTAs; the approximation is labelled. |
...
**Controller pre-flight amendment A-C0 (2026-09-08; the plan is dated 2026-09-05 and `main` has moved through Browse V3, identity 0.1.2, Seed Listings 0.1.3 and the D15 amendment engine).** Read every task against the paragraphs below; where one contradicts the plan's prose, the paragraph wins.

**1. Phase A only, this pass.** Tasks A1–A9 proceed on `feat/census-data-layer` in a worktree with its own database and Redis DB (§6 of the pre-flight). **Tasks B1–B6 do not start**, and **Task C1 is documentation** — `google_places_aggregate` is not cleared, no Google Cloud billing account exists and `GOOGLE_MAPS_API_KEY` is set nowhere. No QA deploy until John says the demo is over.

**2. The beat schedule is merged, never assigned.** A8 and B4 add entries to `celery_app.conf.beat_schedule`; they do not replace the dict. `tests/test_celery.py::test_the_mail_pipeline_tasks_are_registered_and_scheduled` asserts **set equality** on the scheduled task names, so its expected set is widened in the same commit that adds a Census entry, and the three mail entries are asserted individually so a future wipe still goes red. Register the module through `include=[…]`, not `conf.imports`.

**3. `licence.decide` is in `AUDITED` and in `REAUTH`, and both have consequences.** A9's `decide_license` handler must itself call `audit.write(` with `action="licence.decide"` — a helper does not satisfy `tests/auth/test_permissions.py::test_audited_permissions_are_written_by_their_handlers`, which greps the endpoint's own source, and `test_every_audited_action_is_named_after_a_permission` requires the action to be a real permission name. The `license_audit_log` row stays as well; it is the licence ledger, not the security audit trail. In the same commit, **delete `"licence.decide": "no route yet"` from `REAUTH_OUTSIDE_THE_SWEEP` in `tests/auth/test_matrix.py`** — that dict is asserted as an exact set, so mounting the route without removing the entry fails the suite.

**4. No bare `HTTPException`.** Every refusal in `app/api/admin_data_sources.py` and `app/api/market.py` returns decision A5's envelope — `{"error": {"code", "message"}}` — through the pattern `app/api/listings.py::_error` already uses. This covers A9's 404, B5's `NO_MARKET_DATA` and B5's `NOT_FOUND`.

**5. Both routers mount only under `SITE_MODE == "app"`.** `app/main.py:76` gates every member and admin router that way, production runs `coming_soon` until launch, and `scripts/verify-deploy.sh` probes on that assumption. Put `market.router` and `admin_data_sources.router` inside the same `if`, before `not_found_router`. Hoist each `require(...)` to a module-level constant in the style of `REQUIRE_LISTING_READ`; never wrap one, because `tests/auth/test_permissions.py::_unresolvable` fails closed on a guard whose permission cannot be read back.

**6. The six new settings are optional, and they land with their documentation.** `census_api_key`, `census_contact_email`, `s3_endpoint_url`, `s3_bucket`, `s3_access_key_id`, `s3_secret_access_key` all default to `None`. A required census setting would take the api and the worker down at boot, because `load_settings()` exits 1 on a missing required field. A3's `require_key` hard failure applies at the **ingest task and CLI entry points only** — `SystemExit(3)`, never at import. Each new name gets a `# NAME=` line in `.env.example` and a backticked row in `DEPLOY.md`'s Variables table in the same commit, or `tests/test_docs.py::test_every_setting_is_documented_in_env_example_and_deploy_md` goes red.

**7. New dependencies, new obligations.** `boto3`, `pyshp` and `shapely` go in the **main** group (the worker imports them; the image builds `--only main`); `moto[s3]` and any stub packages go in **dev**. `pip install` inside CI is banned (`tests/test_docs.py::FORBIDDEN_CI_SUBSTRINGS`). `mypy app --strict` and `pytest -W error` both apply: add `boto3-stubs[s3]` / `types-shapely` or targeted `# type: ignore[import-untyped]` in the style of `app/tasks/celery_app.py`, and treat any botocore or moto deprecation warning as a failure to fix, not to filter.

**8. `scripts/census_load.py` joins CI's strict-mypy step in the commit that creates it** (`.github/workflows/quality.yml`, the `poetry run mypy scripts/... --strict` line), because `tests/test_docs.py::test_ci_strict_mypy_covers_every_python_script` requires every script measured by `--cov=scripts` to be there. The same script must reach **100 % branch coverage** — the coverage gate is `--cov=app --cov=scripts --cov-branch --cov-fail-under=100`, with `diff-cover --fail-under=100` on top for pull requests. Budget test time for every subcommand, every error arm and the `__main__` guard.

**9. Watch the suite clock.** The CI backend job is `timeout-minutes: 30`, and every `conn` test creates, migrates and drops a scratch database. Census takes the migration count from nine to fourteen. Measure at the end of A6 and again at the end of A9; if the job crosses roughly twenty minutes, the answer is a session-scoped template database, not a raised timeout — a raised budget is a reviewed change with a reason in the commit message.

**10. D4's state list is stale; load six states, not four.** `seeds/hospitals.json` holds eighteen hospitals across eleven markets in **CA (06), TX (48), FL (12), GA (13), NY (36) and CO (08)**. Loading only `48,06,12,13` would leave New York, Denver, Houston, Dallas, Santa Barbara, South Lake Tahoe and Los Angeles with no community data on QA. `market_state` and the `--states` default take all six; the plan's `--states 48,06,12,13` example is rewritten. D4's four CBSAs remain the *design's* markets and stay named as such.

**11. `pop`, `growth`, `income` and `hh` are served by `app/api/listings.py::serialise`, formatted server-side.** The plan routes them through `GET /api/markets/{cbsa}/communities` and `GET /api/listings/{id}/market` as numerics, but the frontend reads them only from `GET /api/listings`, as the design's own formatted strings — `"81,900"`, `"+14.2% since 2015"`, `"$118,400"`, `"27,600 households"` — which `logic.js` then parses back with `num()` and two `.replace()` calls. So Phase B adds a fourth data path: `serialise` reads `market_metric` at band `place` and emits those four strings in exactly the design's spelling. **This costs no design amendment and no pixel movement**: `ApiListing` already declares the four fields, and amendments A12.10/A12.11 (`hasDemo: p.id !== "p8" && p.pop != null`) flip the detail from the "Community data unavailable" card to the populated four-tile grid on their own. The two new endpoints still ship — they carry provenance, bands, MOE, suppression and the panel metrics the detail's market report will need — but they are not what fills the Community Context card. Any new field on `ApiListing` is mirrored in `frontend/tests/design-listings.mjs`, which is asserted to be `toPractice`'s exact inverse.

**12. Nothing in this plan hand-edits `frontend/src/logic.js` or `frontend/src/App.vue`.** Both are generated or ported byte-for-byte from the amended design and pinned by `frontend/tests/app-generated.test.ts`. Task B6's mapping table names seven `logic.js` sites — `communities()`, `VETS`/`ECON_K`, `incomeNat = 75149`, the `per10k`/`compLevel`/`score` block, `LAYER_META`, the six footer cards' `src` strings and the admin Data Sources fixture rows — and every one of them is a **D15 ruled amendment family plus a re-port** (`npm run gen:design && npm run gen:app`), never an edit. A new family is a four-place change: the amendment const, the `amendments()` array, `AMENDMENT_IDS` with its `toHaveLength(82)` number, and a `LOCAL_AMENDMENTS.md` row — and the order of the array is load-bearing. **B6 is re-scoped to documentation.** It writes `docs/integrations/market-data-api.md` and `tests/api/test_contract_doc.py` and nothing else; the design amendments it implies are a separate, John-ruled task with its own screenshots. Its contract test imports `app.api.market` and `app.api.admin_data_sources` at module scope, so it cannot be written RED before A9 and B5 exist — its RED is the missing document, and the import is deferred inside the test functions.

**13. Three of the thirteen frozen screens are Census surfaces.** `detail`, `mobile-detail` and `admin-data-sources` are in `UNCHANGED_SCREENS`. Because the baselines are regenerated from the reference in the same run, an amendment that moves the design and the app equally stays pixel-clean at `maxDiffPixels: 0` — but the hashes in `frontend/tests/baseline-manifest.json` still move, and that file is only ever regenerated to record a ruled design change, with the ruling named in the commit message. Any such change stops for John first.

**14. The reference has no API.** `frontend/tests/reference-server.mjs` serves a static prototype and returns **400** for any `?props=` key the design does not declare. If a Census state has to be reachable by the oracle, it arrives as a declared prototype prop added by amendment — the route `startGate`, `startNotice`, `startAnswerNote` and `layerPalette` took — and it is declared in **both** the design and `src/app.setup.js`, because `app-generated.test.ts` checks prop parity. The plan declares none; none is added without John's ruling.

**15. Attribution stays visible and becomes registry-driven only by amendment.** `dataset_registry.attribution_text` is the source of truth from A1 onward and every API response carries it. The strings the browser renders today are hard-coded in generated files — `App.vue:780/785/787/901` and `logic.js:137-186` through `"Source: " + meta.source` — and `frontend/tests/smoke.spec.ts:320` pins `'Source: U.S. Census ACS 5-year estimates (2023) · community level'` verbatim while `frontend/src/lib/leaflet.test.ts:42` pins `'Tiles © Esri'`. Moving any of them is a design amendment and a ruled change; **serving different copy from `/api/layers` while the browser still renders `LAYER_META` is the one thing that must not happen silently**. Until that amendment is ruled, `/api/layers` is a contract for Wave 2b, not a live copy source.

**16. Blocked datasets and the Google gate are unchanged.** The 2017 export stays out of the repository, the bucket and the database (D15; `.gitignore:36`). `pet_ownership` and `practice_locations` seed as blocked and the `market_metric` trigger enforces it. The `pets` layer renders only the ACS-derived estimate, labelled derived. Task C1 remains documentation.

**17. Editing this plan is itself under test.** `frontend/tests/cross-plan-deltas.test.ts` reads this file in nine cases, several of them negative (names of deleted wrappers and helpers, a retired migration number, a superseded decision sentence) and several positive (the V3 rendering statements, the payroll label, the reserved word, the migration range). Every edit to this plan runs that test before it is committed; the strings live in the test, deliberately not repeated here.
**18. Live defect, recorded for John.** `VETS` and `ECON_K` are keyed by design-fixture ids (`p1`…`g4`) and `num(null)` is `0`, so **all eighteen seeded QA listings currently shade bottom-bucket on every one of the six Browse market layers**. Nothing tests it. It is the gap this sub-project closes; it is not a regression introduced here.

**Controller rulings on the pre-flight's questions (2026-09-08; defaults John may veto):** Q2 the API formats the four Community Context figures to the design's existing spelling (¶11) — no design amendment for them; Q5 six states (¶10); Q7 Phase A ships with the raw archive disabled (`ObjectStore.from_settings → None`) until John creates the bucket; Q10 `MARKET_DATA_PUBLIC` stays `false`; Q12 Task C1 stays documentation; Q13 the Census release bumps 0.1.3 → 0.1.4 in lockstep and nothing deploys before John says the demo is over. Q1 (basemap licence), Q3 (the three frozen screens), Q4 (`CENSUS_CONTACT_EMAIL`), Q6 (ZBP approval), Q8 (when the key is set), Q9 (opportunity-score weights) and Q11 (four copy conflicts) are John's and block only Phase B.


---

**John's rulings, recorded as A-C1 (2026-09-08 ~23:00 WITA, verbatim intent; they answer the pre-flight's thirteen questions and override A-C0 where the two differ).** (1) **Basemap — see "Basemap licence — one decision record" below.** (2) No Census amendment family for the byte-locked `logic.js`; the API formats Census-derived values to the design's existing string spelling. (3) `detail`, `mobile-detail` and `admin-data-sources` stay frozen — no move, no rebaseline in this pass. (4) The Census `User-Agent` carries the VIN Foundation's designated technical contact address, never a developer's personal one; until it is supplied, `CENSUS_CONTACT_EMAIL` is an explicit deployment prerequisite of the ingestion (the load refuses to run without it, and DEPLOY.md lists it as required for the worker before the first live load). (5) Six states: CA, TX, FL, GA, NY, CO. (6) **ZBP approved** — the spec's §2 table gains a ZIP Code Business Patterns row with dataset name, geography, vintage, attribution and refresh requirements. (7) **Bucket approved** — `practice-match-data` is created separately per environment; the raw archive is NOT disabled for Phase A (A-C0's Q7 default is withdrawn; A2's `ObjectStore` is real, tests use moto, the bucket credentials are optional settings supplied per environment). (8) `CENSUS_API_KEY` reaches QA only after the ingestion implementation and its validation gates are ready; stored only as a Railway secret; production remains gated. (9) **Opportunity score not approved for publication** — no score is user-visible or presented as authoritative until stakeholders sign off on the weights; Phase B keeps the computation behind that gate. (10) `MARKET_DATA_PUBLIC` stays `false` on QA and production until the market-data release is explicitly approved. (11) The four design-vs-spec copy conflicts resolve to the SPEC wording — scheduled with Phase B under their own rebaseline ruling, since two of them sit on frozen screens (3). (12) Task C1 untouched. (13) Release 0.1.3 → 0.1.4; prepare and test, no deploy before the demo. **Load-bearing architecture correction (verbatim intent):** the canonical market data consumed by the frontend is `GET /api/listings`; required numerics are not sent to endpoints the frontend never calls; Census-derived listing/market values are implemented through `app/api/listings.py::serialise`, which produces the display strings the existing frontend expects; `logic.js` is not altered to accommodate Census data. The `VETS`/`ECON_K` fixture-id / null-to-zero problem is a genuine defect: real seeded listing/market data must populate the existing market-layer inputs — the problem is not hidden by changing bucket or scoring presentation (Phase B designs the feed; the port's footer export line, not the design, is the only permitted seam).

### Basemap licence — one decision record

**John, 2026-09-08 (verbatim intent):** CARTO is the basemap for the Census analytical / market-data map layer. Esri remains permissible only where the approved visual design requires it. Census analytical rendering must not depend on an Esri-only implementation. The registry's `osm_tiles` row (CARTO light, OSM data, ODbL) is that layer's source of attribution text; the Browse map's design-required Esri tiles keep "Tiles © Esri". This is the single record CLAUDE.md points at; it is not restated elsewhere.

**2026-09-13 — three facts for this record, measured while fixing the zoom ceiling (Task ESRI-ZOOM, ruling D-C52). NOT a provider change; nothing about the decision above moves.**

1. **Attribution correction (shipped, amendment A35.7).** `https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer?f=pjson` carries `"copyrightText": "Source: Esri, Vantor, Earthstar Geographics, and the GIS User Community"` (fetched 2026-09-13). The approved design and the port both credited "Imagery © Esri, Maxar, Earthstar Geographics": **Maxar is the vendor's former name**, and the service's own description now reads "Vantor imagery at 0.3m resolution for select metropolitan areas … 0.5m resolution across the United States". The satellite attribution is now the service's credit line VERBATIM, which is the rule `dataset_registry.attribution_text` already follows for every Census dataset. The gray canvas's "Tiles © Esri" is untouched — it is the approved design's own string and what CLAUDE.md names. (For the record, the Canvas services' own `copyrightText` is `Esri, HERE, Garmin, (c) OpenStreetMap contributors, and the GIS user community`; changing that string is not part of any ruling to date.)

2. **Both Canvas services are in Esri's mature support and have a published retirement date.** The ArcGIS Online items behind `Canvas/World_Light_Gray_Base` (`ed712cb1db3e4bae9e85329040fb9a49`) and `Canvas/World_Light_Gray_Reference` (`87fcdf91a0f14e4a9fda40a763c6f2b8`) both carry the snippet "This item is in mature support. (Retiring)" and the notice "This item is in mature support as of July 2021 and will retire in **December 2029**. A new version of this item is available for your use." (fetched 2026-09-13). That is a dated fact for whoever holds this decision, not an action: the Browse basemap is what the approved design specifies, and swapping it — to Esri's newer Canvas vector basemap, to CARTO, or to anything else — needs the VIN Foundation's decision exactly as this record has always said.

3. **Cache depth is what D-C52 turned on.** Both Canvas services are cached "from Level 0 … through Level 13" worldwide and "from Level 14 … through Level 16" in North America (each service's own `serviceDescription`), and a request past that ceiling is answered `HTTP 200` with a 2,521-byte "Map data not yet available" placeholder rather than an error. World_Imagery is real to **z19** at every US point probed (Esri's published 0.3 m floor) and deeper in some metros. Whatever basemap this decision eventually names, its own native cache depth has to be declared to Leaflet as `maxNativeZoom` — the display ceiling belongs to the map, not to the tile service.
...
**Controller amendment A-C2 (2026-09-08; Task A2 under John's rulings A-C1 ¶7 and the seller-lifecycle spec's A-SL1).** (1) **One storage layer for the programme.** `ObjectStore` lives in `app/storage.py` (not `app/census/storage.py`), with `tests/test_storage.py` beside the other app tests; the seller-lifecycle sub-project reuses it under a `listings/` key prefix, so its interface is generic (`put(key, data, content_type)`, `get(key)`, `delete(key)`, `exists(key)`, `list(prefix)`) and the Census archive uses a `census/` prefix. (2) **The raw archive is ON.** `ObjectStore.from_settings(settings)` returns a real client when the four `S3_*` settings are present and `None` only when they are absent (the api never needs the archive; the worker does); A-C0's Q7 default (archive disabled) is withdrawn. Tests use `moto[s3]`; no network. (3) **Bucket naming.** The bucket is `practice-match-data` in every environment (Railway buckets are environment-scoped — no `-qa`/`-prod` suffix); the QA bucket exists (Railway, region sjc); its credentials are set by the controller on the QA worker after John's demo, never by an implementer, via `railway bucket credentials --json` piped into `railway variable set --skip-deploys` in a subprocess — no value is ever printed. `S3_ENDPOINT_URL`, `S3_BUCKET`, `S3_ACCESS_KEY_ID`, `S3_SECRET_ACCESS_KEY` are optional settings (default `None`), documented in `.env.example` and DEPLOY.md's variables table in the same commit (`tests/test_docs.py` pins both). (4) **Settings added here, enforced later.** `census_api_key` and `census_contact_email` are optional at boot (`None`); A3's `require_key()` and a new `require_contact()` fail closed (`SystemExit(3)`) at the ingest/CLI entry points — the contact address is the VIN Foundation's designated technical contact, never a developer's (A-C1 ¶4); DEPLOY.md lists `CENSUS_CONTACT_EMAIL` as a worker prerequisite before the first live load. (5) **Dependencies.** `boto3` in the main group; `moto[s3]` and `boto3-stubs[s3]` in dev; `pyshp` and `shapely` arrive with A4, not here; `poetry lock` regenerates the lock file in the same commit; `mypy app --strict` stays clean without `# type: ignore` where stubs exist. (6) A2 adds no script, so CI's mypy line is unchanged until A4.
...
**Controller amendment A-C6 (2026-09-09 ~04:20 WITA; the A6 review's Major, two Minor and one Info, plus a controller verification the review could not run — recorded before Task A8).** The A6 brief's illustrative code named Census variables and an endpoint that the live API does not publish; the implementer transcribed them faithfully, so three of the four industry loads would have exited 4 on their first real run. The controller verified every loader's variables and geography forms against the PUBLIC metadata (`variables.json` / `geography.json`; no key used, nothing ingested) and rules: (1) **ZBP** — `2022/zbp` does not exist (404); the standalone `zbp` dataset ends at vintage 2018, and from 2019 the ZIP-level Business Patterns are served by the CBP endpoint's `zip code` geography (summary level 861, variable `ZIPCODE`), which accepts no `in=` clause. The registry row `zbp` keeps its key, name, vintage `2022`, licence and attribution (the Census product is still named ZIP Code Business Patterns) and changes `api_dataset_id` to `2022/cbp`, its `notes` recording this finding; `zbp.py` requests `get=ZIPCODE,ESTAB` with `for=zip code:*` (URL-encoded `zip%20code:*` — `_build_url` encodes the space as `%20`, pinned by a client test) ONCE per NAICS code (three requests, no per-state loop) and keeps only the ZIPs present in `geo_area` as summary level `860` — the market states' ZCTAs A4's TIGER load already bounds (D11's ZIP≈ZCTA approximation, the brief's own fallback made concrete); when `geo_area` holds no `860` rows the load refuses with exit 2 naming the prerequisite (`census_load.py tiger` first). (2) `ESTAB_F` is not a Census variable: the `flag` column leaves `zbp_industry` (Mi2). (3) **CBP** — `EMP_F`/`PAYANN_F` are not variables of `2022/cbp`; the disclosure flags are `EMP_N`/`PAYANN_N` ("Noise range for …": CBP uses noise infusion, not withheld cells), stored verbatim in `cbp_industry.flag` as `EMP_N=…;PAYANN_N=…`; the `== "D"` branches go; an absent or sentinel value stays NULL through `normalise()`/`_int` — never 0 (spec §14 kept). (4) **BDS** — the entry variable is `ESTABS_ENTRY` ("Number of establishments born during the last 12 months"); the table column `estab_entry` is unchanged. (5) **QWI** — `Emp`, `EarnBeg`, `HirA`, `industry`, `ownercode`, `seasonadj` and the `county`-in-`state` form verified; no change. (6) `cmd_qwi` catches `VariableMissing` → 5 around `latest_available` (Mi1) and `PermissionError` → 2 around `qwi.load` (I1), both tested. (7) The brief's Step 3 live smoke did not run and cannot run locally: the key is a Railway secret on the QA worker only (A-C1 ¶8); this metadata verification stands in for the URL and variable shapes, and the first live load runs on the QA worker at A9's activation on John's word — every report states plainly whether a live step ran. (8) Migrations `017` and `023` are edited in place (unreleased; applied only to this worktree's database, which is recreated — the A1 precedent); `tests/census/test_registry.py`'s `2022/zbp` pin moves with the row. Closed in one fix round after A7 lands (single writer).
...
**Controller amendment A-C8 (2026-09-09 ~07:45 WITA; rulings on the A8 review — spec ✅, NOT APPROVED: 1 Major, 6 Minor, 9 Info; closed in one fix round).** (1) **M1** — `load_qwi` checks the registry's `cleared` flag BEFORE `qwi.latest_available` probes Census (the CLI's `cmd_qwi` already does); a blocked `qwi` dataset is refused with no request and no archive write, tested. (2) **m1** — A-C7 (8) becomes a test: an AST scan over every module under `app/tasks/` asserts none references `vintage.activate`. (3) **m2** — `license.audit` isolates each dataset: a malformed `license_url` or any `httpx` error marks that dataset's row failed/skipped and the sweep continues; tested with one bad URL among good ones. (4) **m3** — `ORDER BY checked_at DESC, id DESC`. (5) **m4** — the licence page body is read under the client's `MAX_RESPONSE_BYTES` bound; over it is that dataset's error, tested. (6) **m5** — the tasks' `_conn()` uses the canonical DSN helper in `app/db.py`; no re-implementation. (7) **m6** — one `_factory()` closure shared by every load task. (8) **i1** — a `census.load_zbp` task exists for parity (manual trigger only, NO beat entry: ZBP is annual and John-approved per load). (9) **i2** — `load_tiger` checks `tiger_cb`'s `cleared` flag in the task like its siblings. (10) **i8** — Global Constraints ¶10's User-Agent spelling gains "(superseded by A-C3 (2))". (11) **i3/i4 → A9**: the Data Sources API must show `last_verified_at` null as "never verified" and surface `drift_flagged` independently of the verification time. (12) **i5** — record-only by ruling: no Celery auto-retry on the scheduled loads (a retry would repeat hours of downloads; the cadence and the CLI are the documented recovery; upserts are idempotent if an operator re-runs). (13) Concerns 1–2 ratified (contact gate; per-task plain connections closed in `finally`); the five declared deviations accepted (A-C5). i6, i7, i9 accepted as recorded.
...
**Controller amendment A-C9 (2026-09-09 ~09:05 WITA; rulings on the A9 review — spec ✅ 34/34, APPROVED: 5 Minor, 7 Info; closed in one fix round).** (1) **m1 + m2** — one test monkeypatches `audit.write` to raise on `/license` and asserts: no registry change, no `license_audit_log` row, the gate NOT invalidated (the decision never committed), `FOR UPDATE` taken; a second asserts invalidation happens only after a successful commit. (2) **m3** — the `HttpUrl` normalisation is pinned (`https://example.test` → the exact stored string). (3) **m4** — a Redis failure on `gate.invalidate` AFTER the commit is logged and swallowed (the decision stands; the 60-second TTL self-heals), tested with a raising fake. (4) **m5** — the module docstring names the console as the second author of `last_verified_at`. (5) **I3** — licence URLs must be `https` (a `http://` URL is `422 BAD_FIELD`): the quarterly audit fetches terms pages and must not do so in clear. (6) **I6** — the two handlers become plain `def` (FastAPI's threadpool) — blocking psycopg2 work never runs on the event loop; same rule as A-I5d's `list_signups`. (7) **I5** — `tests/perf/test_api_latency.py` gains the budget row for `GET /api/admin/data-sources`. (8) **I7** — the `max_length` bounds are tested at the boundary. (9) **I1, I2, I4** accepted as recorded (a decision's `notes` replaces the registry note by the brief's contract — the ledger row keeps history). Concerns 1–5 and the five declared deviations ratified; Phase B carries "B5 consumes `gate.layer_enabled` and the Data Sources API" as an explicit acceptance line.

**Controller amendment A-C11 (2026-09-09 ~10:20 WITA; rulings on the Phase A final review — READY TO MERGE: 1 Major, 6 Minor, 9 Info; closed in one final fix round before the release).** (1) **Major — the Phase A exit runs IN the worker container.** The plan's exit sequence (Task A9's closing step) and DEPLOY.md's new "Census Phase A exit" runbook use `railway ssh --service worker --environment QA` (the CLI key `practice-match-cli` is registered; DEPLOY.md's stale "no SSH key" line is corrected) — never `railway run`, which executes locally with the service's variables injected and would pull `CENSUS_API_KEY` and the bucket credentials onto the operator's machine (A-C1 ¶8) and fail on the `.railway.internal` DSN anyway. Order: `railway status` must print Project: Practice Match; then inside the container `python scripts/census_load.py tiger` → `acs` ×3 → `cbp` → `zbp` → `qwi` → `bds` → `activate <dataset> <vintage> --by john` per dataset (no `--force` without `--note` and John's word); the controller runs it, only on John's explicit word. (2) **m1** — `cmd_tiger` refuses when `tiger_cb` is not `cleared` (exit 2), like every other loader entry point. (3) **m2** — `cmd_tiger` maps a corrupt or unreadable shapefile zip to exit 5 with a redacted message. (4) **m3** — the plan's A5/A6 illustrative code blocks gain a one-line banner under each heading: "Illustrative — superseded by A-C6: use `EMP_N`/`PAYANN_N`, `ESTABS_ENTRY`, `2022/cbp` `zip code`, `ObjectStore.put`; the committed code is authoritative (A-C5)" — the blocks themselves are history and are not rewritten. (5) **m4** — the plan's release line says Census Phase A ships as **0.1.7** (John's forced order after the two seed hotfixes took 0.1.5 and 0.1.6). (6) **m5** — an empty `market_state` table refuses with exit 2 and a message naming the prerequisite, never an `IndexError`. (7) **m6** — `ObjectStore.list` paginates with the continuation token; tested with a moto bucket of more than one page. (8) The 17 deferred items' triage is accepted as the review records it; the 9 Info accepted as recorded. (9) Merge readiness: merge `main` into the branch first (it now carries 0.1.6 once hotfix 3 ships — the `photos`/`photo_captions` contract and migration `090`), then the controller bumps to 0.1.7 and runs the four-part gate (backend, frontend, visual+DOM+smoke, QA click-through of the admin Data Sources API through the swagger/curl probe — the screen stays frozen).

**Controller amendment A-C13 (2026-09-09 ~21:45 WITA; Phase B kickoff — John: "finish all pending census market data tasks", B1 through B6, under test-first discipline and the subagent loop).** The rulings that bind every Phase B task, so no implementer has to guess: (1) **Nothing in Phase B calls Census, the geocoder or any external host in a test.** Every adapter is exercised through a stubbed transport, as Phase A's client is; a test that reaches the network is a defect even when it passes. The live loads remain a separate, gated act: John's four `S3_*` variables must be on the QA worker first (the archive-required rule, A-C11 (10)), and then his word. His key is now valid and answering — probed from inside the worker at 21:30, HTTP 200 with real data — so the key is no longer the blocker. (2) **The three screens John froze on 2026-09-08 stay frozen this pass: `detail`, `mobile-detail` and `admin-data-sources`.** No Phase B task may touch the byte-locked design, `screens.ts`, the baseline manifest or the amendment engine. B6 is documentation only, which is why it is compatible; if any other task finds it needs a pixel, it STOPS with NEEDS_CONTEXT rather than opening the engine. (3) **`MARKET_DATA_PUBLIC` stays false in every environment** (John's ruling 10): the market router is reachable by members only, and the flag widens the permission rather than removing the dependency, exactly as B5's own text has it. **The opportunity score is not published** until John signs off on its weights (ruling 9) — compute it, store it, do not expose it. (4) **The basemap licence is one open decision** (Esri versus CARTO, recorded in this plan): no Phase B task hard-codes either, and every attribution string comes from `dataset_registry.attribution_text`, never a literal. (5) **Blocked datasets stay blocked:** pet-ownership incidence and third-party practice-location data are not ingested and not displayed; `pets` is the DERIVED estimate this plan defines, carrying its caveat, and nothing else. (6) **Migrations `060`–`069` are Phase B's range** — B1 takes `060` and `061` as written; the earlier carve-outs stand (`030`–`039` seller, `040`–`049` image identifiability, `090`–`099` main). (7) **The worktree:** `.worktrees/feat-census-phase-b`, cut from `main` at 4466c40 (0.1.10), database `practice_match_censusb`, Redis index `/10`, Playwright ports 5673, 5674, 5675 and 8247, with `npm ci` already run in both `frontend/` and `coming-soon/` and the shared compose stack reused rather than duplicated. (8) **Every task ends where the plan says it ends** — its own tests, the full gate, one commit with explicit pathspecs and the trailer — and every deviation stops for a ruling instead of being decided in the code.

**Controller amendment A-C14 (2026-09-09 ~22:00 WITA; the Phase B pre-flight, from a measured schedule analysis — four readers over the plan, the blockers and every SDD ledger, each adversarially checked).** (1) **The embargo is lifted.** A-C0 paragraph 1 reads "Tasks B1-B6 do not start" and no later amendment lifted it; John's "finish all pending census market data tasks" (2026-09-09 ~21:40) does, and this records it. (2) **D-CENSUS-KEY is CLEARED** — John set a valid forty-character hexadecimal key on the QA worker at ~21:30 and a probe from inside that worker returned HTTP 200 with real data. The remaining live-load blocker is the four `S3_*` variables, which the archive-required rule (A-C11 (10)) turns into a refusal of every keyed load. **Useful sub-case, recorded because it is five minutes of John's time:** clearing `S3_*` alone, even with no key at all, lets the keyless TIGER loader run and fill `geo_area`, which makes B2's fallback ladder and B3's catchments verifiable on QA rather than only in fixtures. (3) **Two defects in the plan's own illustrative code, to fix at the point of use rather than discover mid-task.** B2's `STATE_NAMES` lists only Texas, California, Florida and Georgia while A-C1 (5) rules six states: New York (36) and Colorado (08) are missing, so the place fallback would fail for the New York and Denver seeds. And B2's interface note attributes `census.backfill_listing` to B5 when B4 Step 5 creates it. (4) **The plan contradicts itself on the layer state and B6 wins:** B5's payload uses a boolean `enabled` while B6's contract defines a three-state `state` of enabled, disabled or blocked. The three-state form is the one the admin surface and the licence gate need, since "off because a member turned it off" and "off because the licence is not cleared" are different facts; B5 emits `state` and no boolean. (5) **The opportunity score, interim ruling, mine to make and narrow by design:** B4 computes and stores it, and B5 OMITS the key from every payload — not a flag, not a null, absent — until the VIN Foundation signs off on the weights (A-C1 (9)). A key that exists in a payload gets rendered by someone eventually; a key that is absent cannot be. B5's tests assert the absence. (6) **THE UNASSIGNED SCOPE, which is John's ruling and the one thing that could leave this phase incomplete while every task passes.** A-C1's architecture correction makes the feed for the design's `VETS` and `ECON_K` fixtures Phase B's job, but no task owns it, and the seam it needs is the last line of the byte-locked `frontend/src/logic.js` — its footer export — which is pinned by `app-generated.test.ts`. The defect it exists to close is recorded in A-C0 paragraph 18: all eighteen seeded QA listings shade bottom-bucket on all six Browse market layers. Queued for John as **D-C14**: may Phase B change that export and re-pin the generated-app test, as a ruled seam change, or does the feed become its own task after the phase? Until he answers, B1 through B6 proceed exactly as written and nothing touches the design. (7) **Sequencing facts, measured, not assumed:** B6 cannot go RED before B5 exists (its drift test imports both routers); B5 depends strictly on B4; B3 needs B1 and B2; and the phase cannot pipeline the way Phase A did, because each task's tests import the previous task's module. (8) **The worktree is `feat-census-phase-b` at 0.1.10, not the retired Phase A worktree** (which sat 74 commits behind main); its ledger was archived into the main checkout before reuse.

**Controller amendment A-C15 (2026-09-09 ~22:25 WITA; the Phase B parallel plan and twelve rulings, from five pre-flight readers run against the real code before any implementer met it — they found four critical brief defects that would each have stopped a task dead).** (1) **A-C14 (7) was wrong and is corrected here.** B3 depends on B1 ALONE, not on B2: its test seeds a `practice_location` row directly and its module imports nothing of the geocoder's. The generalisation "each task's tests import the previous task's module" is false for B3 and for the pure-maths half of B4. The real chain is B1 → B3 → B4's materialisation → B5 → B6's route test. (2) **A-C13 (8) is widened:** one commit per DELIVERABLE with explicit pathspecs, one writer per worktree — so a task may land as two commits where its halves are genuinely separable, and an implementer must not stop merely because it produced two. (3) **The registration anchors are pre-placed by this commit** in `app/tasks/census.py`, one comment line for B2's task and one for B4's, so two worktrees append at distinct points instead of colliding at end of file. Neither implementer moves the other's anchor. (4) **Migrations `060` and `061` are FROZEN at B1's commit.** A correction after a second worktree has migrated its own database is refused by the migration runner in every one of them; a later fix is a NEW migration in the range, never an edit. (5) **One full gate at a time across all worktrees.** The compose Postgres is a single emulated instance and every database-backed test clones and drops on it; the controller hands out the gate, implementers run their own files freely. (6) **The metrics signatures are settled before dispatch, not discovered** — every function in `app/census/metrics.py` is fully annotated, the illustrative code in the brief is not, and the half that writes them and the half that calls them must agree exactly. (7) **`competition_level` is derived by B5 at serialisation, not written by B4** — it is a presentation band, not a measurement, and an unowned function with no caller cannot reach the coverage gate. (8) **The materialisation deletes the bands it is about to rewrite before it upserts**, so a vintage change cannot leave two generations of a metric in the same band. (9) **The backfill task reaches the active vintage through the materialisation module, never by importing the vintage module** — a committed guard walks the syntax tree of everything under `app/tasks/` and fails on that import. (10) **Neither B2 nor B4 uses the task decorator**: the committed convention registers by calling, because a typed function under an untyped decorator is refused by strict typing and a suppression is forbidden. (11) **B5 emits the three-state layer value and omits the opportunity score**, as A-C14 (4) and (5) ruled, and its market routes must not trip the committed guard that walks every mounted route under the listings prefix. (12) **The second unassigned scope is escalated to John beside D-C14:** the four formatted Community Context strings the buyer serialiser returns are in no task's file list and are hard-coded to nothing today, and whether Phase B fills them decides whether the phase's own exit check on that card can pass at all. Until he rules, no task touches it and B6 records it as open. **The parallel plan, honestly bounded:** four streams — the trunk (B1, then B3, then the materialisation, then B5, then B6's route test), the pure-maths half of B4 starting immediately with no dependency, the geocoder as a leaf off the critical path, and the frozen half of the contract document — saving roughly three hours of about fifteen. Not more, and it is not sold as more.

**Controller amendment A-C16 (2026-09-09 ~22:35 WITA; B1 closed — spec 31 of 31 against the live schema, quality APPROVED — and its three findings become B3's first steps).** The reviewer checked every column, constraint, index and the trigger's behaviour against the running database rather than the diff, and ratified all three declared deviations: the DECLARE-free trigger (the repository's own check forbids the brief's shape, and `014` is the precedent), the hand-repair of this worktree's private database to reapply a corrected migration, and the six extra not-null columns the listing fixture needed. **What B3 fixes first, each RED before its fix:** (1) **The trigger's update arm is untested.** All four tests insert; none updates, so narrowing the trigger to insert only would pass every one of them. Add the case that updates a metric row onto a dataset whose licence is not cleared and is refused. (2) **The error message can name a stale status.** The DECLARE-free rewrite reads the licence twice, so under concurrent writes the two subqueries can see different snapshots — the accept-or-refuse decision is unaffected, only the words in the message. Remove the possibility rather than document it: the message names the dataset key, which cannot go stale, and not the status it happened to read. (3) **The geocode review table has no foreign key to the listing**, alone among the listing-scoped tables here, so a deleted listing leaves an orphan flag behind. Add it with the same cascade its siblings use. **Both (2) and (3) are a NEW migration `062`, never an edit** — `060` and `061` were frozen at B1's commit by A-C15 (4) and two other worktrees have already applied them; the migration runner refuses a changed file in every database that has seen it, which is the rule doing its job rather than an inconvenience.

**Controller amendment A-C17 (2026-09-09 ~22:45 WITA; B4a closed — spec 16 of 16, quality APPROVED — with one Important that belongs to the materialisation, and the four Minors ruled).** The reviewer worked the arithmetic independently in its own session rather than reading the code, confirmed ten of the eleven formulas and every constant against sections 8 and 14, and reconstructed the contested statistic to show the implementer's correction of the plan's own test comment was genuine (the real coefficient is 0.3118 against a 0.30 threshold, not the 0.36 the brief claimed). **The rulings:** (1) **The Important — a present estimate with no margin of error currently reads as ZERO uncertainty, the most confident case possible, which runs exactly opposite to the suppress-under-uncertainty rule.** It is inherited from the settled signatures, not the implementer's choice, and real data always pairs the two, so the formulas stay as they are and **the materialisation guards it**: an estimate arriving without its margin is treated as unmeasured, suppressed with its own reason, and never silently combined as if it were certain. B4b's brief carries this as a required test. (2) **The compound guard in the weighted median has an untested arm** — a present value with a missing weight — which branch coverage cannot see through a compound boolean. B4b's stream adds the case; it is two lines and it closes a real hole in the proof. (3) **The two weighting functions disagree** on whether a weight may be missing: one requires a real number, the other silently skips. B4b makes them agree, because a caller should not have to remember which is which. (4) **The formula version has no pinning test** — B4b adds one, since a silent bump would let two generations of a metric coexist under the same name. (5) **The score's clamp stays**: reachable, documented, honestly tested against inputs the specification says cannot occur, which is what defensive code is for. B4a merges into the trunk at its next clean point; the metrics module is a pure add with no file overlap.

**Controller amendment A-C18 (2026-09-09 ~23:00 WITA; B2's review — spec 14 of 15, quality NOT APPROVED: one Critical, two Important, two Minor; closed in fix round 1).** (1) **CRITICAL — the fallback ladder stops one rung short.** Section 6 defines four rungs and the implementation has two: ZIP-code area, then place, then it gives up. The county rung is absent, undeclared and untested, and the proof it was meant to exist is already in the database — migration 061's precision constraint, frozen since B1, admits a `county` value the code can never write. Build the rung, with the tests for it and for the ordering of all four. A missing rung is not a smaller feature; it is a listing that could have been placed and was not. (2) **IMPORTANT — the user-agent literal is pinned by nothing.** The implementer's shorter form is RIGHT, and for a better reason than its report gave: A-C3 (2) rules that exact shape, so it is a standing ruling rather than sibling precedent. But the line is executed by a test that never asserts it, so the ruling is unenforced. Assert the literal. (3) **IMPORTANT — a misconfigured worker must not report success.** The refusal when no contact address is configured returns a success-shaped result, so every listing's task is recorded as SUCCEEDED while nothing was geocoded, with a log line as the only trace. The reasoning for not writing an ingestion run stands — a per-listing task has no dataset or vintage to record against — but the conclusion does not: the task RAISES, and a test asserts it raises rather than returns. Silence that looks like success is the failure mode this programme has been bitten by all day. (4) **MINOR — two tests confirm an outcome and claim to prove a mechanism.** One asserts that an unmapped state fails, and says that proves the place query is skipped; a null state code would fail the same way with the guard deleted. The other cannot tell a correct prefix match from an accidental substring match, because the seeded row satisfies both. Make each discriminating, or delete the claim from its docstring. (5) **Recorded, not a finding:** the reviewer could not see amendments A-C16 and A-C17 because B2's worktree was cut before they were written; both are on the trunk and reach B2 at the merge. Nothing in either changes B2's work.

**Controller amendment A-C19 (2026-09-09 ~23:10 WITA; B3 closed — spec 9 of 9, quality APPROVED — the metrics stream merged into the trunk, and B4b's carried list assembled).** The reviewer verified the catchments against the live schema and re-ran the linter and type checker itself; it also ran its own query plan against the empty tables, watched the planner choose the wrong index there, and so confirmed WHY the seeded proof was necessary rather than taking it on trust. The assertion the implementer changed in B1's tests is ratified as necessary. **B4b, the materialisation, opens with everything the phase has carried, each RED first:** from B3 — a test for the case where a practice has no location yet (correct by inspection today, unproven), the numeric floor's link to the column's precision documented where the module explains its other corrections, and one docstring that mixes an alias with a table name; from B4a via A-C17 — an estimate arriving with no margin of error is suppressed as unmeasured rather than combined as if certain, the weighted median's untested arm, the two weighting functions made to agree, and a pin on the formula version; and from the pre-flight — the task registers by calling at its own pre-placed anchor and never by decorator, the beat entry widens the schedule set the drift test asserts by equality, the backfill reaches the active vintage through the materialisation module because a committed guard walks the syntax tree of everything under the tasks package and fails on the direct import, the version stamp must not be a whole-second timestamp because two runs in the same second would collide, the command-line subcommand is under strict typing and the hundred-percent scripts gate so it is real work rather than one line, and the materialisation deletes the bands it is about to rewrite before it upserts so two vintages cannot coexist in one band. **The metrics module is merged into the trunk**, so B4b calls it directly; its eleven signatures are fixed and are not to be changed by the caller.

**Controller amendment A-C20 (2026-09-09 ~23:30 WITA; B2 closed — the re-review returned ALL ADDRESSED, five of five, none new).** The county rung is built and, more to the point, verified as reachable: the reviewer checked against the boundary loader that county geographies load nationally and unfiltered, so the new predicate can match real data — this ladder's earlier defect was a predicate that could never match anything, and the same mistake was not made twice. The order test genuinely pins precedence rather than merely failing: the reviewer traced all four listings by hand and confirmed that a reordering resolves a different listing to a different precision. The refusal raises and the test distinguishes raising from returning. Both rewritten tests now discriminate the mechanism: one proves the place query's SQL never executes when the state is unmapped, which the outcome-only version could not tell from a deleted guard; the other plants a place whose name CONTAINS the target as a substring but not as a prefix, so a sloppy match is caught rather than described. **The merge waits for a clean trunk.** The materialisation is mid-task in the trunk worktree with the shared tasks module among its uncommitted files, so B2's merge happens at its next clean point — a merge into a working tree someone is writing is how a careful branch becomes a negotiation.

**Controller amendment A-C21 (2026-09-10 ~00:30 WITA; B4b's review — spec 14 of 16, quality NOT APPROVED: 1 Critical, 2 Important, 2 Minor — and a correction to my own earlier ruling).** (1) **CRITICAL, and it corrects A-C17 (1).** I ruled that the formulas stay as they are and the materialisation guards the missing-margin case. The reviewer traced by hand what that ruling cannot do: when estimates are present and every contributing margin is absent, the weighted count returns a combined margin of ZERO rather than nothing, so by the time the materialisation looks, the difference between "measured precisely" and "never measured" has already been destroyed. The guard was therefore only ever effective at the place band, and the catchment bands — population, households, and everything derived from them — can never be flagged as unmeasured. **So the fix belongs where the information is lost:** the weighted count returns an unknown combined margin when no part supplied one, and a zero only when a part genuinely reported zero; the materialisation treats unknown as unmeasured and suppresses with its own reason. A test seeds tract rows whose margins are null and proves the flag appears at both catchment bands and in every derived figure. My earlier ruling was right about intent and wrong about mechanism, and the mechanism is what ships. (2) **IMPORTANT — the rewrite is still not atomic.** The catchments task fixed exactly this risk when it was raised there, with the reason written into its own code; the materialisation did not reuse the guard, so a failure between the delete and the upsert leaves a listing with part of its figures. Make it one transaction, the same way. (3) **IMPORTANT — a test that passes under the mutation it exists to catch.** The county-level growth fallback's success path is proven nowhere, and the coverage test added for it survives a change that would break the real fallback, because the observable outcome coincides either way. Make it discriminate or replace it. (4) **MINOR** — no test distinguishes the two catchment bands at the materialisation level, and the pre-placed anchor comment still names a task that was never implemented under that name. (5) **Carried into the market API, recorded now so it is not rediscovered:** until the Critical is fixed, a catchment-band suppression flag is not proof that every input had a margin; the copy must distinguish "hidden because too imprecise" from "shown with an approximate caveat", because the same figure can be hidden at one band and shown with a caveat at another on the same card; the opportunity score stays unexposed; the backfill is now wired by name from the merged geocoder; and the nightly rebuild is the only repair path for a half-written listing until the transaction fix lands. (6) **The declared boundary is RATIFIED:** exempting the catchment-band median income from the missing-margin rule is sound and the specification supports it — a median of medians has no combined margin by construction, which is a different fact from a measurement that was never taken.

**Controller amendment A-C22 (2026-09-10 ~01:00 WITA; B4b closed — the re-review returned ALL ADDRESSED, five of five, one new Minor that does not block).** The reviewer did not take the round's word for anything that mattered: it hand-traced the combining function both ways, confirming that a set of parts where nobody reported a margin now yields an unknown margin while a genuine zero still yields zero, and it re-derived the mutation for the county-growth test itself rather than trusting the implementer's account of having broken and restored the code. So the Critical is closed at the point of loss, the suppression reaches both catchment bands and every derived figure, the rewrite is one transaction proved by rolling back partway with two real connections, and the two Minors are done. **The new Minor, carried into the market API's task rather than parked:** the score's suppression is correct — the reviewer's reasoning is accepted in full, that an income figure separately flagged unreliable must not feed an unflagged composite, and that the catchment-band income is exempt by construction so the condition can only fire where it should — but no committed test would fail if the income half of that condition were deleted, because no case suppresses income alone and then checks the score. B5 adds it. (2) **B4b's own boundary stands ratified**, and the phase's remaining work is the market API and the integration contract.

**Controller amendment A-C23 (2026-09-10 ~02:15 WITA; B5 closed — spec 18 of 18, quality APPROVED — with two Important findings that become B6's first work).** All four of the implementer's declared decisions are ratified, each after the reviewer verified the claim rather than the reasoning: the dropped layer helper really would have re-read a fact already fetched live in the same request and really does leak a pooled connection on every cache miss; the captured publish call really does test what the unobservable queue length could not; and the removed defensive branches really are unreachable, because the four dataset keys are seeded once by migration 017 and the foreign key admits nothing else. The malformed-identifier defect it found and fixed is confirmed genuine. **The two Important findings, both fixed at the start of B6, each RED first:** (1) **A licence hole of the same shape as the one growth already closes.** The veterinarians-per-household figure, and the fallback path of the establishment count, fold household data from one dataset into a row stamped with another, so withdrawing the household dataset's licence would leave them visible while growth correctly vanishes. Give them the same explicit gate, and test that each disappears when the household dataset is not cleared. This is the licence rule the programme calls legally load-bearing, so it is not a tidy-up. (2) **A published-listing check that no test can see.** The check is compounded with the cache-dedupe check in a single condition, so branch coverage reaches a hundred percent without ever exercising an unpublished listing — deleting the published half entirely would pass every committed test, and an unpublished listing's market panel would be served. Separate them and test the case. (3) **The two Minors:** the cache key gained a band segment beyond the brief's text, which is correct and proven but was never declared — record it; and the cache-hit test asserts only the header, never that the payload came back intact. (4) **Carried into the contract B6 writes:** the true cache-key format, the three-valued state with its blocked reason, the score's total absence, growth's explicit prior-vintage gate and the two figures joining it, the public-data widening, and the application-mode-only mounting.

**Controller amendment A-C24 (2026-09-10 ~03:10 WITA; the whole-phase review — READY TO MERGE, no Critical — and the two Importants fixed before the merge rather than after it).** The reviewer traced one listing from its address to a finished payload, hand-checked three formulas against the fixtures, and walked each dataset's withdrawal in turn to confirm the four licence gates remove exactly what they should. It also found something five earlier review rounds did not, which is the reason a phase review exists. **(1) The panel's top-level vintage is right today by accident.** It is read from the first row of an unordered query whose rows carry two different vintage families, so it happens to be correct only because of how those strings sort, and it is pinned by nothing. The moment one family's vintage string sorts past the other's, every panel will quietly label its figures with the wrong year — the kind of error nobody reports because nothing looks broken. Choose the vintage deliberately rather than incidentally, say in the payload which dataset it describes, and pin it with a test that fails if the row order changes. **(2) The band column has no constraint** where its sibling in the catchment table has one; add it as a new migration in the phase's range, because the constraint that exists on one table and not its twin is the one that eventually admits a bad value. Neither blocks the merge in the reviewer's judgement, and both are fixed before it anyway: this programme does not carry a known mislabelling into a release because it is small. **(3) Recorded, not fixed here:** the branch is unpushed, which the controller does at the merge; and the two deliberately unassigned scopes — the Browse map's figure feed and the community strings that still return nothing — are John's, already queued, and named plainly in the shipped contract rather than described as done.

**Controller amendment A-C25 (2026-09-10 ~03:45 WITA; the phase fixes re-reviewed — ALL ADDRESSED — and Phase B closed for merge).** The reviewer derived the index's leaf ordering itself to confirm the vintage regression test is a deterministic guard rather than a lucky one, then did the thing I most wanted: it searched every place in the phase where a single value is taken from a set — every first row, every single fetch, every dictionary built from a result — and checked each against the primary key of the table it queries. Every one is uniquely determined by its filters, and the one that looks like a first-row choice relies on Census geographies being mutually exclusive rather than on row order. So the class is closed, not just the instance. **Two pre-existing Minors are recorded rather than fixed here, with reasons.** The top-level vintage is a bare string that does not name the dataset it describes, which is how the markets route has always been and is documented in the shipped contract — changing the shape now would break a contract the next sub-project is about to build against, so it belongs to that sub-project's first conversation, not to a release-eve edit. And a figure's total absence from a payload carries no reason code, so a reader cannot tell licence-gated from never-computed from not-applicable; suppression does carry its reason, so the gap is narrow, and the fix is a payload change that again belongs with the consumer rather than ahead of it. Both are named in the contract as open. **Payload legibility is otherwise achieved:** approximate, hidden-for-imprecision and never-measured are distinguishable by named fields explained in the contract rather than in the plan, which was the test. **Phase B is closed for merge.** Every task reviewed, every review's findings fixed at source or ruled with a reason, no critical or important finding outstanding. What remains genuinely unfinished is not code: no live Census data has ever been loaded, so the whole phase is proven against recorded fixtures, and the first real load waits on the four storage variables and John's word.

**Controller amendment A-C26 (2026-09-10 ~04:35 WITA; John's three rulings on the Phase B decisions, verbatim, and the two tasks they create).** John ruled: **"Change the export"** (D-C14-SEAM — the Browse map's veterinarian and economic figures are fed by changing the export at the foot of the ported script, `frontend/src/logic.js:2030` `export { Component, MARKETS, P };`, which is the port transform's own line and not the design's, so it moves in the generator and in the byte-identity test's allowed-edit list rather than as a design amendment); **"Phase B fills the community card"** (D-C15-SERIALISE — the four Community Context strings `GET /api/listings` returns as null under D4, `app/api/listings.py:282`, are Phase B's to fill from `market_metric`, and A-C15 (12)'s open item closes); **"Phase B in parallel"** (D-C14-QUEUE — Phase B's remaining work runs beside the image-identifiability build). Two tasks follow, B7 and B8, in a worktree cut from `main` after 0.1.12: `.worktrees/feat-census-ux`, database `practice_match_censusux`, Redis `/13`, Playwright ports 5773–5775 / 8347. B7 is backend and lands first; B8 is frontend and consumes B7's two new row fields. Both are reviewed as every task before them. **What "functional" means for this phase, plainly:** with these two tasks the code path from `market_metric` to a buyer's screen is complete — the detail's Community Context card, the Browse Market data card's veterinarian and economic layers — but the tables are empty until the first live load, which still waits on the four `S3_*` variables (D-S3-QA2). Until then QA renders the design's own "Community data unavailable" card for every listing, which is the honest state, not a defect.

**Controller amendment A-C27 (2026-09-10 ~07:05 WITA; John: "please close the census gap so census data is completed with zero-gaps and the UX is functioning with the data" — the gap MEASURED, and the two tasks nobody owned).** Three readers went over the pipeline against the running code, and the QA database was read from inside the api container. **What is true today:** every Census table on QA is empty — `geo_area`, `acs_measure`, `cbp_industry`, `zbp_industry`, `bds_measure`, `qwi_measure`, `active_vintage`, `ingest_run`, `market_metric`, `practice_location`, `practice_catchment`, `geocode_cache` all at zero; only `dataset_registry` (17 licence rows) and `market_state` (6 states) hold the metadata migrations seed. **The single live-load blocker is the four `S3_*` variables**: `app/census/client.py:120-144`'s `require_archive` exits 2 for every subcommand once `CENSUS_API_KEY` is set and any of the four is missing — TIGER included, because the key is set. The load itself is small: roughly 111–115 keyed API calls plus 17 static downloads, and the whole sequence is documented at `DEPLOY.md:352-384`, to be run by the controller inside the worker on John's word.

**The gap the readers found, which no task owned and which would have left the UX empty even after a perfect load.** `census.geocode_listing` exists (`app/tasks/census.py:301`, registered at `:405`) and correctly enqueues `census.backfill_listing` when it succeeds — but **nothing in the repository ever enqueues `geocode_listing`**: not a route, not a CLI subcommand, not the seeder, not the seller's publish. `materialize_all` iterates `practice_location` (`app/census/materialize.py:299`), which is populated only by geocoding, and `census_load.py` has a `materialize` subcommand but no `geocode` one. So the chain is broken at its first link for every listing that already exists: eighteen seeded hospitals would sit with no location, no catchment and no metric rows however much raw data were loaded, and the panel's backfill-on-miss (`app/api/market.py:379`) cannot help, because it rebuilds catchments from a point that was never resolved. **Task B9 closes it, at both ends.**

**Task B9: the geocode trigger — the operator's and the product's — 1 day.** In `.worktrees/feat-census-geocode` (database `practice_match_geocode`, Redis `/2`, no Playwright). (1) **`census_load.py gains a `geocode` subcommand**: with no argument it resolves every listing that has no `practice_location` row, one at a time, printing one line per listing (id, the rung the ladder reached, whether a `geocode_review` row was written); with `--listing <id>` it does one; with `--force` it re-resolves a listing that already has a row. It calls the same `app.census.geocode` code the task calls — no second implementation — and it enqueues nothing: it geocodes synchronously and then calls `catchment.build` and `materialize.materialize_listing` for that listing, so one command takes a listing from an address to its figures. Exit codes follow the file's own table (0 done, 2 refused, 3 database, 4 fetch, 5 validation). It refuses with a named reason, never a traceback, when `tiger_cb` has no active vintage — the state the reader found would otherwise raise `RuntimeError` deep inside. (2) **A listing is geocoded when it is published.** The publish path (`app/api/seller_listings.py`'s transition into `published`, and the admin reviewer's publish in `app/api/admin_listings.py`) enqueues `census.geocode_listing` by name, after the transaction commits, exactly as the upload path enqueues its own work; a listing that already has a `practice_location` row is not re-geocoded. This is what makes a NEW seller listing carry market figures without an operator, and it is the product behaviour the spec always implied. (3) Tests: the subcommand over a listing with no row, one already resolved (skipped), `--force` (re-resolved), the refusal when no `tiger_cb` vintage is active, and the publish enqueue asserted on the captured `send_task` call for both publishers — plus one that a re-publish of an already-located listing enqueues nothing.

**Task B10: the attribution the licence rule requires, and the label the design hard-codes — 0.5 day.** Two things the readers found that are John's rule rather than a preference. (1) `CLAUDE.md`'s "Legally load-bearing" says the Community Context attribution "will come from `dataset_registry.attribution_text`, not hard-coded, once Sub-project 3 lands" — this is Sub-project 3, and the string is still the design's own copy. The listing row carries the attribution for the datasets its figures actually came from (the panel already does this: `app/api/market.py:317` builds `attribution` from `reg[k]["attribution_text"]` for the datasets it used), and the design's hard-coded line is replaced by that value through a ruled amendment. (2) The design's growth tile says "Since 2015" as fixed copy while B7 derives "since 2018" from the prior vintage; one of them must move, and it is the design's, by the same mechanism. Both are amendments in family **A21**, which this phase reserves. If either turns out to move a frozen screen, the implementer STOPS with `NEEDS_CONTEXT` rather than re-pinning.

**What remains John's, and cannot be closed from here:** the four `S3_*` variables on the QA worker. Everything above is code; none of it puts a number on screen until the raw data exists.

**Controller amendment A-C28 (2026-09-10 ~07:15 WITA; the B8 review — the "safe default" that would publish a false statement, and the two labels that would publish a wrong one).** B8's implementer was told to STOP if a missing figure rendered badly and instead recorded that it is "handled safely via logic.js's built-in `|| 0` defaults" and shipped. It is not safe, and the reason matters more than the fix. `logic.js:394-395` reads `econ: (ECON_K[p.id] || 0) * 1000` and `vets: VETS[p.id] || 0`; `marketVals` (`:513-516`) then buckets that value and labels it, and the Veterinary Competition layer's own caption is "Number of veterinary establishments". **Before B8 this never fired** — the design's fixtures carry a value for all twenty-one keys — but B8 makes the maps real, and a community whose figure is missing is now drawn in the LOWEST-competition colour with the label "0". A buyer reads that as "nobody else practises here". It is the most commercially consequential sentence this product could say about a market, it would be said about places we simply have no data for, and it would be said with the confidence of a number. Absence must never be rendered as zero. **Family A21, three entries, and B8's fix round owns them.**

**(1) A21.1 — a missing figure is omitted, never zeroed.** A practice whose `VETS`/`ECON_K` key is absent is EXCLUDED from that value layer rather than bucketed at zero: no colour, no label, no contribution to the legend's spread. The design already distinguishes a layer being off from a layer being on, and `mapVals`'s consumers already filter; the implementer establishes the exact mechanism by reading how `values[k]` is consumed for a marker and either omitting the entry or carrying an explicit no-value marker the renderer already understands — and if NEITHER is expressible without inventing UI, it STOPS with `NEEDS_CONTEXT` quoting the lines, because the alternative is publishing a false figure. The two `|| 0` expressions go; a fixture-only render (the reference, which passes no API) keeps every pixel, because its keys are all present.

**(2) A21.2 — the economic layer says what the number is.** The design's label is "Average Practice Payroll (CBP)" and B7 feeds it `revenue_per_establishment`, which is revenue, not payroll — B6's contract says so in as many words and A-C26 told B7 not to relabel it. Piping revenue into a layer captioned payroll is a factual error on a map. The label becomes "Average Practice Revenue (CBP)" and the layer's `short`/`caption` follow, as a ruled literal edit. If John prefers the payroll figure instead, that is a different task in the pipeline, not a relabel here — queued as D-C28.

**(3) A21.3 — the growth layer stops naming a year it does not use.** "Population Growth Since 2015 (ACS)" is fixed copy while the figure derives from whichever prior vintage is active (2018 today). The label becomes vintage-neutral — "Population Growth (ACS)" — and the year, where the design shows one, comes from the same place B7's `growth` string takes it. The detail card's own "since 2018" text B7 already produces is unaffected.

**Every one of the three is a literal edit through the amendment engine, family A21, and no approved screen may move: the reference passes no API and its fixtures are complete, so a moved `baseline-manifest.json` hash means a code change reached further than the ruling and is a STOP, not a re-pin.**

**Controller amendment A-C29 (2026-09-10 ~09:05 WITA; I got A-C28 (2) WRONG, and this reverses it before it ships).** I ruled that the Browse economic layer's label must change from "Average Practice Payroll (CBP)" to revenue, on the grounds that B7 feeds it `revenue_per_establishment` and that piping revenue into a payroll caption is a factual error on a map. I reached that by reading the identifier's NAME and not its definition. `app/census/metrics.py:116` says in as many words: *"Annual payroll (reported in thousands) per establishment, in dollars — a payroll proxy, not true revenue"*, and it computes `payroll_k * 1000 / estab`. The materialiser stamps every row it writes with `"note": "payroll per establishment, not revenue"` (`materialize.py:269`). **The figure is payroll. The design's label was right all along, and my ruling made the UI say something untrue.** The metric key is simply misnamed — a name chosen in Phase A and now carried in `market_metric` rows and in the shipped contract, which is why it is not renamed here.

**(1) A21.2 and A21.2b are REVERTED.** The layer label returns to "Average Practice Payroll (CBP)", its short form and the market-data radio to "Payroll". Nothing else in the payroll apparatus is touched, and this is why that matters: the design's own explanatory text (`logic.js:183`) tells the buyer *"It is not revenue, and not any individual practice's…"*, the source line credits CBP payroll and establishment counts, and — the part a relabel would have quietly broken — `VALUE_LAYERS.econ`'s buckets (`< $450K`, `$450–650K`, `$650–900K`, `> $900K`) and the detail chip's Strong/Typical/Lean thresholds at 650,000 and 450,000 were calibrated for PAYROLL magnitudes. Relabelling the axis without recalibrating them would have coloured every market wrongly while sounding more precise. **(2) B6's contract is corrected instead**, because it states the falsehood my ruling came from: it says `econ_k` is revenue per establishment and explicitly "not payroll". It becomes payroll per establishment in thousands of dollars, with one sentence recording that the column is historically named `revenue_per_establishment` and that the name is wrong, not the number. **(3) A21.3 stands and was incomplete.** "Since 2015" survives in three more user-facing places: the layer's sub-line (`logic.js:164`), the Data Layers card's blurb and caption (`:480`), and — a real defect, not just wording — the detail's Growth row (`:1504`), which does `(p.growth || "").replace(" since 2015", "")` and labels the field "Since 2015". B7 now returns "+11.6% since 2018", so the strip no longer matches: the value renders with its year still attached, inside a field captioned with a different year. All four move to the vintage-neutral form, and the detail takes the year from the string the API already supplies. **(4) The lesson, recorded because it cost a round:** a metric's name is not its definition. Before ruling on what a figure MEANS, read the function that computes it and the note the writer stamps on it.

**Controller amendment A-C30 (2026-09-10 ~09:30 WITA; B9 closed — spec 5 of 5, quality APPROVED — with one Important the review only found when pushed).** The reviewer confirmed both of the controller's own deletions are behaviour-identical (the identity ladder, and the inner handler the outer one and its parameterised guard already own), sampled twelve tests and found each load-bearing, and verified the enqueue is on the Celery application object. Its first answer to "is there any remaining path by which a listing becomes published and never gets geocoded" was "no". **That was wrong, and it is the same shape as the gap this task exists to close.** `scripts/seed_listings.py` INSERTs all eighteen hospitals directly as `published` and never geocodes: a trigger on a TRANSITION cannot see a direct insert. Re-asked, the reviewer graded it Important and enumerated the writers — the two API transitions, the seeder, and test helpers that do not matter outside tests. **The ruling, taking the reviewer's recommendation and its reasoning:** the seeder does NOT enqueue. It is a bootstrapping tool that runs where no worker may be listening, and silently queueing work nothing will consume is worse than not queueing it. The boundary is the operator command — `census_load.py geocode`, which resolves every listing lacking a location, is idempotent, and now exits 0 when there is nothing to do precisely so it can be run repeatedly. **What must change before this merges:** `DEPLOY.md` mentions the `geocode` subcommand nowhere — not in the Census sequence, not after `activate`, not in "Seeding the demo hospitals (QA)". So an operator can seed eighteen hospitals, follow the documented sequence to the letter, and get no market data anywhere with nothing saying why. That is the gap moved from the code to the runbook, which is not closing it. The sequence gains the step after `activate` (figures need the vintages first), the seeding section gains a line pointing at it, and `tests/test_docs.py` pins both so the runbook cannot drift from the command again.

**Controller amendment A-C31 (2026-09-10 ~12:15 WITA; the end-to-end gate caught what four rounds of unit tests could not — the docked panel still fabricates).** With the stale servers cleared, the census branch's end-to-end run is 147 passed, 4 failed, and the four are one defect seen twice: `browse-market-panel` and `browse-panel-lightbox`, in both the structural and the pixel oracle. The structural diff says it plainly — where the reference renders `7` the app renders **`undefined`**, where the reference renders `2.5` the app renders **`NaN`**, and where the reference says `Strong` the app says **`Lean`**. **A-C28 was applied to the map layers and not to the docked panel, which reads the same figures by a different path.** `logic.js:989` takes `c = selComm || comms[0] || {…, vets: 0}` — a real community from the API now carries `undefined` for a figure it has no data for — and then `:990` computes `per10k = c.vets / (c.hh/10000)` → `NaN`, `:1053` does `compEstab: String(c.vets)` → the literal string `"undefined"`, and `:1062` evaluates `c.econ > 650000 ? "Strong" : c.econ > 450000 ? "Typical" : "Lean"`, where `undefined > n` is false twice, so a market nobody has measured is labelled **Lean Competition**. That is the same false statement A-C28 exists to forbid, dressed as a verdict rather than a zero, and the unit tests could not see it because they never render the panel against a listing with no metrics. The end-to-end suite could, because it runs the real API against the eighteen seeded hospitals — which have no market data, which is exactly the state of QA today.

**The ruling, family A21 continued.** (1) **Per figure, absence renders as the design's own empty treatment, never a fabricated value.** `compEstab` shows the design's empty form rather than `String(undefined)`; `per10k`, `compLevel` and `compFill` are not computed when `vets` or `hh` is absent, and the competition row is omitted rather than shown at a guessed level; the payroll tile is omitted when `econ` is absent rather than resolving to `Lean`; the same rule applies to every `oppTiles` entry and to `overviewTiles`, each of which reads a figure directly (`c.growth.toFixed(1)` throws outright on undefined, so check that one first). (2) **When a listing has NO community figures at all** — the common case on QA right now — the Insights tab shows the design's own "Community data unavailable" treatment, the one the detail already reaches through `hasDemo` at `:1500`; reuse it rather than inventing a second empty state. (3) **The reference must keep its pixels:** it passes no API and its fixtures are complete, so every approved capture stays byte-identical and `baseline-manifest.json` must not move. (4) **The test that proves it is an end-to-end one**, not a unit test: the suite already runs the app against eighteen listings with no metrics, so the assertion is that no rendered panel anywhere contains `undefined`, `NaN` or a competition verdict for a listing without figures. A unit test may accompany it; it may not replace it. **(5) The lesson, recorded twice now in one day:** a fix for "absence must not be fabricated" has to be applied at every reader of the value, and the only gate that finds the readers you forgot is the one that runs the real app against real empty data.

**Controller amendment A-C32 (2026-09-10 ~13:40 WITA; John's two rulings after the first live load, and the task that carries them).** The load put real figures on seventeen of the eighteen hospitals. The eighteenth, the Orlando specialist centre, geocoded to ROOFTOP like every other — its address is not wrong in any way — but it sits in unincorporated Orange County, where the Census has no `place`, so it has no `place`-band row. It does have complete `drive_10` and `drive_20` figures. Two rulings follow, and they are deliberately different responses to two conditions this session nearly conflated. **(1) D-C32 — the no-place fallback (this task, B10).** A listing with no `place`-band figures falls back to its `drive_10` band, and the card SAYS SO: the heading reads "Within 10 minutes of the practice" instead of a community name, so a buyer is never shown a drive-time area disguised as a named city, and never shown an empty card for a market we can describe. The Orlando listing then reads 167,997 people, 59,588 households, $69,780 median income, +9.0 % growth and 10.8 veterinary establishments — real figures it has had all along. If neither band has figures, the design's own unavailable card stands. **(2) D-SL38 — a poor geocode is the seller's business, and only then.** An address that resolves BELOW rooftop, or fails, is surfaced to the seller at submit while they still own the draft, and to the reviewer in the queue, from the `geocode_review` rows the pipeline already writes and nothing has ever read. That is a seller-lifecycle and admin surface, NOT this task, and it is recorded here only so the two are never merged into one ticket: telling the Orlando seller their address "cannot be mapped" would be false and would send them chasing a problem that does not exist.

### Task B10: the drive-time fallback, and the docked panel stops fabricating — **1 day**

**Files:** modify `app/census/serve.py` (the band choice and the label the row carries), `app/api/listings.py` (`serialise` passes the label through), `frontend/tests/design-amendments.ts` (family A21 continues — the panel's own readers), `docs/integrations/market-data-api.md`; tests in `tests/census/test_serve.py`, `tests/api/test_listings.py`, and an end-to-end assertion.

**This task carries A-C31 as well**, because it is the same case: the docked panel renders `String(undefined)`, a `NaN` ratio and a false "Lean" verdict for a listing with no figures (`logic.js:989`, `:990`, `:1053`, `:1062`). Once the fallback lands, a listing with drive-time figures is no longer that case — but a listing with NEITHER band still is, so both must be fixed together and proved together.

- [ ] **RED, the serialiser:** a listing with `place` rows uses them and the label names the community; a listing with only `drive_10` rows uses those and the label reads "Within 10 minutes of the practice"; a listing with neither returns the six nulls it returns today. Drive the third case with the Orlando shape — rooftop location, no `place_geoid`, full `drive_10` band.
- [ ] **GREEN** in `serve.py`: the band is chosen per listing, not per request, and the chosen band travels with the row so nothing downstream has to guess which one it got.
- [ ] **RED, the panel:** drive `marketPanel` with a community that has no `vets` and no `econ` and assert the rendered output contains no `undefined`, no `NaN`, and no competition verdict; assert the same for `compEstab`, `per10k`, `compLevel`, `compFill` and every `oppTiles` and `overviewTiles` entry that reads a figure directly — `c.growth.toFixed(1)` throws outright on undefined, so cover that one first.
- [ ] **GREEN** through the amendment engine, family A21, never by hand-editing `logic.js` and never by teaching a gate to accept a difference (A-C29's lesson, 2026-09-10).
- [ ] **The end-to-end proof:** the suite already runs the real API against the seeded hospitals; assert no rendered panel anywhere contains `undefined`, `NaN`, or a verdict for a listing without figures.
- [ ] **Gates and commit** as every task: ruff, mypy strict, the frontend gates, baselines and e2e with `baseline-manifest.json` unmoved, then the backend gate once, gated on its exit code.
...
### Task B7: the buyer serialiser fills the Community Context card, and carries the two Browse figures — **1 day**

**Files:**
- Create: `app/census/serve.py` — `community_rows(conn, listing_ids, *, active) -> dict[str, CommunityRow]`, one batched query, the gate logic shared with `app/api/market.py`
- Modify: `app/api/market.py` — `listing_market()` reads the same figures through `serve.py` where it can, so there is ONE place that decides what a buyer may see (no second copy of `_extra_cleared`)
- Modify: `app/api/listings.py` — `serialise()`'s four community strings and two new fields `vets` and `econ_k`; the list route fetches `community_rows` once per page
- Modify: `docs/integrations/market-data-api.md` — the listing row's six fields, and `tests/api/test_contract_doc.py` if the drift test's required-name list needs them
- Test: `tests/census/test_serve.py` (new), `tests/api/test_listings.py` (the four strings and two fields, populated and unavailable), `tests/perf/test_api_latency.py` (the list budget must hold with the extra query)

**Interfaces:**
- Consumes: `market_metric` (061/062/063), `dataset_registry.license_status` (017); the active vintages and the registry exactly as `app/api/market.py` reads them today — `act = await _active(conn)` and `reg = await _registry(conn)` (`market.py:215`), so `act.get("acs5")`, `act.get("acs5_prior")`; `app/api/market.py:191` `_extra_cleared(reg, metric_key, source_dataset) -> bool` (move it into `serve.py` with `_active`/`_registry` if they are private to `market.py`, and import them back — every function keeps its name and its tests)
- Produces: `CommunityRow = TypedDict(pop: str|None, growth: str|None, income: str|None, hh: str|None, vets: int|None, econ_k: int|None)`; `serialise()` emits exactly those six keys on every listing row, null when unavailable — the design's `A12.10/A12.11` path renders the "Community data unavailable" card whenever `pop` is null, so nulls are the contract, never empty strings

**The rules, copied from the phase so the implementer need not read it:**
- Band: the `place` band (the listing's own community) — the card names the community, not a drive time.
- Vintage: the active `acs5` vintage (`act.get("acs5")`), chosen deliberately as A-C24 did for the panel — never the first row of an unordered query.
- Licence: a figure whose `source_dataset` is not `cleared` in `dataset_registry` is null. `vets_per_10k_households` and the `cbp`-fallback `establishments` additionally require `acs5` cleared (`_extra_cleared`, A-C23 (1)). `population_growth_pct` requires `acs5_prior` (A-C23). Nothing is ever "approximately shown" here: the card has no caveat affordance, so a `suppressed` OR `approximate` metric is null in the card (the panel, which has the affordance, keeps showing approximate figures — this task does not touch that).
- Formatting, exactly the design's fixtures' register (`logic.js:11`): `pop` → `f"{int(round(v)):,}"` ("142,300"); `hh` → `f"{int(round(v)):,} households"`; `income` → `f"${int(round(v)):,}"`; `growth` → `f"{v:+.1f}% since {prior_end_year}"` where `prior_end_year` is the last four characters of the `acs5_prior` vintage string ("2019–2023" prior "2014–2018" → "since 2018"); `vets` → `int(establishments)`; `econ_k` → `int(round(revenue_per_establishment / 1000))` (the design's ECON_K is annual payroll per establishment in $K — B4b's `revenue_per_establishment` is the figure the phase actually computes and the contract must SAY so; do not relabel it as payroll).
- Performance: ONE query per page — `WHERE listing_id = ANY(%s) AND band = 'place' AND vintage = %s`; the `market_metric_lookup_idx` serves it (prove with `EXPLAIN` in the query-plan harness, `tests/perf/test_query_plans.py`, one new case). No per-row query, no Redis cache on the list route (the panel's 24 h cache stays the panel's). The existing budget is `tests/perf/test_api_latency.py:362` `LISTINGS_BUDGET_MS = {"list": 100, "one": 100, "photo": 150}` — both `list` and `one` must still hold with the extra query, measured by the test that already exists; the numbers are not raised.
- `MARKET_DATA_PUBLIC` stays false: the list route is already member-gated by `listing.read`; the card's figures ride that gate. `opportunity_score` appears nowhere.

- [ ] **Step 1: RED — `tests/census/test_serve.py`**: seed one published listing (the `make_listing` fixture, which carries `sqft`/`est`/`price` since the 0.1.12 fix) with `place`-band rows for population, households, median_income, population_growth_pct, establishments (`zbp`), revenue_per_establishment; all datasets cleared → `community_rows` returns the six formatted values with the exact strings above. Second case: `acs5` set to `pending` → every figure null except none (establishments is `zbp`-stamped but its fallback path is `cbp`; assert the `zbp` case stays and the `cbp`-fallback case nulls). Third: `acs5_prior` not cleared → `growth` null, others present. Fourth: a suppressed population (`suppress_reason = 'high_moe'`) → `pop` null AND `hh` present (independent). Fifth: two listing ids, one with rows and one without → the second is absent from the dict (the serialiser turns absence into six nulls). Run: `poetry run pytest tests/census/test_serve.py -q -W error` — FAIL with `ModuleNotFoundError: app.census.serve`.
- [ ] **Step 2: GREEN — `app/census/serve.py`** with `community_rows` and the moved `_extra_cleared`; `app/api/market.py` imports it from there (its own tests unchanged and green).
- [ ] **Step 3: RED — `tests/api/test_listings.py`**: `GET /api/listings` and `GET /api/listings/{id}` carry `pop`, `growth`, `income`, `hh`, `vets`, `econ_k`; populated for a listing with rows; all six null for one without; the perf budget case `tests/perf/test_api_latency.py::test_listings_p95_within_budget` (or its current name) still holds with twenty seeded listings on the page. Run and watch the KeyError / null assertions fail.
- [ ] **Step 4: GREEN — `serialise()` and the list route**: the route calls `community_rows` once with the page's ids and hands each row its entry (or `None`).
- [ ] **Step 5: the query plan**: add the `community_rows` SQL to `tests/perf/test_query_plans.py`'s cases; assert the index scan the harness expects; if the planner picks the primary key instead, record which and why in the report — do not force it.
- [ ] **Step 6: the contract**: `docs/integrations/market-data-api.md` documents the six row fields, their formats, that `econ_k` is revenue per establishment in $K (not payroll), and the null-means-unavailable contract; `tests/api/test_contract_doc.py` green.
- [ ] **Step 7: gates** — ruff, mypy strict, the full backend gate once (exit code gates the commit). Commit with explicit pathspecs.

### Task B8: the Browse map's veterinarian and economic layers read the API — the export changes — **1 day**

**Files:**
- Modify: `frontend/scripts/convert-dc.mjs` — the port transform emits the footer `export { Component, MARKETS, P };`; it becomes `export { Component, MARKETS, P, VETS, ECON_K };`
- Modify: `frontend/tests/app-generated.test.ts:51` — `const FOOTER = '\nexport { Component, MARKETS, P };\n'` pins the footer byte for byte; change the pin FIRST so it fails, then the generator (if it does not fail first, the pin is not a pin)
- Modify: `docs/superpowers/specs/2026-09-06-browse-v3-mobile-design.md` §3 — the allowed-edit list names the footer line verbatim; it names the new one — and `frontend/tests/cross-plan-deltas.test.ts:418`, which asserts the spec contains that literal, moves with it (the four places move in ONE commit; a fifth `grep -rn "export { Component, MARKETS, P }"` across `frontend/` and `docs/` must find nothing else)
- Modify: `frontend/src/listings/load.ts` — `applyListings()` also installs `VETS[p.id]` and `ECON_K[p.id]` from the row's `vets` / `econ_k` (B7's fields), and DELETES the design's fixture keys (`p1…g4`) when the API replaces `P`, the same way it empties the prototype's practices — a stale fixture key must never colour a real listing's layer
- Modify: `frontend/src/listings/load.ts`'s `ApiListing` interface — `vets: number | null; econ_k: number | null`
- Test: `frontend/tests/listings-load.test.ts` (or the existing load tests) — install, null handling, fixture-key removal; `frontend/tests/app-generated.test.ts`; the visual + DOM + smoke gates unchanged

**Interfaces:**
- Consumes: B7's `vets` and `econ_k` on every listing row; `logic.js:64` `const VETS = {…}` and `:71` `const ECON_K = {…}`, keyed by listing id, read by the Market data card's veterinarian and economic layers
- Produces: nothing new for others; this closes D-C14-SEAM

**Before writing a line, characterise the design (this is the step a reviewer will check first):** read every use of `VETS[` and `ECON_K[` in `frontend/src/logic.js` and write down what the design renders when the key is MISSING (undefined). If a missing key renders a blank or `NaN` rather than the design's own no-data treatment, STOP with NEEDS_CONTEXT and quote the lines — that is a design amendment (family A21 is reserved for this phase) for the controller to rule on, not something to patch in `load.ts`. If a missing key already renders acceptably (for example the layer simply omits the marker), record that in the report with the line numbers and proceed.

- [ ] **Step 1: RED — the pin**: change the expected export line in `app-generated.test.ts` first; run `cd frontend && npx vitest run tests/app-generated.test.ts` — FAIL (the generated file still has the three-name export).
- [ ] **Step 2: GREEN — the generator**: change the transform's emitted line; `npm run gen:app`; the pin passes; the byte-identity test passes for every other byte.
- [ ] **Step 3: RED — `load.ts`**: a test that hands `applyListings` two API rows (`vets: 7, econ_k: 685` and `vets: null, econ_k: null`) and asserts `VETS[id1] === 7`, `ECON_K[id1] === 685`, `id2 in VETS === false`, and that `VETS.p1` (a fixture key) is gone after the call. Run — FAIL (`vets` not on the interface; nothing installed).
- [ ] **Step 4: GREEN**: the interface fields and the install/removal in `applyListings`.
- [ ] **Step 5: gates** — `npm run typecheck && npm test && npm run build`; then `npm run test:visual:baselines && npm run test:e2e` on this worktree's ports: every approved state keeps its pixels (the design's own fixtures are untouched — the reference passes no API rows), `baseline-manifest.json` does not move. Commit with explicit pathspecs.
...
| Median Household Income (`income`) | community boundary shading (real `geo_area` polygons through `L.geoJSON`, `fillOpacity: 0.5`), **ZCTA (860)**, green ramp; legend buckets `< $50K / $50–75K / $75–100K / $100–150K / > $150K` (five classes — V3 widened income's ramp from V2's four; every other layer keeps four) | `communities[].income` | `market_metric.median_hh_income` (band `place` by default) | place (drive bands on request) | ACS attribution + vintage; `approximate` for catchment bands | `acs5` cleared |
| Pet Ownership (est.) (`pets`) | graduated symbols at the listing point (D-C35), orange | `communities[].pets` | `pet_households_est` (derived, rate 0.57) | place | ACS attribution; "Derived estimate: households × 0.57 …" | `acs5` cleared |
| Population Growth (`growth`) | community boundary shading (real `geo_area` polygons through `L.geoJSON`, `fillOpacity: 0.5`), **place (160)**, **brown** ramp (C10: teal read as a second green beside income); buckets `Declining / 0–5% / 5–15% / > 15%` (D-C46, 2026-09-11 — re-scaled to real ACS data, with a band below zero) | `communities[].growth` | `population_growth_pct` (derived) | **place** (D12) | "ACS 2014–2018 → 2019–2023 …" | `acs5_prior` cleared |
| Households (`households`) | graduated symbols at the listing point (D-C35), blue | `communities[].hh` | `households` | place | ACS attribution | `acs5` cleared |
| Average Practice Payroll (`econ`) | community boundary shading (real `geo_area` polygons through `L.geoJSON`, `fillOpacity: 0.5`), **county (050)**, rose ramp; buckets `<$450K … >$900K` | `communities[].econ` | `revenue_per_establishment` (CBP payroll ÷ establishments) | **county** | CBP attribution; "Payroll per establishment, not revenue; county level." | `cbp` cleared |
| Veterinary Competition (`competition`) | graduated symbols at the listing point (D-C35), purple (`#e9e2f6 · #c3b0e6 · #9a7ed4 · #7856be`); buckets `1–2 / 3–5 / 6–9 / 10+`; select label "Veterinary competition", legend title "Veterinary Establishments (CBP)"; `rf-tip` on hover; panel bars Low/Moderate/High (C5 — V2's offset `dot()` bubble pass is gone) | `communities[].vets`, `communities[].competition.{count, per_10k_households, level}` | `establishments` from **ZBP** ZIP counts aggregated over the community's ZCTAs (fallback: county CBP apportioned by household share, labelled derived). Phase C (Task C1, D17): `competition.freshness.{level_live, diverges, as_of}` from the Places Aggregate API — a bucket and a flag, never a count | ZCTA→place / catchment | ZBP attribution; "… proxy for competitive density, not a count of independent practices. ZIP-code counts aggregated to the community." Freshness: the `google_places_aggregate` attribution text | `zbp` cleared (fallback needs `cbp`); `freshness` only while `google_places_aggregate` is cleared |
...
|---|---|---|---|
| ~~Tract choropleth vector tiles~~ **— PROMOTED OUT OF PHASE C, 2026-09-10** | §2b, §7, §10 | Triggered: John, on seeing the mosaic — "the data is not to the granular level required at neighborhood level". The four rulings D-C34 to D-C37 are below; the work is Phase D. **A tract toggle stays deferred here** under D-C34, and its own trigger is a measurement: the share of tract polygons whose margin of error spans more than one legend band. Tract-level `growth` stays deferred independently — it needs the 2010→2020 crosswalk, its own row further down this table. |
...
**Controller amendment A-C33 (2026-09-10 ~15:30 WITA; John, on the market-data map: "the data is not to the granular level required at neighborhood level — right now it's just a blob over the whole city and misses the entire point", then, correcting himself, "it is SUPPOSE to look like this" against the same screenshot).** The screenshot is the target, not the defect. Three facts, each read from the source rather than the screen.

**(1) It was built as this plan says, and this plan defers it.** Phase C's first row reads "Tract choropleth vector tiles · §7 'Choropleth tiles', §10 CDN row · Trigger to build: **the approved design gains tract-level shading (today it shades community mosaic cells)**". The deferral is explicit and its trigger names the present state exactly. So nothing was skipped or forgotten: the shading John is looking at is the shading this plan scheduled, and the neighbourhood shading he expected is the row directly above the line. The plan is what was wrong, not the build.

**(2) What is on screen today is not Census geography at all.** `frontend/src/map/mosaic.js` carries its own header, ported verbatim from the prototype's `MarketMapV3.jsx`: the prototype has no ZCTA boundary file, so community areas are approximated as the grid cells nearest each community's centroid, clipped to the metro bounding box, at `MOSAIC_STEP = 0.0055°`. Those cells "are NOT real Census boundaries". Measured against real fixtures: two listings draw 3,070 rectangles in two colours; five listings draw 7,989. The blob is a listing-count artefact — it grows with how many practices are on the map, not with the data.

**(3) Only three of the nine layers are shaded at all, and their honest geographies differ.** `logic.js:118` — `FILL_KEYS = ["income","growth","econ"]`; `:121` — `SYMBOL_KEYS = ["pets","households","competition"]`, which are graduated symbols at the listing point and must stay that way (moving city-scale class breaks onto small-area polygons produces a picture with no information: the first `< 10K` households bucket swallows essentially every tract). Of the three fills, `income` is tract-honest subject to margin of error, `growth` is **place only** until the 2010→2020 tract crosswalk lands (itself a Phase C row), and `econ` is **county only** — CBP carries industry detail at no finer level. §6's standing rule applies: "Never silently promote a county figure into a tract-labeled slot."

**Consequence.** The Phase C row is promoted out of Phase C and rewritten. The unit of shading, whether coarser layers draw at their own geography or disappear, whether the design's published dollar bands survive contact with small-area estimates, and whether this is served as an endpoint or as CDN tiles are four rulings for John; they are recorded as D-C34 to D-C37 when he gives them. The GEOMETRY needed for the first slice — ZCTA, place and county polygons for the four markets — is already in `geo_area` today. **The VALUES are not, and this sentence was too broad when first written.** `app/census/acs.py` loads ACS at summary levels 140, 160, 050, 040, 310 and 010, and its `geoid()` raises on anything else; there are no ZCTA measures. Correcting it here rather than quietly: D-C34 is unaffected — ZCTA is still the right unit — but the first slice carries an ACS load at 860, not merely a join. The alternative, aggregating tract figures up to ZCTA, is refused: a weighted median has no combined margin of error by construction, so the result could never be suppressed, which would hollow out D-C36 for the one layer it governs. Recorded as D-NS4 in the spec.


### John's four rulings on neighbourhood shading (2026-09-10 ~15:50 WITA)

Put to him with the measurements behind each, and answered in one pass. They are recorded as decisions, not as sketches: each one selects different software, and the differences are named.

**D-C34 — the shading geography is ZCTA, with tract as a later, evidence-gated refinement.** John chose "Both, ZCTA first". ZCTA ships now: spec §2b already rules it ("the level buyers think in, and it matches the granularity of the reference design"), it is ~130 polygons across the Austin metro rather than 376 tracts in one z11 viewport, its denominators are large enough to survive the margin-of-error test, and ZIP is the unit buyers and sellers already speak in. A tract toggle follows, and **its trigger is a measurement, not a date**: the share of tract polygons whose margin of error spans more than one legend band, measured under D-C36 and reported before the toggle is built. The geography selector this implies — in the API, the legend and the layer menu — does not exist in the approved design and will be a ruled amendment when the toggle is built, not now.

**D-C35 — every layer draws at its own geography, always, and the legend names it.** `income` on ZCTA polygons (summary level 860), `growth` on place polygons (**160** — 150 is block group, which is loaded nowhere in this tree; an earlier draft of this ruling wrote "150/160" and that was wrong), `econ` on county polygons (050). No layer is hidden at a zoom, and no county figure is ever painted across the small polygons inside it. This is spec §6's standing rule applied to the map: "Never silently promote a county figure into a tract-labeled slot." It needs no new ingest — all three geographies are in `geo_area` today — and it makes the legend the first thing in the product to read `/api/layers`. The three graduated-symbol layers (`pets`, `households`, `competition`) are NOT moved onto polygons: city-scale class breaks on small areas produce a picture with no information, and `logic.js:107-113`'s first `< 10K` households bucket would swallow essentially every polygon.

**D-C36 — keep the design's published dollar bands, show the margin of error in the hover tip, and measure.** The bands are dollar-meaningful and legible, and they are what the design published. The honesty is carried by the tip, not by re-cutting the legend. `materialize._suppression` is reused verbatim — no second implementation of the CV/Z90 test, because two suppression code paths that disagree would put "$72,400" in the docked panel over a grey polygon. A polygon whose margin spans more than one band is shown with its value and a caveat, not greyed: greying a measured figure is its own false statement. The measurement this produces is what gates D-C34's tract toggle.

**D-C37 — served as a member-gated endpoint, not as CDN tiles.** `MARKET_DATA_PUBLIC` is false, so tiles would have to be member-gated anyway, and spec §10's "30 days, immutable" CDN row **directly contradicts §11's "the layer disappears within one minute"** the moment a tile carries values rather than only geometry. One route inside the existing `market:gate:v` cache key honours both. The tile pipeline was measured and is real — 412 tiles and 591 KiB for the whole Austin metro at z9–z12 per metric per vintage, a z11 viewport costing 34 KB against 424 KB of GeoJSON — and it buys nothing at four metros and ~130 polygons. It is recorded here as the scaling answer, with its own trigger: national coverage, or a metro whose polygon count makes a viewport response exceed 500 KB gzipped.

**What is deliberately NOT in the first slice**, so nobody reads these rulings as wider than they are: tract polygons; vector tiles, the CDN path and a vector-tile client; any change to the three graduated-symbol layers; tract-level growth; the Esri-vs-CARTO basemap decision, which boundaries do not force; the Satellite tab; national coverage beyond the four markets and six states; and any exposure of the opportunity score.

**Two measured facts that bound the work.** The current mosaic draws 3,070 rectangles for two listings and 7,989 for five — real polygons are one to two orders of magnitude FEWER objects, so this is a performance improvement, not a cost. And the zero-pixel visual gate, not the backend, is the largest single line item: roughly ten approved Browse states carry a map, the reference draws the fill too, so the change must reach the design through an amendment or the gate cannot pass — and `baseline-manifest.json`'s thirteen frozen hashes must not move, which is the proof the change stayed inside Browse.


### John's two rulings on the Community Context card's geography (2026-09-11) — D-C38, D-C39

**What he was shown.** The Foxtrot listing, headed *"Specialty practice — Highland Park / affluent central"*, whose Community Context card read **Population 1,299,553 · Growth −1.5% · Median income $67,760 · Households 528,038** — the City of Dallas, and the same four numbers on all twelve Dallas listings, South Dallas and Southwest Dallas included. Their own catchment figures were already materialised in `market_metric` and ran from **$54,826** (Echo, "Southwest Dallas") to **$109,548** (Foxtrot), a 2.0× spread no screen in the product could reach: nothing under `frontend/src` requested a band. The cause is D-C32's whole-row rule (`serve.py`, plan line 2613), written for one condition — the Orlando specialist centre in unincorporated Orange County, which has no Census place at all — and never asked what it does to a listing INSIDE a large city.

**He was also told that leaving it alone is not neutral, and that mattered.** Three live sentences asserted a drive-time ring over PLACE-band figures on 28 of 29 listings: the docked panel's Insights heading ("Market Overview (10 min drive)"), its footnote ("Drive-time figures are approximated from a straight-line catchment around the practice") and the Browse Market-data card's ("Figures describe the community around each practice"). Doing nothing was a decision to keep them.

**D-C38 — Option C. Each figure is served at its own honest geography and the card names it, per tile.** The three area figures (population, households, median income) and the off-card `vets` come from the catchment band with `place` as the fallback; **growth keeps its place-or-county geography and its own sub-line says so**; `econ_k` keeps county. This is D-C35 ("every layer draws at its own geography, always, and the legend names it") applied to the card instead of the map, which is the point: the card and the map read the SAME row, and two answers on two surfaces from one row is how a product starts contradicting itself.

He was shown, and chose against, the cheaper option that relabels every tile uniformly. The reason it was rejected is worth keeping: it moves the defect one tile over rather than fixing it, because the Growth number would still be the whole City of Dallas under a caption saying otherwise.

**D-C39 — the ring is described by DISTANCE, not by time, and the two existing sentences are corrected to match.** The band is an 8 km straight-line buffer (spec §8), not a routed drive time, and spec §15 still lists true isochrones as open for V1. So the card reads "within about 5 miles of the practice", and the panel's heading and footnote are corrected in the same release rather than left to contradict it. This is the largest of the three wording options he was offered and he took it knowingly: it touches approved design copy on a screen family whose pixels are gated, so it needs its own amendment entries (A27.3, A27.4) and re-bases the Browse states that render the panel. `baseline-manifest.json`'s thirteen frozen hashes still do not move — none of them is a Browse capture.

**THE HONEST MEASURE, which no summary of this work may soften.** Four of the six served figures gain neighbourhood detail and **three of the card's four tiles** do; the fourth gains an honest label and nothing more. Two figures cannot vary by band AT ALL, by construction: `materialize.py` computes `population_growth_pct` once per listing OUTSIDE the band loop and writes that one value into all three bands (D12), and always writes the county CBP row for `revenue_per_establishment`. On the Browse map, two of the three fill layers (`growth`, `econ`) still paint one flat colour per city or county. **Nobody may describe this change as "per-neighbourhood market data."**

**What this supersedes, explicitly.** `serve.py`'s all-six-null fallback rule; the contract document's row saying a null label means the place band; and `serve.py`'s "There is no geoid lookup and none is wanted" — a geoid lookup IS now wanted, for the Growth tile's place name, joining `practice_location.place_geoid` to `geo_area.name` at summary level 160 (or `county_geoid` at 050). Also `_extra_cleared`'s docstring claim that "a metric that is `suppressed` OR `approximate` is null in the card", which was already false for every approximate figure the pipeline has produced, because nothing in that module read `is_derived`.

**What survives untouched.** D-C32's labelling requirement, which now governs nearly every listing instead of one; `drive_20` is never a fallback; D-C31's rule that an absent figure is null and never zero; the design's own "Community data unavailable" card when a listing has no figures at any band — a per-figure rule makes that row rarer and must not make it unreachable; and the Orlando listing's own behaviour, which changes only in the Growth tile's new sub-line and the label's wording.

**Owed and NOT part of this.** Growth cannot become per-neighbourhood until the 2010→2020 tract crosswalk is loaded, which is the registered Phase C deferral in the table above.

**D-C39 extended — the third sentence is corrected too, as A27.5** (controller ruling, 2026-09-11, on the implementer's own report). D-C39 named two sentences. The implementer found a third — the Browse "Market data" card's "Figures describe the community around each practice…" — and correctly declined to widen its own scope. The ruling is to correct it in this release, with its own id: the figures that sentence describes are exactly the ones D-C38 moves to the catchment, so leaving it would have meant one release corrected two false sentences and made a third one false by its own act. It reads "Figures describe the **area** around each practice, not the practice itself." — "area", not a named geography, because the footnote covers every listing on the screen at once and they no longer all answer from the same band.

**D-C40 — the strip gets an approved state** (controller ruling, 2026-09-11, same round). A27.5's sentence lives inside the Browse "Market data" strip behind `md.stripOpen`, and no state in `frontend/tests/screens.ts` opened the strip: a ruled UI change with no oracle, the same gap A26's Task F2 closed for the More-filters popover. `browse-market-strip` is appended to the approved states in this release, reached by clicking the card's own "Expand all six layers" control, so the corrected sentence is photographed and a regression in it fails a gate. It is the third Browse state to re-base; no frozen hash is a Browse capture.

**D-C41 — `growth_scope` carries the name TIGER gives and nothing composed** (controller ruling, 2026-09-11, on the implementer's own flagged concern). The first implementation composed `"City of " + name` for summary level 160. `app/census/tiger.py` loads level 160 from the bare `NAME` column and level 050 from `NAMELSAD`, and level 160 covers Census designated places as well as incorporated ones — so the prefix would have rendered "City of Florin" for a CDP, on the one tile whose entire purpose is to say truthfully where its number came from. The prefix is removed at the source; `tests/census/test_serve.py::test_a_census_designated_place_is_not_called_a_city` pins it, and `docs/integrations/market-data-api.md` records that a consumer must not synthesise a descriptor either.

**D-C42 — the Insights heading keeps its name and states its geography on a sub-line** (John's ruling, 2026-09-11, on the `feat/card-geography` review; full text in `.superpowers/sdd/2026-09-05-practice-match-census-data-layer/ruling-D-C42-D-C43.md`). A21.5a let `community_label` REPLACE the docked panel's Insights heading, and D-C38 gives 28 of 29 QA listings a label — so on QA the heading read "Within about 5 miles of the practice" and the words "Market Overview" appeared nowhere, while A27.3's own change was invisible because its default was never reached. The ruling that produced Option C named exactly this as a cost of the Option B John rejected; it landed under Option C too and had never been put to him. Ruled: the heading is always "Market Overview" and the geography renders on its own sub-line directly beneath it, which is the card's own established idiom (A21.5b, A21.5c, A27.1 and A27.2 all put the geography under the value it describes). A27.6 retires A21.5a's override as a BEHAVIOUR — A21.5a's markup edit stays, because it is what made the heading data — and A27.7 adds the sub-line, composed from the panel's own `md.panel.place` declaration and gated by `sc-if` so a listing with no label has no element at all. It is not a softening of D-C38. `browse-market-panel` and `browse-panel-lightbox` were re-basing under A27.3/A27.4 already; this changes what they re-base to, and no frozen hash is a Browse capture. Its oracle is the real-browser gate in `frontend/tests/smoke.spec.ts`, not an approved state: the design's own fixtures carry no `communityLabel` and the reference has no way to be handed one without editing approved fixture data or declaring a ninth prototype prop.
...
- **Type consistency:** `Dataset` fields (A1) used by A3/A5/A6; `CensusClient.fetch_table(get, for_, expected, in_=None, extra=None)/validate_variables/request_count/concurrency` used consistently in A5/A6/A8 — `fetch_table` takes the raw parameters and builds the URL internally through the now-private `_build_url`/`_archive_key` (controller amendment A-C3b m5, A3 review round 1: nothing public or logged may carry a keyed URL), so A5+ never holds a keyed URL to pass anywhere, and `validate_variables` runs INSIDE `fetch_table` before anything is archived (A-C3b M2); `require_key`/`require_contact` raise `SystemExit(2)`, not the earlier `SystemExit(3)` (A-C4 ¶2 supersedes A-C3 ¶2); `ingest.run` yields `Run(id, rows, requests, raw_uri)` used identically in A5/A6; `ObjectStore.exists(key)`/`put(key, data, content_type)` matches the client's call (there is no `put_immutable` — controller amendment A-C3 (1)); `ObjectStore.delete(key) -> bool` reports whether a key was actually removed (A-SL1 addendum); `catchment.build(conn, listing_id, geo_vintage)` matches B4/B5 tasks; `materialize_listing(conn, redis, listing_id)` matches B5 fixtures; `gate.layer_enabled(r, conn_factory, key)` signature matches A9 and B5 (`sync_conn` is a factory); metric keys in B4 rows match B5's `METRIC_FOR` and the API contract.
