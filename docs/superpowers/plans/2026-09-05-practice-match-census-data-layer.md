...
- **Migration numbering:** Sub-project 3 Phase A uses `017`–`059`; Sub-project 2 owns `010`–`015` and the Seed Listings plan owns `016`; Sub-project 3 Phase B (listing-dependent) uses `060`+. **Amended 2026-09-09 (controller, A-C10):** the seller lifecycle plan carved `030`–`039` out of SP3-A on 2026-09-08 (A-SL5: `030_listing_owner_and_status.sql`, `031_listing_asset.sql`, `032_listing_submitted_at.sql`), so SP3-A holds `017`–`029` and `040`–`059`; `080`–`089` are the map engines'; and `090`–`099` are reserved for platform and hotfix migrations that land on `main` between releases (first: `090_listing_photo_captions.sql`, seed hotfix A-L11/A-L12).
