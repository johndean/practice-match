...
| R8 | Persona credentials in QA are a standing admin login. | Medium | `PERSONA_PASSWORD` random per environment, QA only, refused on production by `seed_persona.py`; rotate with `railway variables --set` (superseded 2026-09-08 by A-S6.2 in the account-screens plan — the value lives in the operator's Keychain, never in Railway). |
