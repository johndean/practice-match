...
- [ ] Gates: `poetry run pytest -q -W error`; `cd frontend && npx vitest run --coverage` still 100/100/100/100. Commit — `test(quality): drift tests pin the coverage and ruff contracts; policy row matches CI; three assertions made discriminating`.
- [ ] Final commit message for this round — `ci(quality): tracked tools, ruff clean at source, frontend coverage 100/100/100, SKIP_VERIFY rule` (supersedes the shorter message above).
