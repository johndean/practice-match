...
- [ ] **Step 2: survive the log-stream timeout** — `railway up --ci` returned non-zero with `reqwest error … operation timed out` *after* the upload succeeded, while the deployment carried on to SUCCESS; aborting there leaves the api deployed and the worker not. A non-zero `up` now polls `railway deployment list --service … --environment … --json` for the **newest** deployment by `createdAt` — SUCCESS continues, `FAILED`/`CRASHED` exits **67**, and so does a deployment still unsettled after the bound (`DEPLOY_POLL_INTERVAL` / `DEPLOY_POLL_TIMEOUT`, default 10 s / 900 s). Residual, recorded in `DEPLOY.md`: an `up` that fails at upload time leaves no new deployment, the poll sees the previous SUCCESS and the run continues — the verifier is then the backstop.
