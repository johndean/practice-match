...
- [ ] **Step 7: gates** — `poetry run pytest -q -W error --cov=app --cov-report=term --cov-fail-under=90` (100 % on `app/`); the same run with `--cov=app --cov=scripts --cov-branch --cov-fail-under=100` for the record; `ruff check app tests scripts`; `mypy app --strict`; `mypy scripts/migrate.py --strict`; all five `tests/scripts/*.sh`. Proof on the real image rather than by assertion: built with a `BUILD_SHA` stamp and `COMMIT_SHA=variable-only` in the environment, `/api/healthz` reported `17f40c3`; built without the stamp, it reported `variable-only`.
