...
**Rulings recorded here (controller, surfaced to John):** F2 — the review held spec §3's "first hop" to be caller-controlled and the RIGHTMOST hop was used for rounds 1–3; **superseded by fix round 4:** the live QA probe showed Railway writes the accepted client FIRST, and the rule is the first field (see FR4/FR5 below). M5 (pgcrypto) — declined: both Railway databases are pinned to PostgreSQL 16 (`gen_random_uuid()` is built in from 13), and `CREATE EXTENSION` would add a privilege dependency; a comment records the requirement. M12 (`IF NOT EXISTS`) — kept, matching `001_init.sql`. M7 — recorded: `BUDGET_MS` stays GET-only; POST budgets are their own tests beside it. `ip_day` end-to-end — recorded as a deliberate gap: the 5/min limit sits in front of it; `hit()`'s window semantics and the `LIMITS` values are tested directly. Everything else below is fixed.
...
#### Task 11c — fix round 5 (2026-09-06, from the round-4 re-review: OBS-1/2 medium, OBS-3–7 low, OBS-8/9 info; the round cap — later observations are adjudicated)

**Rulings.** OBS-1 — `client_ip` mirrors uvicorn's rule EXACTLY: the first field of the joined header; if that field is empty the peer is used (not the next field); repeated lines are joined because uvicorn joins them, and the two-line cases below pin it. OBS-3 — a port on the first hop is stripped the way uvicorn's `_parse_host_port` does (`1.2.3.4:51234` → `1.2.3.4`, `[2001:db8::1]:4711` → `2001:db8::1`, a bare IPv6 address is left alone), so an edge that ever wrote `host:port` could not give every request its own bucket. OBS-4 — docstrings say exactly this. OBS-5 — recorded in `DEPLOY.md`: the header is trusted from any peer, as uvicorn's `--forwarded-allow-ips='*'` already does; only Railway's edge and the project's private network reach the api. OBS-6 — done: 793f87e re-probed on QA, `202 ×5` then `429` in both passes. OBS-7 — the runbook states the probe's cost (rows, 6 of the operator IP's 30/day) and the stop condition becomes "anything other than `202 ×5` then `429`". OBS-2 — the recipe performs both passes. OBS-8 — the controller rewrites the stale "rightmost" wording in the FR1 rulings paragraph and Step 4b (done in this commit). OBS-9 — the drift test pins the stop-condition sentence.

**Files:** Modify `app/api/interest.py`, `tests/api/test_interest.py`, `DEPLOY.md`, `tests/test_docs.py`.

- [ ] **FR5 Step 1: failing tests.** Replace `test_client_ip_uses_the_first_hop_or_falls_back_to_the_peer` and `test_client_ip_joins_repeated_forwarded_header_lines` with:
```python
...
def test_client_ip_is_the_first_field_of_the_joined_header_like_uvicorn(lines, expected):
    ...
    assert client_ip(Request(scope)) == expected
```
`tests/test_docs.py` — the forwarded-for drift test additionally asserts `"anything other than" in text` and `"198.51.100" in text` (the second pass) and `"30/day" in text` (OBS-7).

RED: the port cases return the port-suffixed string; `", 5.6.7.8"` returns `"5.6.7.8"`; the drift test fails on the missing sentences.

- [ ] **FR5 Step 2: implement.** `app/api/interest.py`:
```python
...
def client_ip(request: Request) -> str:
    ...
    return request.client.host if request.client else "unknown"
```
`DEPLOY.md` — the "Client address for the sign-up rate limits" paragraph becomes:
...
- [ ] **FR5 Step 3: GREEN** — every node id; `poetry run pytest -q -W error` twice; mypy; ruff.
- [ ] **FR5 Step 4: Commit** — `git add app/api/interest.py tests/api/test_interest.py DEPLOY.md tests/test_docs.py` · `fix(api): client_ip mirrors uvicorn exactly (first field, port stripped, empty field → peer); runbook probe performs both passes and states its cost and stop condition` with the trailer.
...
### Task 14: Deploy what is committed, verify what is deployed (hotfix, 2026-09-07)

**Why (root cause, measured):** `scripts/deploy.sh QA` run from the linked worktree `.worktrees/feat-browse-v3` (HEAD 17f40c3) uploaded the MAIN checkout's tree. QA then served `pyproject` version 0.1.0 and a bundle containing `Layered Market Data` and `View full market report` — strings present only in main's `frontend/src/App.vue` and absent from the branch. Railway CLI 5.26.0 resolves the upload root through a worktree's `.git` **pointer file** back to the main repository directory, and `railway up [PATH]` only treats the path as the archive root when `--path-as-root` is passed ("Use the path argument as the prefix for the archive instead of the project directory", `railway up --help`). `scripts/verify-deploy.sh` still passed, because healthz's `commit_sha` was the `COMMIT_SHA` service variable the script sets immediately before each upload — a property of the deploy command, not of the image. Two defects: the deploy path could ship a different tree than the one it claimed, and the verifier could not tell.

**Files:**
- Modify: `scripts/deploy.sh`, `scripts/verify-deploy.sh`, `app/api/health.py`, `Dockerfile`, `tests/scripts/test_deploy_guard.sh`, `tests/scripts/test_verify_deploy.sh`, `tests/test_health.py`, `tests/test_docs.py`, `.github/workflows/quality.yml`, `DEPLOY.md`, `CLAUDE.md`
- Create: `tests/scripts/test_deploy_archive.sh` (the fifth shell suite; added to the CI backend job)

- [ ] **Step 1: `deploy.sh QA|production [SOURCE_DIR]` uploads the committed tree** — SOURCE_DIR defaults to the script's own repo root and is resolved to an absolute path *before* the `cd`, so a relative argument means what the caller typed. The script extracts `git -C "$SOURCE_DIR" archive --format=tar HEAD | tar -x -C "$TMP"` (a `mktemp -d` under `${TMPDIR:-/tmp}`, removed by an EXIT trap) and runs `railway up "$TMP" --path-as-root --environment "$ENV" --service "$svc" --ci` — so the upload carries no `.git` to mis-resolve, no untracked files and no uncommitted edits, and `git archive` run with `-C` inside a linked worktree is worktree-correct. Guards, both before the CLI is touched: a SOURCE_DIR that is not a git working tree is a usage error (**64**); `git status --porcelain --untracked-files=no` non-empty is refused with the offending paths (**66**), because "deploy what is committed" would otherwise drop those edits in silence. The 🚦 check is unchanged and still reads `railway status --json` back from the script's own (linked) directory. `$TMP/BUILD_SHA` carries the source's short sha; `COMMIT_SHA` is still set per service; `EXPECT_SHA` and `EXPECT_VERSION` (read from the archive's own `pyproject.toml`) are passed to `verify-deploy.sh`.

- [ ] **Step 2: survive the log-stream timeout, fail closed** — `railway up --ci` returned non-zero with `reqwest error … operation timed out` *after* the upload succeeded, while the deployment carried on to SUCCESS; aborting there leaves the api deployed and the worker not. A non-zero `up` now asks Railway what happened via `railway deployment list --service … --environment … --json`. It must not be fooled by the previous deploy, though: `up` also exits non-zero when the UPLOAD failed, and then no new deployment exists and Railway's newest is the previous one — very possibly a SUCCESS a status-only poll would misread as this deploy's (C2, ruled FIX pre-review 2026-09-07). So the newest deployment's `createdAt` is recorded BEFORE the upload; a deployment with a **strictly newer** `createdAt` must appear within `DEPLOY_APPEAR_TIMEOUT` (120 s) or the script exits **67** with `upload did not create a deployment for <service>`; rows without a `createdAt` are dropped, so a JSON shape lacking the field reads as no new deployment (67) rather than hanging. That deployment and only that one (by `id` when the CLI supplies one) is then polled to SUCCESS — continue — or `FAILED`/`CRASHED`, or still unsettled after `DEPLOY_POLL_TIMEOUT` (900 s), each **67**. `DEPLOY_POLL_INTERVAL` defaults to 10 s and is floored at 1 s so the loop can never spin.

- [ ] **Step 3: `verify-deploy.sh` gains `EXPECT_VERSION`** — the live `version` must equal it or the script fails with one `FAIL:` line naming both. Default: the version in the `pyproject.toml` beside the script, empty (and so skipped, exactly as `EXPECT_SHA` is outside a git checkout) when that file is unreadable or carries no version.

- [ ] **Step 4: healthz reports the artefact** — `app/api/health.py` gains `BUILD_SHA_FILE` (module level, so tests can move it) resolving to the app root's `BUILD_SHA`, i.e. `/app/BUILD_SHA` in the image, and `build_sha()` returns its stripped contents, falling back to the `COMMIT_SHA` setting when the file is absent or blank. `Dockerfile` copies it as the last layer: `COPY pyproject.toml BUILD_SH[A] ./`. The glob makes the stamp optional; the pairing with an always-present source is required, because a `COPY` whose only source matches nothing fails outright with `COPY failed: no source files were specified` (measured against the local daemon). `pyproject.toml` is the pairing — already in the image with the same content, and last means poetry install is never invalidated.

- [ ] **Step 5: tests (RED watched first)** — `tests/scripts/test_deploy_archive.sh`, on the existing fake-`railway` harness, with scratch git repositories and a real `git worktree add`: the archive carries SOURCE_DIR's committed `pyproject.toml` and neither `.git` nor untracked files; `--path-as-root` is passed and the temp directory is gone afterwards; `BUILD_SHA` is the source's short sha; an uncommitted tracked edit exits 66 with the CLI untouched; a linked worktree as SOURCE_DIR archives the WORKTREE's HEAD and the main checkout's tree appears nowhere; the log-stream-timeout fallback polls BUILDING → SUCCESS and still uploads the worker; FAILED exits 67 before the worker upload; the wait is bounded; the default SOURCE_DIR is the script's own repo root (proved with a scratch checkout carrying its own copy of `deploy.sh`); a SOURCE_DIR that is not a git working tree exits 64. The fake's `deployment list` puts an already-SUCCESS row FIRST and the popped status second, so a parser reading element 0 rather than the newest by `createdAt` fails the FAILED case. `test_deploy_guard.sh`'s happy path now passes a clean scratch SOURCE_DIR, so it tests the 🚦 guard and not this repository's working state. `test_verify_deploy.sh` gains a `wrong_version` mode: an explicit mismatch fails naming both versions, and an unset `EXPECT_VERSION` still fails against the source's version. `tests/test_health.py` covers the stamp present, absent and blank, and pins the stamp to the app root beside `pyproject.toml`. `tests/test_docs.py` pins the `BUILD_SH[A]` pairing, the new CI step, and the `DEPLOY.md` / `CLAUDE.md` sentences.

- [ ] **Step 6: docs** — `DEPLOY.md`: what is uploaded, the SOURCE_DIR usage line, the worktree pointer-file hazard, exit 66/67 with the polling fallback, `EXPECT_VERSION`, and the `COMMIT_SHA` row demoted to a fallback behind `/app/BUILD_SHA`. `CLAUDE.md`: one sentence in the 🚦 block.

- [ ] **Step 7: gates** — `poetry run pytest -q -W error --cov=app --cov=scripts --cov-branch --cov-report=xml --cov-fail-under=100`; `ruff check app tests scripts`; `mypy app --strict`; `mypy scripts/migrate.py --strict`; all five `tests/scripts/*.sh`. Proof on the real image rather than by assertion: built with a `BUILD_SHA` stamp and `COMMIT_SHA=variable-only` in the environment, `/api/healthz` reported `17f40c3`; built without the stamp, it reported `variable-only`.

- [ ] **Step 8: the two standing coverage gaps close and the gate rises (C4, ruled FIX pre-review)** — the stricter gate had been failing at 99 % on gaps that predate this task: `app/db.py`'s `if loop is current:` false arm (branches 77->75 and 82->80 — every other test disposes the loop it runs on) and `scripts/migrate.py`'s `if __name__ == "__main__": sys.exit(main())`, the line `railway.json`'s pre-deploy hook and `scripts/start.sh`'s migrate role actually run. `tests/test_db.py` seeds `_engines`/`_redis_clients` under a different, already-closed loop with a sentinel whose `dispose()`/`aclose()` would record being awaited, and asserts the entries are dropped untouched; `tests/test_migrate.py` re-executes the module with `runpy.run_path(migrate.__file__, run_name="__main__")` (`run_path`, not `run_module`, whose RuntimeWarning `-W error` would fail) and asserts `SystemExit(0)`. With both closed, main's backend gate becomes the one Wave 2a already runs — `--cov=app --cov=scripts --cov-branch --cov-report=xml --cov-fail-under=100` — in `.github/workflows/quality.yml`, in `CLAUDE.md`'s Common operations, and in the quality policy's gate table, all three pinned by `tests/test_docs.py`.
