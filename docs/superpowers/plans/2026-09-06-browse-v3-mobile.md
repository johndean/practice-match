...
**(f) The thirteen non-Browse screens — how zero regression is proved (re-amended 2026-09-07, option B — V13).** The screens the bundle names as untouched are:
...
> **V9 ran under option A; Task V13 withdrew it (recorded 2026-09-07 — the steps below are history and are exactly what was executed).** Every citation of "option A" in this task is the premise V9 executed on: V3's display typography accepted, the manifest re-based onto the V3 baselines. John's ruling ("keep the V2 header and do not restyle header or fonts") reversed the premise, not the steps: V13's amendment A1 restored V2's typography in the design reference, every oracle was regenerated from it, and the thirteen non-Browse screens are byte-identical to V2 again (spec D6/D16; Global Constraint (f)).
...
reserved for the Phase C tract tiles, migrations start at 017, and the
...
a member's own toggle. Both plans also record V13's correction: amendment A1
restored V2's display typography, so the thirteen non-Browse screens are
byte-identical to V2 again and a pre-V3 pixel oracle exists for them
(Browse-only elements keep V3's type). A drift test keeps all of it true.
CLAUDE.md names the V2 folder as the pre-V3 oracle and records the typography
ruling (option B) and that C14's byte-identical rule holds as written.
...
>
> **Headings keep the V2 typography you approved.** The V3 design had dropped the all-capital display headings; on your ruling ("keep the V2 header and do not restyle header or fonts") they are restored in the design reference itself, so the screens outside Browse look exactly as they did before. The small all-capital labels (section eyebrows, tags) are unchanged. *(Rewritten 2026-09-07 after Task V13 — superseding the paragraph that said headings now read in sentence case: that was option A, which you withdrew, and it must not be forwarded.)*
>
...
- [ ] **Step 2: Run — RED** — `cd frontend && npx vitest run tests/design-amendments.test.ts`. Expected (cases named, not numbered — fix round 1 added the pristine-hash case at the top of the file): `the amended reference is the pristine Rev 2 file plus exactly the ruled edits` fails (`AMENDED` still equals the pristine file), `after A1 every display-size heading…` fails (no uppercase on display headings), and the `LOCAL_AMENDMENTS.md` case fails (the file is missing). `the pristine Rev 2 copy is the bundle's file, untouched` and `A1 derives exactly the 24 V2-typography edits` must PASS as written (it measures the derivation, not the tree; corrected 2026-09-07 after the first V13 attempt's STOP: the earlier code strip-and-appended declarations, which produced 35 reorder-only no-ops, and the spec wrongly excluded `{{ m.v }}`, which V2 does set uppercase) — if it fails, stop and report the actual count and the unexpected elements, do not tune the number.
...
**Files:**
- Modify: whatever the systematic debugging identifies — never a widened tolerance (Global Constraint (e)) and never a `waitForTimeout` alone. *Outcome (2026-09-07): nothing in the page moves — 24/24 identical geometry and identical viewport captures; the nondeterminism is Chromium's **partial raster** resolving the results rail's fractional-offset rounded-corner antialiasing differently per `fullPage` pass. Three condition-based waits were tried and refuted with counts; the fix is one line in `frontend/tests/playwright.config.ts`: `launchOptions: { args: ['--disable-partial-raster'] }` (12/12 deterministic; both Playwright projects; tolerance untouched). Nine git-ignored Browse/header/mobile-sheet PNGs re-baselined by ±1 grey; the thirteen manifest screens byte-identical; DOM snapshots unmoved. Controller ruling: accepted on the evidence; surfaced to John as a default applied. Residual risks recorded (V17 review M5/M6): a third capture variant seen only when captures share a renderer process was never pixel-diffed, so the pre-fix flake's upper bound is unproven — the flag removes the source; `--disable-partial-raster` is a Chromium switch outside Playwright's default set and Chromium ignores unknown switches silently, so a future Chromium that drops it would reinstate the flake without error — the ten-capture guard and a deterministic presence assertion on the config are the detectors; if the Linux gate ever surfaces a mismatch the task reopens.*
- Test: `frontend/tests/capture-determinism.spec.ts` (reference project): ten page loads through the state's own `screens.ts` steps, ten `fullPage` captures, one image expected — for `header-1000` and `header-1100` (the two states with the identified 12-pixel signature; `browse` flipped too and is covered by the same flag); the docstring names the signature and the states known to have flipped
...
| §4 D6 re-amended, option B: V13 restored V2's typography via A1; the thirteen are byte-identical to V2 again and the manifest carries the V1-era hashes; DOM parity + the pixel gate stay the secondary proof | V1, V9 Steps 3/5/8, V10 Step 4, V11, V13 |
...
| C14 What did not change — "their baselines must be byte-identical" | V1 (through V7), V9, V10, V11, V13 | **Withdrawn 2026-09-07 (option B, V13): A1 restored V2's typography, all thirteen byte-identical again, so C14 is correct as written.** V7's finding — V3 restyled all 26 display-size headings, so twelve of the thirteen moved on typography alone — held only under option A, which John withdrew (spec D6/D16). Proved by `baseline-manifest.test.ts` against the V1-era V2 hashes, with V9's regenerated oracles as the secondary proof: the 28-state DOM oracle (node-for-node identical to the V3 reference, 13 of 13) and the zero-tolerance pixel gate, which carried V10 and every V11 deletion. The reversal is recorded beside the sixth cross-plan delta in V12 |
...
| Zero-risk: "the thirteen … baselines are **byte-identical** before and after" (`DEAD_CODE_CHECKLIST:60-62`) | V9, V13 | **Withdrawn 2026-09-07 (option B, V13): A1 restored V2's typography, all thirteen byte-identical again, so the checklist is correct as written.** The V7 finding that superseded it — spec D6 as it then stood, under option A — fell with option A. Proved by the V13 manifest against the V1-era V2 hashes, with DOM parity against the V3 reference (13 of 13, V9 Step 5) + the zero-tolerance pixel gate as the secondary proof through V10/V11 |
