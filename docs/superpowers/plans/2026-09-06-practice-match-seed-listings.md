...
**Controller pre-flight amendment A-L0 (2026-09-08; John: "launch SDD + TDD to Seed listings"; the plan is dated 2026-09-06 and main has moved).** (1) **Counts.** `frontend/tests/screens.ts` holds 28 approved states on main today (the plan says 27); after `feat/identity` merges it holds 43. Wherever a task says "27" read "the current list" and pin the actual count. (2) **L6's STOP is resolved by mechanism, not yet by ruling.** The plan's option (iii) — change the design file so the script reads `p.name`/`p.photos` — is now done through the D15 ruled-amendment engine (`frontend/tests/design-amendments.ts` + `LOCAL_AMENDMENTS.md`; pristine `.rev2` never edited; families A1–A7, A10, A11 on main; A8/A9 reserved by `feat/identity`), as family **A12** with the plan's minimum edit set at the five sites (`practiceName`, both `photoSet` branches, `heroSrc`, `thumbSrc`). It is a design change and needs John's explicit ruling; queued on the artifact with the default "approve". **L6 does not start until he rules.** (3) **Sequencing.** L6 modifies `frontend/tests/harness.ts` and the design script, both rewritten by `feat/identity` (S5–S7). L1–L5 proceed now on this branch; L6–L7 start only after `feat/identity` has merged to `main` and this branch has merged `main`. (4) **Already true on main:** `.github/workflows/quality.yml` runs `--cov=scripts` (Wave 2a I9a) — L5's CI step verifies, it does not change; `tests/perf/gate.py::gate_p95` (Platform Task 15) is how latency budgets are asserted — L5 follows the existing perf tests; migration `016` is free (`010`–`015` applied; the I5d plan takes `003`). (5) **Gates every commit**, exactly as CI: `poetry run pytest -q -W error --cov=app --cov=scripts --cov-branch --cov-fail-under=100`; `poetry run ruff check .`; `poetry run mypy app --strict` and `poetry run mypy scripts/migrate.py scripts/bootstrap_admin.py scripts/seed_persona.py --strict` (add any new script to the second command in the same commit that adds it); `for t in tests/scripts/test_*.sh; do bash "$t" || exit 1; done`; frontend gates when `frontend/` changes. Playwright (L6 only) runs with `DATABASE_URL=postgresql://pm:pm_dev_pw@localhost:5433/practice_match_seed REDIS_URL=redis://localhost:6380/3 PW_APP_PORT=5473 PW_REF_PORT=5474 PW_CS_PORT=5475 PW_API_PORT=8317` — other suites share the machine. (6) **Photos.** The curated source folders exist at the path L3 names (eighteen `<slug>_individual_images` folders, read-only, outside the repository). (7) **Version.** L7's lockstep bump goes from whatever `main` carries when L7 runs (0.1.1 today); if `feat/identity` bumps first, L7 bumps once more.

---

**Controller amendment A-L13 (2026-09-10 ~06:40 WITA; John: "why is the old seed data now visible on qa.foundation.vin/browse … delete the old data so this revert stops happening and undermining confidence in the site" — the premise corrected, and the real defect).** **There is no old data to delete.** The QA database holds exactly eighteen rows, every one `status='published'`, `source='seed'`, with ten to twelve photographs — read from inside the api container. The practices on screen are the DESIGN'S OWN prototype fixtures, the twenty-one entries of `P` in `frontend/src/logic.js`, which the launch-removal list deliberately keeps (`CLAUDE.md`: "Fixture data in `logic.js` … also stays — keep field names; the UI reads them"). They cannot be deleted: `app-generated.test.ts` proves the ported script byte for byte, the reference oracle renders from them, all 100 baselines derive from them, and the design reads `P[0]` and `MARKETS[s.market || "Austin, TX"].center` on every render, so an empty catalogue is a blank app rather than an empty Browse (already recorded in `load.ts`'s docstring as A-L6.2 (1)). Deleting them would break every visual gate and would not have fixed what John saw.

**The real defect — and it is exactly the "revert" he describes.** `loadListings` is called ONCE, at `frontend/src/main.ts:41`, before the app mounts. A visitor who is not yet signed in gets `401` from `GET /api/listings?limit=200` — correct, `listing.read` is a member permission — so the loader returns false and the fixtures stay in `P` by design. The visitor then signs in at the gate; `adapter.signIn` writes the principal into the store and the app moves to Browse — **and nothing re-reads the catalogue.** `logic.js:1131`'s `reloadListings()` is the SELLER dashboard's loader (A16.9), not the buyer's. So every interactive sign-in lands on Browse showing twenty-one invented practices with invented asking prices while eighteen real hospitals sit unread. Only a visitor arriving with a valid session cookie already set sees the real ones. Verified live: an anonymous visit to `/browse` renders the sign-in gate correctly (the gate is not the problem), and the api log shows one session's `GET /api/listings?limit=200` answering `200` and a later one `401`.

**Task L8 (this plan's next task), in `.worktrees/feat-browse-reload` (database `practice_match_browse`, Redis `/15`, Playwright 5973–5975/8547):** the catalogue is re-read the moment a member becomes authenticated, and a signed-in member is never shown invented practices. (1) `loadListings` runs again after a successful interactive sign-in, through the adapter seam `signIn` already owns — the store write stays FIRST, because `logic.js`'s `.then` and the next `guard()` depend on that ordering (A5.2), and the re-read follows it. (2) A member whose re-read succeeds sees only real listings. (3) A member whose re-read FAILS must not be shown invented ones — if clearing the fixtures is not reachable without the blank-app crash A-L6.2 (1) describes, the implementer STOPS with `NEEDS_CONTEXT` and quotes the lines rather than inventing a screen. (4) The anonymous boot path is unchanged: the gate as today. (5) Every approved state keeps its pixels — the reference passes no adapter and no API, so its fixtures are untouched and `baseline-manifest.json` must not move.

**Controller amendment A-L14 (2026-09-10 ~06:55 WITA; ruling on Task L8's NEEDS_CONTEXT — what a member sees when the post-sign-in re-read fails).** The implementer stopped correctly and quoted the lines: `logic.js`'s `detail()` does `P.filter(...)[0] || P[0]` and then reads `p.id`/`p.bldg` unconditionally, so an empty catalogue is a crash, not an empty Browse. It therefore cannot clear the fixtures, and it must not keep them. **The answer is the design's own sign-in error path, which already exists and needs no new screen and no design amendment.** The card renders `form.error`/`form.errorText` from `formError`, and `logic.js`'s existing rejection handler sets `formError: (e && e.message) || "Sign-in failed."` while keeping `screen: "gate"`. So: **the adapter's `signIn` reads the catalogue after the store write; if that read fails it retries ONCE; if the retry also fails it clears the store and rejects with an Error whose message is shown on the card.** The member stays on the gate with an honest message instead of being walked into a marketplace of invented practices, and pressing Sign in again re-runs the whole path against the session cookie that is already set, which is the ordinary recovery. The store is cleared on that path deliberately: a populated store behind a signed-out screen is the stale principal the `signOut` docstring warns `guard()` would trust — and A5.2's ordering regression is about the RESOLVE path, which is unchanged. **Copy (default applied, queued for John as D-L14):** "Signed in, but the listings could not be loaded. Please try again." It is thrown at runtime, so it is not in the byte-locked design file and moves no pixel; if John prefers other words, only that string changes. **Tests this adds:** the re-read happens after the store write and after `api.signIn` resolves (the happy path, already required); one failure then success proves the single retry; two failures prove the store is cleared, the promise rejects with that message, and the practices array was never replaced by fixtures the member could see; and the anonymous boot path still does no re-read and clears nothing.
...
**Controller amendment A-L16 (2026-09-10 ~08:30 WITA; a latent crash found by building a demonstration, not by a test).** While seeding three listings into a local database to show the stakeholder what market figures look like on screen, Browse rendered a BLANK PAGE. The console says why: `TypeError: Cannot read properties of undefined (reading 'center')` at `logic.js:1882`, which is `mapCenter: MARKETS[s.market || "Austin, TX"].center` (and `:1883` for the zoom). **The design hard-codes "Austin, TX" as the fallback market.** `applyListings` replaces `MARKETS` with exactly the markets the API returned, so the moment the published inventory contains no Austin listing, that lookup is `undefined` and every member's Browse throws on render. The seeded three were Orlando, Sacramento and Round Rock; there was no Austin key; the page died. **This is not live today** — two of the eighteen QA hospitals are in Austin, so the key exists and QA renders — which is precisely why no test and no gate has ever caught it: the fixtures and the seeds both happen to contain Austin. It is one withdrawn listing away from being live, and it would be a blank marketplace for everyone, at launch, with no error visible to the user. **Task L9 fixes it:** the fallback market is the first market actually present in `MARKETS` (a deterministic choice — the design's own Austin centre when Austin IS present, so no approved screen moves), never a hard-coded city; `s.place`'s "Austin, TX metro" default at `:1884` follows the same rule; and the test seeds an inventory with NO Austin listing and asserts Browse renders with a sensible centre rather than throwing. The design edit is a ruled amendment in family A23. If removing the literal moves a frozen screen, the implementer STOPS — the reference's own fixtures contain Austin, so it should not.
...
> **Thirteen, not fifteen and not fourteen (pre-flight I4).** Browse V3 freezes exactly thirteen baseline hashes in `frontend/tests/baseline-manifest.json`: `mobile-list`, `mobile-detail`, `detail`, `requests`, `seller-dash`, the four `wizard-*` and the four `admin-*`. `header-1100` and `header-1000` are deliberately **excluded** — they are Browse screenshots and are expected to move. An earlier draft of this plan said fifteen and the spec said fourteen; both were corrected on 2026-09-06 — the spec now reads thirteen and names the manifest as the authority. Nothing is outstanding here.
...
**Controller amendment A-L6.1 (2026-09-08; L6's NEEDS_CONTEXT ruled).** The design's `detail()` dereferences two of the four community figures (`p.growth.replace(…)`, `p.hh.replace(…)`), and D4 leaves all four `null` until the Census plan — measured against the real eighteen, every screen threw. Under the same A12 ruling, A12.6 and A12.7 make the two `.replace` sites null-safe (`(p.growth || "").replace(…)`, `(p.hh || "").replace(…)`), pixel-safe because the fixtures carry strings; A12.5 is adjusted so the thumbnail takes the SECOND photograph where one exists (the design's thumb is a second view — the parking photograph), the first otherwise. Counts: twelve families, 78 entries, 55 rows. Recorded as latent for Wave 2b, not fixed here: `MARKETS[s.market || "Austin, TX"].center` assumes an Austin listing exists (true for the eighteen); `p.sqft.toLocaleString()` assumes `sqft` is set (the schema requires it). The D6 stub sends `name: null` deliberately so the design's `NAMES` titles keep the fixtures' pixels; the real API sends the stored name or the anonymised label.

**Controller amendment A-L6.2 (2026-09-08; rulings on the L6 review — 1 Critical, 5 Important, 7 Minor, all closed at source).** (1) **C1/I2 — an empty or malformed 200 never installs.** `loadListings` returns `false` and the design's fixtures stay whenever `items` is not a non-empty array: the design cannot render an empty catalogue (`detail()` reads `P[0]`, `renderVals` reads `MARKETS[…].center`), so the brief's "a 200 always wins, empty list included" is withdrawn; `main.ts`'s listings arm catches like the account arm, so no read can blank the page. On QA the fixtures therefore show between deploy and seed, and in any environment that has not been seeded — the pre-L6 behaviour. (2) **I5 — the remote parity run.** `visual.spec.ts` and `dom.spec.ts` skip when `PW_APP_URL` is set, with the reason in the skip ("the oracles compare against the design's fixtures; a seeded target differs by construction — A-L6.2"); `docs/RUNBOOK-identity.md` §12 states that the remote run is smoke, sign-in and account flows from here; L7's hand-back says the same. (3) **I1, I3, I4** are fixed exactly as the review specifies (real assertions through the reset registry; the dead `DESIGN_MARKETS` export deleted; the stub's arming and matching extracted and pinned in `harness.test.ts`). (4) **Minors, fixed at source:** M1 (the comment drops the count), M2 (`next_cursor` is followed until null, tested), M3 (`centroid` copies `US_CENTER`), M4 (the push/pop moves to `beforeEach`/`afterEach`), M5 (the brief's `(p.market || ', TX')` fallback restored). **M6 — no change, recorded:** the A12 ruling cell is John's launch message verbatim (he pasted the plan's goal line as the launch instruction). **M7 — accepted, recorded:** the design renders `P` at first paint, the wait is bounded at 5 s, an anonymous visitor's read is an immediate 401, so the gate is unaffected. Every number written into a doc or a pin is counted from the tree.

**Controller amendment A-L8 (2026-09-08; rulings on the final whole-branch review — 1 Critical, 2 Important, 9 Minor; one fix dispatch, all closed at source unless stated).** (1) **C1 — the detail names the listing's own state.** A12.8/A12.9 replace the design's two `", TX"` literals in `detail()` (the subtitle and the "General location" row) with the design's own `stateOf(market)` helper, called exactly as the design calls it elsewhere; pixel-safe because every approved detail state captures an Austin fixture. The seller wizard's own `", TX"` (fixture-driven) is out of scope. (2) **I1 — Community Context reaches the design's empty state.** A12.10/A12.11: `hasDemo` is `p.id !== "p8" && p.pop != null`, `noDemo` is `p.id === "p8" || p.pop == null`; the `logic.test.ts` assertion that called the blank grid "the design's own empty state" is corrected and asserts `noDemo`/`hasDemo`; the comment in `app/api/listings.py` becomes true. Counts: twelve families / 82 entries / 59 rows (derived by `tests/test_docs.py`; CLAUDE.md prose and the pinned id list edited by hand). (3) **I2** — both `smoke.spec.ts` URL assertions accept either id shape (`/\/practices\/[A-Za-z0-9-]+$/`), and `playwright-config.test.ts` pins that no `p\d`-shaped listing-id assertion returns. (4) **Minors:** M1 `name_disclosed` joins `ApiListing` and `toApiShape`; M2 `prepare_photos.py` refuses a slug containing `/` or starting with `.`, tested; M3 DEPLOY.md's seeding section leads with the route the operator can take (locally against the QA PostGIS service's public URL, values in a subprocess environment and never printed) and demotes `railway ssh` to "when a key is on file"; M4 a seed test pins that at least one hospital is in `Austin, TX`; M7 `toPractice` uses `!= null` for `name`; M8 the size is written "MiB" in the Dockerfile comment and the test; M9 one `sync_redis()` binding per request. **M5 — no code change:** Browse is market-scoped by approval; the hand-back and the demo run sheet say that Browse opens on Austin (two hospitals) and the metro selector shows the rest; a denser default is a data decision queued for John. **M6 — no code change now, recorded as a Wave 2b hard requirement beside A-L5.2 (2):** photographs become disclosure-gated and their cache lifetime follows the flags.

**Controller amendment A-L7 (2026-09-08; the hand-back after A12, and who runs what).** (1) **Division of labour.** The implementer runs Steps 1, 2 and 9 in the worktree, on its isolated database and ports, plus the docs sweep in (6); Steps 3–8 and 10 are the controller's, from the main checkout after the branch merges — the implementer never runs `railway`, `scripts/deploy.sh` or `git push`. (2) **Version.** main is 0.1.2 (the identity release of 2026-09-08) and the branch already contains it; the merge of main's hotfix precedes the bump; the bump is to **0.1.3** in lockstep. (3) **QA seeding** runs locally through the operator's rollout script against the QA PostGIS service's public URL, with the `api` service's `ENVIRONMENT`, `API_SECRET_KEY` and `REDIS_URL` in a subprocess environment and never printed — `railway ssh` needs an SSH key this machine does not hold. Same script, same idempotency, same output lines; a plain run suffices for the first import (every import removes stale seed rows, A-L4), `--reset` is not needed. (4) **Step 7** items 8 and 9 are no longer conditional: A12 ruled — the title slot shows the seeded name where `name_disclosed` is true (all eighteen), the gallery shows the seeded photographs, the thumbnail the second one. Item 7 reads: Community Context shows its empty state AND the detail renders (A12.6/A12.7). Item 1 is unchanged (all twenty-one fixtures gone). New item 12: a listing's photo URL answers `200 image/webp` signed in and `401` anonymous. (5) **Step 9's hand-back** is rewritten for what shipped and written to `.superpowers/sdd/2026-09-06-practice-match-seed-listings/task-L7-handback.md` (never into this plan): the "Outstanding decision — hospital names and photographs" paragraph becomes a short "Decided" note (John's launch statement; A12, five edits plus the two null guards); the photo-captions decision stays open for Rev 3; the address/phone note stays; two demo caveats are added — the community statistics panel is empty until the Census plan lands, and the Admin tabs Listings, Requests and Data Sources still show the design's fixtures until their endpoints land (only Users is real, and John has asked for all four to be fact-based). (6) **Docs sweep** (docs-vs-code, RED first through `tests/test_docs.py` where a pin exists or is added): CLAUDE.md §Launch-removal list's sentence "Fixture data in `logic.js` … also stays … until the listings API replaces it (Seed Listings plan, D6)" → the API replaces `P` and `MARKETS` at boot (`frontend/src/main.ts` → `frontend/src/listings/load.ts`), and the fixture arrays stay in the script as the D6 stub's source for the gates; CLAUDE.md §Layout gains `seeds/` and `scripts/` `seed_listings.py`, `prepare_photos.py`, and `start.sh` reads `roles api|worker|migrate|seed`; §Common operations gains the one seed command line DEPLOY.md documents (L4 review item 11); DEPLOY.md's exit-code list is checked against `scripts/seed_listings.py` (0/2/3/4/5). Nothing else in the docs changes.
...
|---|---|---|
| §1 eighteen hospitals replace the design's fixtures (**twenty-one**, per the corrected §1), QA only, production unaffected | L4, L7 | `test_seed_writes_eighteen_published_seed_rows`; L7 Step 7 item 1 (one fixture checked per design market) and Step 10 |
...
| D9 zero regression on the unchanged screens (**thirteen** frozen hashes, per D9 and the Browse V3 manifest) | L6 | `tests/baseline-manifest.test.ts` + `npm run test:visual` at `maxDiffPixels: 0` |
...
**Controller amendment A-L9 (2026-09-09; John's ruling verbatim: "the seed phase failed to upload ALL the images - only exterior where uploaded and none of the interior images are being rendered, identify the failure and resolve quickly").** The failure was Task L3's cap, not the upload: `scripts/prepare_photos.py` kept `sorted(folder)[:MAX_PHOTOS]` with `MAX_PHOTOS = 4`, and John's source folders number the exteriors first (`01_exterior_…`–`05_exterior_…`) and the interiors after them, so four exteriors were encoded and every interior was dropped — 70 exteriors and 2 interiors across the eighteen hospitals. Upload (`scripts/seed_listings.py`), serving (`/api/listings/{id}/photos/{n}`) and rendering (amendments A12.2–A12.5, where `p.photos[i]` fills the design's photo slot `i`) were all working as built. **The resolution:** the design's detail page renders exactly SIX captioned photo slots per practice, chosen by practice type (`photoSet(p)` in `Practice Match V3.dc.html`: `exterior, barn, office, truck, lab, grounds` for Large animal · `exterior, triage, treatment, icu, surgery, imaging` for Emergency · `exterior, lobby, consult, surgery, imaging, recovery` for Specialty · `exterior, lobby, exam, treatment, surgery, kennel` otherwise), and nothing beyond six can ever render — so the seed now carries six photographs per hospital and each slot receives the source image whose curated filename names its subject, which is what makes the design's own fixed captions TRUE of the photographs shown. `MAX_PHOTOS = 6` is now the length of every slot list; `SLOT_KEYWORDS` maps each slot to the words John's filenames use for it (most specific first, so `lobby` prefers a reception photograph to a waiting-area one); `SLOTS_BY_TYPE`/`DEFAULT_SLOTS` carry the four design lists and `prepare(source_root, out_root, slugs, types)` takes the slug → type map `main()` reads from `seeds/hospitals.json`. Keyword matches are assigned for every slot first and the fallbacks only afterwards — an unmatched slot takes the first unused interior, then any unused file, never a duplicate — so a fallback cannot swallow the interior a later slot's keyword names; each `index.json` entry records the `slot` it fills, because a folder thinner than six slots can leave a middle slot empty and the slot is then not recoverable by counting. Regenerating gave **108 files (18 × 6), 4.0 MB**, of which **84 slots matched by keyword and 24 fell back** (the folders hold no surgery photograph for eight small-animal hospitals, no consult room for four specialty hospitals, and so on). **Still John's:** the seller wizard's four-photo cap (`docs/superpowers/specs/2026-09-08-seller-listing-lifecycle-design.md` D18, "Keep the existing 4-photo seller-upload cap") now differs from the seed's six and is queued for him as an open question — this amendment touches only the seed pipeline.

---

**Controller amendment A-L10 (2026-09-09; John's rulings verbatim: "explain where the image description is coming from because they don't mirror the file name of the image and the images don" and "match the description").**

**Where the description comes from.** Nowhere in the data: it is the DESIGN's own. `photoSet(p)` in `Practice Match V3.dc.html` renders exactly six captioned slots per practice and the captions are fixed per practice type — Small animal: *Exterior — street view · Reception and waiting · Exam room · Treatment area · Surgery suite · Boarding and runs*; Emergency: *Exterior — entrance · Triage and intake · Treatment floor · ICU and hospitalization · Surgery suite · Imaging room*; Specialty: *Exterior — building · Reception and waiting · Consult room · Surgery suite · CT and imaging · Recovery ward*; Large animal: *Exterior — street view · Barn and stocks · Office and pharmacy · Ambulatory vehicle · Lab and storage · Grounds and turnout*. The API sends photo URLs only (`/api/listings/{id}/photos/{n}`), never a caption; `logic.js` is byte-locked and was not changed. Amendment A12.2/A12.3 makes `p.photos[i]` fill slot `i`. **So the only way a caption can be true is for the photograph at that POSITION to show that subject** — which is precisely why the filename is not good enough.

**What A-L9 got wrong.** It picked photographs by filename keyword. Many of John's filenames lie: `ghi_veterinary_hospital/06_interior_reception.png` is a photograph of an exterior sign, and `08_interior_exam_room.png` in the same folder is a building. Many files are sliced fragments of a collage sheet with other photographs or caption strips baked into the frame (`1111_pet_hospital/06–10`, `pqr/03–08`, `stu/06–10`, `def/05,07`, `ghi/03–05,09–11`, `abc/14–18`, parts of `jkl/03,05`, `5555/10`). No keyword rule can detect either, and A-L9's fallback rule made it worse by filling an unmatched slot with "the first unused interior" — a boarding run under "Exam room".

**The fix.** The controller viewed every source image and wrote a content-verified map, committed as **`seeds/hospitals/photos/curation.json`** (its `_comment` records what it is). For each of the eighteen slugs it lists the practice type's six slots **in order**; each value is the source filename whose CONTENT shows that slot's subject, or `null` where no single photograph in that folder truthfully does. For every slug it names the map is **authoritative** and the keyword path is not consulted; the keyword path survives only for a slug absent from the map. `scripts/prepare_photos.py` gained `load_curation()` and `validate_curation()`, and refuses (`SeedDataError`, exit 2, before anything is written) a curated slug the seed file does not name, slot keys that are not the type's list in order, a referenced file the folder does not hold, and one file used for two slots.

**Empty slots are the point, not a shortfall.** Files are numbered by POSITION — slot `k` writes `<k>.webp`, so an empty slot leaves a gap and `p.photos[i]` still fills slot `i` — and an empty slot writes no file, records `{"slot": …, "file": null, "source": null, "caption": null}` in `index.json`, is stored as a JSON `null` at its position by `scripts/seed_listings.py::photo_paths`, and is served as `null` (not a URL) by `serialise`, with `GET …/photos/{n}` a 404 for it exactly as for an out-of-range index. The design's own `photoSet`/`heroSrc`/`thumbSrc` already tolerate a null there — `src: (p.photos && p.photos[i]) || ""` gives `hasSrc: false`, which renders **the design's own placeholder** — so no design change and no new amendment was needed, and `frontend/src/logic.test.ts` now pins that behaviour. Absent beats faked: a placeholder is honest, a boarding run captioned "Exam room" is not.

**The measured result: 73 of the 108 slots filled, 35 empty** (108 files/4.0 MB before → 73 files/2.3 MB after). Every hospital has its exterior. Four have **nothing else** — `1111_pet_hospital`, `ghi_veterinary_hospital`, `pqr_veterinary_hospital` and `stu_veterinary_specialist_center` — because their interiors exist only as collage fragments or mislabeled exteriors. **John owes clean interior images for those four**; until they arrive those screens show five placeholders each, which is the correct behaviour and not a bug to work around.

**Supersedes A-L9's fallback rule** ("an unmatched slot takes the first unused interior, then any unused file"): a slot with no truthful photograph is now left empty rather than filled with the nearest spare. A-L9's keyword selection itself stands for any slug the curation map does not name.

**Controller amendment A-L11 (2026-09-09; John's ruling verbatim: "HAS FAILED and wiped out all the images - if the logic is trying to match and failing then surface all images uploaded and have the user articulate what it is and render ALL images - what was 9 images now are only showing 3 after this hotfix!!!").**

**Root cause.** A-L10 placed a photograph only where a by-eye match to one of the design's SIX fixed slot captions existed, and left every other slot empty. Underneath it is a design fact, not a bug: `photoSet(p)` in `Practice Match V3.dc.html` renders exactly six captioned slots per practice, the captions are fixed by practice type, and the API sent no caption at all — so a photograph could only ever sit under a TRUE caption by being the one that shows that slot's subject. Six per hospital was therefore a ceiling nobody had stated, and A-L10 turned it into a floor: John's eighteen folders hold **195 photographs**, of which A-L10 rendered **73**. (Both counts are measured — `seeds/hospitals/photos/index.json` and the per-hospital pin in `tests/seeds/test_photo_inventory.py` are the evidence. An earlier draft of this record repeated the hotfix brief's subtraction instead of counting the folders, which is how a number nobody could reproduce got as far as a document of record; A-L12 struck it.) `def_veterinary_hospital` holds nine photographs and showed three, which is the number John counted. The ceiling was never surfaced to him, and that is the failure here — not the curation, which is still right about which image best fits a slot.

**The rule, reversed.** Never drop an uploaded image. Every photograph renders, and a photograph carries its OWN description — the supplier's or the seller's words — with the design's fixed slot caption as the FALLBACK for a slot that has none.

* **`scripts/prepare_photos.py`** keeps `curation.json` and its four validation arms, and keeps the design's four slot lists as `SLOT_COUNT = 6` positions; `MAX_PHOTOS` is gone, and `index.json` states `slot_count: 6` instead of `max_photos: 6`. Positions 1-6 are those slots: the curation places what it names, then every remaining image of the folder — composites and sliced sheets included, they are John's material — fills a still-empty slot in FOLDER order. Whatever is left becomes positions 7, 8, … with `slot: null`. A slot is empty only when the folder holds fewer images than the design has slots. Every entry carries `file`, `source`, `sha256`, `width`, `height`, `bytes`, `quality`, `slot` and **`caption`** (`caption_of(source)`, the supplier's filename description).
* **The description reaches the browser.** `migrations/090_listing_photo_captions.sql` adds `listing.photo_captions jsonb NOT NULL DEFAULT '[]'::jsonb`; `scripts/seed_listings.py` writes it in both halves of the UPSERT, parallel to `photos` (same length, `null` at a position with no photograph); `app/api/listings.py::serialise` emits `photo_captions` beside `photos`, and a position past the sixth is an ordinary position — a hospital with eleven photographs serves `/api/listings/{id}/photos/11`. **Numbering (A-L12, controller, 2026-09-09):** the first cut of this hotfix took `024`, which is inside the Census plan's reserved `017`–`059` (its D14; `060`+ is that plan's Phase B). It is renumbered to `090`, and **`090`–`099` is now the reserved range for platform and hotfix migrations on `main`** — `080`–`089` are the map engines. `scripts/migrate.py` applies by filename order and this file depends only on `016_listing.sql`, so nothing in between has to exist first; the suite's template database applies the whole ladder into a fresh database on every run, which is where that is proved. The Census plan's D14 is amended on its own branch, not here.
* **Amendment family A15** (six literal script edits through the D15 engine; A13/A14 are reserved by the design-dropdowns branch). A15.1/A15.2: both `photoSet` branches read `(p.photoCaptions && p.photoCaptions[i]) || v[1]` for the caption and the placeholder. A15.3a-A15.3d: each branch appends one tile per photograph beyond the sixth — id `ph-<listing>-extraN`, index continuing the design's numbering, caption the photograph's own description and otherwise `"Photo N"`. No template edit was needed: the detail grid (`<sc-for list="{{ d.photos }}">`) and the docked panel's carousel (`withPhoto`, `counter`, `dots`) already iterate whatever `photoSet` returns, so eleven tiles wrap into more rows and the carousel counts 1/11 on its own. `frontend/src/listings/load.ts` maps `photo_captions` → `p.photoCaptions`, and only when the API sent one.
* **Pixel-safe, and proved:** the design's fixtures carry no `photos` and no `photoCaptions` AT ALL — `p2`'s three photographs are the `SRC` map, keyed by slot id — so `p.photoCaptions && …` is falsey for every caption (each one is the design's own) and `p.photos && p.photos.length > views.length` is falsey for every practice, which makes the extra arm `[]` and appends no tile anywhere. `npm run test:visual:baselines && npm run test:e2e` pass with `frontend/tests/baseline-manifest.json` unchanged.

**The measured result: 195 files, every photograph in the eighteen source folders, and not one empty slot** (A-L9 rendered 108 files/4.0 MB · A-L10 73/2.3 MB · A-L11 all 195/7.4 MB). Per hospital: `abc_animal_hospital` 18 · `2222_pet_hospital` and `5555_new_york_veterinary_specialist_hospital` 12 · `3333_santa_barbara_veterinary_specialist_hospital`, `4444_denver_veterinary_specialist_hospital`, `456_pet_er`, `ghi_veterinary_hospital`, `mno_pet_hospital`, `yz_rural_animal_hospital` 11 · `1111_pet_hospital`, `123_route66`, `6666_dallas_veterinary_specialist_hospital`, `789_lake_tahoe_pet_hospital`, `jkl_animal_hospital`, `stu_veterinary_specialist_center`, `vwx_veterinary_hospital` 10 · `def_veterinary_hospital` 9 · `pqr_veterinary_hospital` 8. The four hospitals A-L10 left with nothing but an exterior — `1111_pet_hospital`, `ghi_veterinary_hospital`, `pqr_veterinary_hospital`, `stu_veterinary_specialist_center` — now show all ten or eleven of their images; their interiors are still collage fragments and mislabeled exteriors, so **clean images from John would still improve them**, but nothing is hidden waiting for those.

**Supersedes A-L10's empty-slot rule** ("a slot with no truthful photograph stays empty, where the design renders its own placeholder"): an empty slot is now only ever a thin folder. A-L10's curation map, its four refusals and its positional numbering all stand, and its record above stays as the account of why the captions are the design's.

**Queued for John, deliberately NOT changed here.** The design's own copy line "Six views per practice, all photographed at the same location…" is design copy and now understates what the page shows. It is the controller's to put to John, not this hotfix's to edit.

---

**Controller amendment A-L14 (2026-09-10 ~18:20 WITA; Task SD1 — John's eleven Dallas hospitals, their 119 photographs and the provenance he attached to them).** He supplied a table (name, area segment, real mappable address, operating hours, fake phone) and a JSON shape carrying five booleans: `phone_is_fake`, `address_is_real`, `address_is_seed_anchor`, `business_identity_is_fictional`, `operating_hours_is_seed_data`. **The businesses are fictional and the addresses are real** — that is his explicit ruling, and the addresses are what make the Census market data resolve, which is the point of them.

**Rulings made in the brief.** (1) His JSON's `address` → `street` and `postal_code` → `zip`, because eighteen existing rows and the loader already use those names, and **nothing else is renamed**; the mapping is recorded in `seeds/hospitals.json`'s own `note` and pinned by a test, so it is visible rather than silent. (2) The five booleans are **data, not UI** — nothing renders them; they exist so the Admin Data Sources drill-down and the seller controls can read them, and so nobody reverse-engineers the wrong conclusion from a real street address. (3) His "Area / market segment" column becomes each row's `area` verbatim ("Far North / Preston corridor" and the rest), because that is the community label the design shows and his segments are exactly that. (4) Practice types are written out rather than derived, because D4's name-and-hours derivation reads Beta and Indigo as small-animal practices: Alpha and Beta Specialty, Delta and Indigo Emergency, the other seven Small animal.

**The photographs, and the rule that had to be corrected mid-task.** Content verification of all 119 found that roughly eighty carry a readable business name — and the name is the hospital's OWN fictional one, rendered into the image. The brief's original blanket refusal would have discarded most of the set for identifying nobody. **Corrected ruling: the seed's own name is encoded; a street number matching that seed's own already-published address is encoded; only a third-party business name, a legible licence plate or an identifiable face is refused; composites and contact sheets are all encoded and never given one of the design's six captioned slots.** Two were refused, both recorded as data in `seeds/hospitals/photos/descriptions.json` with a reason rather than quietly omitted: `alpha_dallas_11_individual_images.png`, whose wall signage reads "COMPASSIONATE HEARTS", a plausible real business; and `alpha_dallas_05.png` (controller ruling on the report), a monument sign whose entire subject is "4140 CEDAR SPRINGS ROAD" — a real-looking Dallas address that is **not** this listing's, which tells a buyer the practice is somewhere it is not. Every other street number is kept and flagged `own_street_number`, so the class can be re-cut without re-reading anything. Every photograph carries its own verified description as its caption, which is what makes the slot map safe here: none is ever read under one of the design's fixed slot captions.

**Provenance storage.** `migrations/091_listing_provenance.sql` adds `provenance jsonb NOT NULL DEFAULT '{}'::jsonb`. One bag rather than five boolean columns, because `listing` holds real sellers' listings too and "is the telephone number fake" has no answer for a real practice: five columns would leave five permanent nulls on every real row, where `{}` says exactly what is true — no provenance claims are recorded. `091` is the next free number in `090`–`099`, main's platform and hotfix range (A-C10).

**DEVIATION FOR JOHN — the 25 km radius, one row.** `tests/seeds/test_geocode_bounds.py` has enforced a hard 25 km bound between a listing and its city centroid since Task L7, and its own docstring says never to widen it. **Alpha is 25.43 km from the Dallas centroid.** It is not a bad geocode: the address matched exactly, one candidate, a real TIGER line and a Census tract, and John's own segment for it reads "Far North / Preston corridor" — the northern edge of a city whose limits run well past 25 km from downtown. The bound itself is UNTOUCHED at 25.0 for the other twenty-eight; Alpha is a named exception carrying its measured distance and a ceiling of 26.0, pinned in both directions so a second row cannot join it quietly. **Accepted as a controller amendment and referred to John**: if he wants the radius absolute, the recourse is his — a different address, a per-market radius, or the row dropped.

**INFERENCE, not a fact.** John's telephone numbers run `214-555-0101` to `214-555-0110` in table order and he gave eleven hospitals. Lima is recorded as `214-555-0111` and the inference is pinned as `LIMA_PHONE_IS_INFERRED` in the tests so it is visible in the code as well as here.

**Owed at merge:** `CLAUDE.md` still says "eighteen demo hospitals" in two places — the controller's to change, not the implementer's. And QA must be re-seeded and then `env PYTHONPATH=/app python scripts/census_load.py geocode` run inside the container (A-C30, `DEPLOY.md`), or the eleven will carry no market data.
