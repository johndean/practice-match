...
* **The description reaches the browser.** `migrations/024_listing_photo_captions.sql` adds `listing.photo_captions jsonb NOT NULL DEFAULT '[]'::jsonb`; `scripts/seed_listings.py` writes it in both halves of the UPSERT, parallel to `photos` (same length, `null` at a position with no photograph); `app/api/listings.py::serialise` emits `photo_captions` beside `photos`, and a position past the sixth is an ordinary position — a hospital with eleven photographs serves `/api/listings/{id}/photos/11`.
