...
- **(a) 100 % lines AND branches, backend.** Every task's local gate is `poetry run pytest -q -W error --cov=app --cov=scripts --cov-branch --cov-fail-under=100`. `scripts/` is in the coverage scope for this plan because two of its four deliverables live there. CI keeps its own thresholds plus `diff-cover coverage.xml --compare-branch=origin/main --fail-under=100`, which this plan must also satisfy.
...
ls frontend/tests/baseline-manifest.json                              # the fifteen frozen hashes
```
...
The Dallas row is the verification anchor and has already been probed: `17727 Dallas Pkwy, Suite 150, Dallas, TX 75287` resolves to `(-96.829738, 32.991596)` — that is `lng = -96.829738`, `lat = 32.991596`. If your run disagrees, stop.
...
def test_slugs_are_unique_and_name_the_photograph_folders() -> None:
    ...
    for slug in slugs:
        ...
        assert slug + PHOTO_SOURCE_SUFFIX, slug  # the folder name prepare_photos.py will open
...
def test_the_dallas_anchor_is_the_probed_coordinate() -> None:
    """The one coordinate verified live against the Census Geocoder on 2026-09-06."""
    dallas = next(h for h in hospitals() if h["slug"] == "6666_dallas_veterinary_specialist_hospital")
    assert (dallas["lat"], dallas["lng"]) == (32.991596, -96.829738)
...
def _noisy(path: Path, size: tuple[int, int]) -> None:
    """A deliberately incompressible image: a flat colour would encode to a few hundred bytes
    and prove nothing about the 250 KB ceiling."""
    import random
...
def source(tmp_path: Path) -> Path:
    ...
    for i, name in enumerate(
        ...
        _noisy(folder / name, (2400, 1600) if i % 2 else (1200, 2000))
    ...
    for edge in (MAX_EDGE_PX, FALLBACK_EDGE_PX):
        ...
        for quality in QUALITY_LADDER:
            ...
            if dest.stat().st_size <= MAX_BYTES:
                return {
                    "file": dest.name, "source": src.name, "bytes": dest.stat().st_size,
...
```
Expected: `[photos] 72 files, <N> MB → …`; 72 files (18 × 4); the directory total **≤ 18 MB** (the arithmetic ceiling is 72 × 250 KB = 17.6 MB; D3 estimates ≈ 15 MB). If it exceeds 18 MB something has bypassed `MAX_BYTES` — **STOP**.

If any of the eighteen folders is missing or has fewer than four images the run reports it; fewer than four is fine (the pipeline takes what is there), a missing folder is exit 2 and a **STOP**.
...
git add pyproject.toml poetry.lock scripts/prepare_photos.py \
        tests/scripts/test_prepare_photos.py tests/seeds/test_photo_inventory.py \
        seeds/hospitals/photos
...
from scripts.migrate import normalize_dsn
...
seed data is missing or malformed (fix the file, redeploy). The same run is available as a
container role — `bash scripts/start.sh seed --reset` — for a one-off Railway service command.
````
...
git add scripts/seed_listings.py tests/scripts/test_seed_listings.py \
        ...
        Dockerfile tests/test_build_config.py DEPLOY.md tests/test_docs.py
...
from app.api.listings import LIST_TTL_S, decode_cursor, encode_cursor, photo_file, relative_listed
...
def serialise(row: dict[str, Any], now: datetime) -> dict[str, Any]:
    ...
    photos = row["photos"] if isinstance(row["photos"], list) else json.loads(row["photos"])
    return {
...
poetry run pytest -q -W error --cov=app --cov=scripts --cov-branch --cov-fail-under=100
```
Expected: all green at 100 % lines and branches. `app/api/listings.py`'s branches that need a test if coverage complains: the `market` length refusal, the `photos` value already being a list vs a JSON string (psycopg2 returns `jsonb` as a Python list, so the `json.loads` arm is exercised by `test_photo_file_refuses_an_escaping_path`-style direct calls — add a unit test on `serialise` with a string `photos` value if the arm is uncovered).
...
- Produces: `frontend/src/listings/load.ts` exporting `MARKET_ZOOM = 10`, `US_CENTER`, `type ApiListing`, `type Practice`, `type Markets`, `type ListingsPage`, `toPractice(row: ApiListing): Practice`, `centroid(practices: Practice[], market: string): [number, number]`, `applyListings(rows: ApiListing[], practices: Practice[], markets: Markets): void`, `loadListings(fetchFn: typeof fetch, practices: Practice[], markets: Markets, url?: string): Promise<boolean>`. `frontend/src/logic.js` exports `{ Component, MARKETS, P }`. `frontend/tests/design-listings.mjs` exports `toApiShape(practice)` and `designListingsBody()`.
...
> **⚠️ STOP — two design readers this plan cannot verify until Browse V3 is merged.** `logic.js` (V2) resolves a listing's title through `practiceName(p)`, which is `NAMES[p.id] || p.area + " Veterinary"` — a fixture map keyed by the design's own ids — and its photographs through `photoSet(p)`, whose `SRC` map hard-codes three files for `p2` only. Neither reads `p.name` or `p.photos`. **D5 ("the hospital name is stored and shown wherever the V3 design has a title slot") and D3 ("served by the API through `GET /api/listings/{id}/photos/{n}`") therefore depend on whether the V3 port reads those two fields.** Step 0 below is a probe. If V3 still uses fixture-id lookups, **STOP and report to John** with the two options — a fourth documented port edit (`NAMES[p.id] || p.name || p.area + " Veterinary"`, and `SRC` replaced by `p.photos`), or shipping the derived fallback title and empty photo slots — and do not choose one unilaterally. `load.ts`, the API and the seeds all carry `name` and `photos` either way, so the fix is a one-line change once John rules.

- [ ] **Step 0: Probe the merged V3 `logic.js` for the two readers**

```bash
cd "/Users/johndean/Development/Practice Match/frontend"
grep -n "practiceName" src/logic.js | head
grep -n "p\.name\|sel\.name\|\.photos" src/logic.js | head -20
grep -n "photoSet" src/logic.js | head
```

Record what you find in the hand-back note (Task L7). If neither `p.name` nor `p.photos` is read, raise the STOP above **now**, before writing code — the rest of this task is unaffected either way, but John's answer changes what QA will show.
...
import { applyListings, centroid, loadListings, MARKET_ZOOM, toPractice, US_CENTER } from './load';
...
describe('loadListings', () => {
  ...
  it('asks for the whole catalogue in one page and sends the session cookie', async () => {
    const spy = ok([]);
 ...
 *
 * A refusal (the anonymous 401 the identity design intends), an unreachable API or an
 * unparseable body all leave the design's fixtures in place — the screen must never go blank
 * because a read failed. A 200 always wins, empty list included: at that point the API is the
 * source of truth and the design's own "no results" state is the honest thing to show.
 */
...
```
Expected: 0 type errors; 100 % lines/branches/functions/statements on `src/**` (the exclude list unchanged); the fifteen frozen unchanged-screen hashes byte-identical; the smoke suite green; **27 visual states at `maxDiffPixels: 0`**; 27 DOM-oracle states green.
...
Exit 3 means the container could not reach Postgres — check `railway logs --service api --environment QA --lines 50`. Exit 4 means `seeds/` did not reach the image — confirm the `COPY seeds/ ./seeds/` line landed and redeploy.
...
1. **Browse Practices** shows result cards for the seeded hospitals, not Cedar Park / Round Rock / South Austin — the design's old nine are **gone**.
...
> the nine placeholder practices the design shipped with. Each one uses the hospital names you
> supplied, sits at its real street address on the map, and carries a fake 555 phone number,
> its opening hours and up to four of the photographs you sent. Everything else about the
...
If Step 0's probe found that the V3 design does not read `p.name`/`p.photos`, add to the note:

> **Outstanding decision for John:** the approved V3 design still looks its hospital titles and
> photographs up from its own built-in fixture list rather than from the listing data, so the
> seeded names and photographs are stored and served but not yet shown in the title and photo
> slots. Two options: a one-line change to the ported design script so it prefers the listing's
> own name and photographs, or leave it until the seller wizard lands in Wave 2b. Say which.
...
|---|---|---|
| §1 eighteen hospitals replace the nine, QA only, production unaffected | L4, L7 | `test_seed_writes_eighteen_published_seed_rows`; L7 Steps 7 and 10 |
| §2 John's table, verbatim | L2 | `test_johns_table_is_reproduced_verbatim_and_in_order` |
...
| D2 Census Geocoder, coordinates + tier committed | L2 | `test_the_geocode_provenance_is_recorded`, `test_the_dallas_anchor_is_the_probed_coordinate` |
...
| D3 WebP ≤ 1600 px, ≤ 250 KB, metadata stripped | L3 | `test_every_output_is_webp_within_the_dimension_and_size_ceilings`, `test_metadata_is_stripped` |
| D3 inventory with SHA-256s; committed outputs | L3 | `test_the_inventory_records_a_matching_sha256_and_dimensions`, `test_every_committed_file_matches_its_recorded_hash_and_size` |
| D3 size ceiling (≈ 15 MB, hard 18 MB) | L3 | `test_the_committed_set_stays_under_the_size_ceiling` |
| D3 served by the API, not bundled | L5 | `test_photos_are_served_as_webp_with_a_private_cache_header`, `test_photos_are_refused_to_an_anonymous_caller` |
...
| D8 pins use the geocoded point; `location_disclosed` | L5 | `test_an_undisclosed_listing_returns_no_address_and_no_point` |
| D9 perf budgets in `tests/perf/test_api_latency.py` | L5 | `test_listings_p95_within_budget` |
| D9 100 % backend and frontend | every task's gate step | `--cov-branch --cov-fail-under=100`; `npx vitest run --coverage` |
| D9 zero regression on the unchanged screens | L6 | `tests/baseline-manifest.test.ts` + `npm run test:visual` |
...
**Gap found and escalated, not closed:** D5 and D3 both assume the design reads `p.name` and `p.photos`. The V2 port does not — `practiceName()` and `photoSet()` are fixture-id lookups. Whether V3 does cannot be checked until Browse V3 merges, so Task L6 Step 0 probes it and **STOPs for John** with two named options rather than inventing a fourth edit to the ported script. The data is stored and served either way, so the fix is one line once John rules.

**2. Placeholder scan.** No "TBD", no "TODO", no "implement later", no "similar to Task N", no "add appropriate error handling". Every code step carries real code. The two places that read like gaps are deliberate and named: the `0.0` / `REPLACE FROM THE GEOCODER RUN` values in `seeds/hospitals.json` (they are the output of the one step that must be run live, and the bounds test fails until they are replaced — that is the mechanism, not an omission) and L6 Step 0's STOP (a decision that is John's, with both options spelled out).
...
- `relative_listed`, `encode_cursor`, `decode_cursor`, `serialise`, `photo_file` are the exact names L5's tests import.
- `toPractice`, `centroid`, `applyListings`, `loadListings`, `MARKET_ZOOM`, `US_CENTER` are the exact names L6's tests import, and `toApiShape` in `design-listings.mjs` is their inverse.
- `REQUIRE_LISTING_READ` is one module constant, never wrapped (Constraint (g)); `PUBLIC_ROUTES` is untouched (Constraint (e)).
- `seed(dsn, *, reset=False) -> int` is the one signature L4's tests and L5's end-to-end test both call.
