...
**A-S5.1 (2026-09-08; John's ruling on A-S5 (5)):** no runs-per-hour limitation is accepted. The real limits stay exactly as they are and stay tested; the LOCAL suite becomes repeatable by resetting the test environment's rate-limit state before the API serves: `scripts/reset_rate_limits.py` deletes every `rl:*` bucket (the `bucket_key` prefix in `app/ratelimit.py`, SCAN + DEL, never FLUSHDB) from the configured `REDIS_URL` and prints the count; it FAILS CLOSED — it refuses (non-zero, one line to stderr) unless `ENVIRONMENT` is exactly `test` AND the Redis host is `localhost`/`127.0.0.1`, so it can never run against QA or production. `frontend/tests/targets.ts`'s `api` web-server command runs it after `migrate` and before `seed_persona`; `targets.test.ts` pins the command. Budgets are therefore per RUN only (sixteen sign-ins of thirty, three forgot calls of ten, one sign-up of five, each within a single run) and consecutive local runs are independent; the harness docstrings say so and drop every 'wait for the window' instruction. CI is untouched (fresh Redis per job; the script runs there too and deletes nothing). TDD: pytest for the script at 100 % line+branch (refusal on a non-test environment, refusal on a non-local host, deletion scoped to `rl:*` with other keys untouched, the count printed), the `targets.test.ts` pin RED first; `--cov=scripts` already covers it, so the backend gate and the six shell suites `for t in tests/scripts/test_*.sh; do bash "$t" || exit 1; done` join your gate list.
