...
**A-S5.1 (2026-09-08; John's ruling on A-S5 (5)):** no runs-per-hour limitation is accepted. The real limits stay exactly as they are and stay tested; the LOCAL suite becomes repeatable by resetting the test environment's rate-limit state before the API serves: `scripts/reset_rate_limits.py` deletes every `rl:*` bucket (the `bucket_key` prefix in `app/ratelimit.py`, SCAN + DEL, never FLUSHDB) from the configured `REDIS_URL` and prints the count; it FAILS CLOSED — it refuses (non-zero, one line to stderr) unless `ENVIRONMENT` is exactly `test` AND the Redis host is `localhost`/`127.0.0.1`, so it can never run against QA or production. `frontend/tests/targets.ts`'s `api` web-server command runs it after `migrate` and before `seed_persona`; `targets.test.ts` pins the command. Budgets are therefore per RUN only (fourteen sign-ins of thirty, three forgot calls of ten, one sign-up of five, each within a single run) and consecutive local runs are independent; the harness docstrings say so and drop every 'wait for the window' instruction. CI is untouched (fresh Redis per job; the script runs there too and deletes nothing). TDD: pytest for the script at 100 % line+branch (refusal on a non-test environment, refusal on a non-local host, deletion scoped to `rl:*` with other keys untouched, the count printed), the `targets.test.ts` pin RED first; `--cov=scripts` already covers it, so the backend gate and the six shell suites `for t in tests/scripts/test_*.sh; do bash "$t" || exit 1; done` join your gate list.

**A-S5.2 (2026-09-08; the implementer's second NEEDS_CONTEXT, two seams found by running the suite).** (S-1) `POST /api/auth/verify` flips its account to `verified` for the rest of the run, and every `verify` fixture token belonged to `unverified@` — so `gate-signin-verified`'s own capture destroyed the account `gate-check-email` and the resend flow need. Ruling — option (a): `scripts/seed_persona.py` seeds a tenth identity account, `verify-me@practice-match.test` (state `unverified`, the same password, never signed in by any state), and the twelve `verify` tokens belong to IT; `unverified@` is never verified by a test. The S3 pin in `tests/test_docs.py` and the harness constants gain the address; the seed's docstring and the QA runbook (S6) count ten accounts. (S-2) React materialises a filled `<textarea>` by also writing `defaultValue`, which is the element's child text; Vue sets the property only, so `gate-reapply` — the only approved state with a non-empty textarea — is pixel-identical but DOM-different. Ruling — option (a): `frontend/tests/dom.ts` compares a `<textarea>` by its `value` instead of its children (strictly stronger than today, when the app's value was compared nowhere), RED first in `dom.test.ts` (same value / different children → equal; different values → not equal; an empty textarea unchanged), and the 28 existing DOM snapshots must regenerate byte-identical. Recorded, accepted: the answer and re-apply flows restore their fixtures through the product's own reviewer API (`request_info` / `decline` as the `design` admin) — spec §8's staff half exercised for free; the report restates the sign-in budget with those sign-ins counted. Recorded for Task I10: a live QA run mutates QA's fixtures, so `scripts/seed_persona.py` is re-run against QA before every QA parity run (it is idempotent and restores every fixture) — the runbook step S6 writes down.
