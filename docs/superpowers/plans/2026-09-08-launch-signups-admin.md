...
| `app/main.py` | one `include_router` line, outside the `site_mode == "app"` block (D-I5d-5) (superseded 2026-09-09 by A-I5d.5) | I5d.3 |
...
- Modify: `app/main.py` (one `include_router` line, outside the `site_mode == "app"` block) (superseded 2026-09-09 by A-I5d.5), `tests/perf/test_api_latency.py` (`BUDGET_MS`, `STAFF_PATHS`), `tests/perf/test_query_plans.py` (`PLANS`, `INDEXES`, `SEEDS`)
...
- **Zero-regression surfaces named out loud.** `app/main.py` (one added line, outside the `site_mode` block) (superseded 2026-09-09 by A-I5d.5 — the line now sits inside that block), `app/mail/outbox.py::TEMPLATES` and `app/mail/templates.py::TEMPLATES` (one key each, and the paired count test that keeps them equal), `tests/perf/*` (two dict entries and one seed). `app/api/admin_users.py` is imported from and never modified. No screen changes until Task I5d.5, so the visual and DOM oracles are pure regression proof for Tasks I5d.1–I5d.4.

---

**Controller amendment A-I5d.5 (2026-09-09; John's ruling, verbatim):** "Task I5d.3 — CHANGE: Gate the entire Admin Launch Sign-ups router behind `SITE_MODE=app`. Do not expose the sign-up list or CSV export on production while Coming Soon, even to an `API_SECRET_KEY` bearer. Existing authentication, permission checks, re-authentication, and audit logging remain in force. The SEND operation remains refused until `SITE_MODE=app`."

This reverses D-I5d-5 (above, marked superseded rather than deleted): `app.include_router(admin_signups_router)` moves from unconditional to inside the same `if settings.site_mode == "app":` block as `admin_users_router`, `applications_router` and `listings_router`. Every route on the router — `GET /api/admin/signups`, `GET /api/admin/signups.csv` and `POST /api/admin/signups/launch-mail`, both the dry run and the real send — is a plain `404` while `SITE_MODE=coming_soon`, for any credential including the legacy `API_SECRET_KEY` bearer that D-I5d-5's whole rationale had rested on. Nothing about `require(...)`, `REAUTH`, `TokenCannotReauth` or `audit.write(` changes: they still guard and record every route exactly as before, for the one site mode (`app`) in which the routes are now reachable at all. The `SiteNotLaunched` 409 `NOT_LAUNCHED` refusal inside `launch_mail` stays as defence in depth, covered by tests that monkeypatch `settings.site_mode` after the app is built in app mode.

**Operational consequence.** The `interest_signup` rows this router reads only exist on production, and production runs `SITE_MODE=coming_soon` until launch — so the Admin Launch Sign-ups list and CSV export are reachable there only once the launch flip sets `SITE_MODE=app`. Until then, an operator reads the sign-ups with the read-only SQL RUNBOOK-identity.md §13 now gives (`SELECT email, created_at FROM interest_signup ORDER BY created_at`), not through the API. `scripts/verify-deploy.sh` and DEPLOY.md's site-mode notes are updated in the same commit to match: the router is probed as absent (404) on a coming-soon deployment and present-and-guarded (401 to an anonymous caller) on an app-mode one, exactly like `/api/admin/users` beside it.

---

**Controller amendment A-I5d.6 (2026-09-09; the task-I5d.5 review — APPROVED, spec ✅, 0 Critical, 0 Major, 5 Minor, 4 Info — with every Minor and two Infos ruled fixed at source, in the same fix round.** M-1: `DEPLOY.md`'s "App-mode-only surfaces" sentence overstated the app-mode probe as covering all five gated routers when `scripts/verify-deploy.sh`'s `else` branch only probes `/api/listings` and `/api/admin/signups` — corrected to name exactly what each mode probes, and the docs test widened to pin the corrected wording and reject the old overstatement. M-2: four more places in this plan (the file map, Task I5d.3's own Modify list, Open Questions §2 and the Self-review) still described the pre-ruling mount as current fact — an implementer could re-derive `app/main.py`'s shape from any of them and reintroduce the regression A-I5d.5 fixed — so each gained a `(superseded 2026-09-09 by A-I5d.5)` clause beside the original text, and the docs test now sweeps every plan line pairing that wording with the mount (pinned at exactly 4, so a fifth such line — or zero, a broken pattern — fails the count assertion too; the historical Step 3 code-instruction line, whose bold `**outside**` markdown means it does not match the sweep's plain-text pattern, was deliberately left as the frozen record of what Task I5d.3 already did). M-3: `tests/scripts/test_verify_deploy.sh` gained mode `signups_open` (an anonymous 200 to `/api/admin/signups` in app mode) and a case proving the script already fails it — a coverage gap closed, not a live hole, since `[[ "$code" == "401" ]]` already caught a 200; the case exists so a future relaxation of that check breaks the suite instead of shipping silently. M-4: the five D-I5d-5 rationale comments that read as false after A-I5d.5 (`app/api/admin_signups.py`'s `SiteNotLaunched` docstring and the `launch_mail` docstring's two mentions; two test docstrings in `tests/api/test_admin_signups.py`) each gained a parenthetical noting the supersession — comments only, no behaviour changed, `app/api/admin_signups.py`'s own guards and 409 untouched. M-5: RUNBOOK §13's interim SQL gained the connection recipe the repo already establishes elsewhere (`DEPLOY.md`'s seeding recipe, RUNBOOK §12's QA parity run) — the PostGIS service's `DATABASE_URL` pulled from Railway into an env prefix, never argv, never echoed — and the projection was extended with `count(*) FILTER (WHERE launch_mailed_at IS NULL) AS not_mailed, count(*)` so the pre-flip path answers the same question the (now-unreachable) dry run would have. I-3: the RUNBOOK docs test's §13 slice is now bounded to the next `## ` heading, matching the `DEPLOY.md` test's own bound, so a future §14 cannot satisfy its substring pins by accident. I-4: `tests/api/test_admin_signups.py` gained the app-mode positive for the legacy `API_SECRET_KEY` bearer on `/api/admin/signups` (200, once `SITE_MODE=app`), in the shape of `tests/api/test_admin_users.py`'s existing pin for `/api/admin/users`. I-1 (the CSV negative test's hard-coded literal) and I-2 (the `NOT_LAUNCHED` 409 being unreachable in a live process, kept as documented defence in depth) are recorded with no change, per the review's own verdict that both are adequate as they stand.
