...
- Steps mirror SL7's: RED unit test → the mapping module → one amendment (`A16.1`, the `adminVals()` Listings `rows:` source, with the design's five literal rows kept as the no-data fallback, exactly as A15.1 does for the seller) → `gen:design && gen:app` → the derived pins → the gate.
