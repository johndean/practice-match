...
**Controller amendment A-SL37 (2026-09-10 ~07:20 WITA; John closes two applied defaults, both ratifying what is built).** **D-SL28 — "Keep revenue-band detail hidden by default; server truth wins over the prototype."** Step 7's disclosure switch opens ON for a real seller's new listing, as spec D11 requires and as the app already does; the design prototype draws it OFF and that is not followed. The rule is general and worth stating once: where the prototype's drawn state and the server's truth disagree, the app shows the server. **D-SL25 — "Keep seeded photographs as seeded until the Admin & seller controls stage. Do NOT materialise them now."** SL7b's positional caption route stays, so all 195 seeded photographs can be re-described by their seller. Converting a seeded photograph into the seller's own asset record — which is what would let them be reordered or deleted — is DEFERRED to the Admin and seller controls stage and is not a lifecycle task. Until then, a task that needs to reorder or delete a seeded photograph is BLOCKED by this ruling rather than free to improvise a conversion.
...
**Controller amendment A-SL24 (2026-09-09 ~12:00 WITA; SL8 pre-flight rulings — they override the brief's literals where the two differ).** (1) **No fixtures behind an adapter (John: "every Admin tab must show real database data, never dummy rows"; A-SL17; the SL7 Critical-2 lesson).** With the `adminListings` adapter present, Admin › Listings renders the loaded rows or ZERO rows — never the design's five literal rows, whatever the API answers; a failed load renders zero rows and the design's existing error surface. The brief's "five literal rows kept as the no-data fallback" applies ONLY with no adapter (the reference and the preview). The frozen `admin-listings` oracle capture keeps its hash the honest way: the harness serves the design's fixture rows as a REAL page of `/api/admin/listings` (the D6 / A-SL23 (2) precedent) so the app renders them through the success path. (2) **The flow spec is SL8's deliverable (A-SL23 (8)):** `frontend/tests/listing-flows.spec.ts`, value-assertion, no stub armed, against the oracle's seeded eighteen: sign in as the seller persona → "Create a listing" creates a real draft → each step's PATCH persists → a photograph uploads with a caption → Submit → the dashboard shows the new row; then sign in as the design persona → Admin › Listings shows the real queue → decide → the seller's dashboard reflects it. (3) **Family id** is the next free one derived from the files (expected A17 — A15 main, A16 SL7). (4) The fifth fixture row ("Flagged"/Investigate) has no column behind it and is not reproduced — the design's row shape is used for the statuses that exist; recorded, not invented. (5) Before SL8's gates, merge `main` (now 7a85245 = 0.1.9: A13/A14 dropdowns + the re-pinned manifest) — the amendment-engine conflicts are the same class SL7 resolved; every `find` must match once; citations recomputed; the manifest inherits main's A14 re-pin and moves for nothing else.

**Controller amendment A-SL25 (2026-09-09 ~12:50 WITA; rulings on the SL7 fix-round re-review — NOT ALL ADDRESSED: A-SL23 (1)'s `wizAssets` half open; new 1 Critical, 2 Major, 4 Minor, 5 Info; closed in fix round 2).** (1) **The `wizAssets: null` deviation is REJECTED (Critical-A).** With an adapter present the wizard renders the server's draft and NOTHING else: A16.4's tile source keys on adapter presence exactly as A16.1 does (`this.props.listings ? s.wizAssets : <the design's literal>`), so no value of `wizAssets` can ever reach the design's three fixture tiles or A16.12's "Photos attached 3" on the app; `wizAssets` is an ARRAY on every adapter path — `[]` on a failed `create()`, `[]` on a failed `get()`, and the draft's own assets on success. `startWizard` chains `create().then(get)` and opens the returned draft through the SAME setter `openWizard`'s success arm uses (one place turns a `WizardDraft` into `editingId`/`w`/`wizAssets`), so a new listing shows no tiles and "Photos attached 0". The three frozen wizard captures keep their hashes the honest way (A-SL23 (2), the dashboard's own precedent): `prepare()` answers `GET /api/seller/listings/{WIZARD_LISTING_ID}` — and the submit's answer — with a REAL draft whose assets are the design's own three photograph tiles, DERIVED from `logic.js`'s `wizAssets` literal (a `design-wizard-draft.mjs` beside `design-seller-listings.mjs`, never hand-copied) in `ApiAsset` shape, and whose fields map through `toWizardState` to exactly the design's `state.w` — a vitest pin proves `toWizardState(<that draft>)` deep-equals `new Component({}).state.w`, and `harness.test.ts` pins the body's assets against the literal (this also closes Info-A: `newDraftBody` becomes that derived body, pinned). Manifest unchanged is the acceptance test. (2) **Major-A:** `openWizard`'s rejection arm resets what A16.14's does — `editingId: null`, `wizAssets: []`, `w` = the design's initial literal, `wizSubmitted: false`, `step: 1` — and shows the message; a failed Edit can never leave another listing live under the wizard. (3) **Major-B:** chain, never nest — one `reloadListings()` literal (A16.x) used by every reload (Withdraw/Pause/Republish success, Submit success, save-and-exit), whose single rejection arm is A16.9's (`myListings: []`); `logic.js:977` and `:1279` take it. A characterisation case for each: a refused reload after a successful transition and after Submit rejects nowhere and leaves the state A16.9 defines. (4) **Minor-A:** one pytest assertion that `serialise_draft`'s key set contains none of `title`, `meta`, `note`, citing `toDashboardRow`'s `DesignRow` arm as the reason. (5) **Minor-B:** a `creating` flag in the design's state literal (script literal, A16 entry); `startWizard`'s adapter arm returns early while a create is pending and clears the flag on both arms — no markup, no spinner (the design has no busy state; recorded for Rev 3 in spec §14). (6) **Minor-C:** the three "no-page stub" comments (`screens.ts:264`, `harness.ts:1020`, `:1198`) rewritten to the mechanism that exists. (7) **Minor-D:** `photo_captions()` returns exactly one caption per photograph on BOTH paths (pad `""` / truncate to `len(photos)`), the docstring says so, and four tests pin it (longer and shorter, owned and not). (8) **Info-B:** with an adapter present the Add-files handler never takes the design's counter arm; with no `editingId` it returns early (the failed-create banner already stands). **Info-D:** the JSDoc indent fixed. **Info-C** and **Info-E** are record-only (no observable effect; the remote parity worry does not arise). (9) **⚠️ items:** run `tests/perf/test_api_latency.py` against the Browse list with `_SELECT`'s new `asset_captions` aggregate and quote the figures in the report (budget 150 ms, quality policy); the QA click-through of the wizard is SL9's (the branch reaches QA at 0.1.10); migration `033`'s number is re-derived at the merge in SL8/SL9; the flow spec runs with NO stub armed (A-SL24 (2) chose the plan's line; A-SL23 (8)'s "against the stubbed API" is superseded). (10) **Click-to-caption for EXISTING photographs is its own task, SL7b, after this round and before SL8:** two literals (the step-6 tile's `onClick` carrying the asset id, photographs only, one chained promise into `wizErr`), PLUS a positional route `PATCH /api/seller/listings/{id}/photos/{n}` `{caption}` writing `listing.photo_captions[n]` for a seed photograph (claims the listing, A-SL21) so all 195 seeded photographs can be re-described — the tile is pixel-safe (one occurrence, A7.3 precedent, step 6 uncaptured). DEFAULT queued for John (D-SL25): seeded photographs stay path entries with positional captions, so they can be re-described but not reordered or deleted until asset rows are materialised at claim time — his call whether that materialisation is a lifecycle task now.

**Controller amendment A-SL26 (2026-09-09 ~13:35 WITA; rulings on the round-2 re-review — NOT ALL ADDRESSED: round 2 closed 12/12 findings and 9/9 A-SL25 points, but found CRITICAL-B; closed in fix round 3).** (1) **CRITICAL-B — the wizard cannot save a step against the real API.** A16.6 and A16.15 pass the whole `w` to `patch()`, `seller.ts` sends it unfiltered, and `columns_for` (D10, one-directional whitelist) refuses every stray field with 400 — so step 1's Continue fails on seventeen strays, step 6's Continue asks for a step that takes no fields, and save-and-exit traps the seller. D10 stands: the API is NOT loosened. The ADAPTER owns the step → fields map (`STEP_FIELDS` in `seller.ts`, wizard key names), `patch(id, step, w)` sends only that step's keys — the plan's own test name, `patch() sends only the step's fields and the step in the query`, is the RED, written as the plan wrote it — and the design's literals keep passing `w`. Step 6 has no fields: its Continue advances and its Save-and-exit exits WITHOUT a PATCH (assets are saved on upload). **Two-way pin against drift:** a pytest reads `STEP_FIELDS` out of `frontend/src/listings/seller.ts` (as `tests/test_docs.py` reads files) and asserts, for every step, that the wizard keys — mapped to columns through the same key↔column table `serialise_draft`/`toWizardState` use — are exactly `columns_for(step)`'s whitelist, and that `w.photos` appears in no step; plus one pytest that PATCHes each step with exactly its `STEP_FIELDS` payload and gets 200. The implementer's concern 3 was this defect's visible tip, not a harmless passenger. (2) **Minor-E:** `photo_captions()` yields strings only — an explicit `null` at a position becomes `""` like a missing one — the docstring states the single rule, one test pins the interior null. (3) **Info-F:** the oracle's submit answer carries `status: "in_review"` (the API's own post-submit status) — the stub tells the truth even where nothing reads it. **Info-H:** the `creating` test's comment states the `reactive` reason. **Info-I:** the report's count corrected to eleven. **Info-G:** record-only. (4) **SL8 is not dispatched before this round closes**; SL8's no-stub flow spec (A-SL24 (2)) is the end-to-end proof that presses Continue against the real API, and it now also asserts each step's PATCH is 200 with the step's own fields.

**Controller amendment A-SL27 (2026-09-09 ~14:15 WITA; rulings on the round-3 re-review — NOT ALL ADDRESSED: round 3 closed 6/6 and 4/4, but found CRITICAL-C, MAJOR-D, MAJOR-E, INFO-J–L; closed in fix round 4 by a FRESH implementer on the most capable model — the loop's rule for round 4 — and the loop's architectural lesson is applied: the seller half of the end-to-end flow spec lands IN THIS ROUND).** (1) **CRITICAL-C — a new listing cannot pass step 1 or 5.** `create` inserts no `type`/`ownership`/`bldg`/`facility_type`; `toWizardState` turns each null into `""`; `openDraft`'s `Object.assign` overwrites the design's four defaults; `columns_for` refuses `""` for an enum. Adapter-side fix, pixel-safe: `toWizardState` OMITS a null column (the design's `w` literal supplies the default, exactly as the prototype shows it), so the first Continue PATCHes the design's defaults and is 200. The fixtures that hid it are made honest: the oracle's created draft (`design-wizard-draft.mjs`) carries NULL for every column `create` leaves null — `harness.test.ts` pins that null set, a pytest pins `serialise_draft(<bare create>)`'s null set, and the `state.w` pin becomes `Object.assign({}, state.w, toWizardState(bareDraft))` deep-equals `state.w`; a `logic.test.ts` case drives create → first Continue and asserts the recorded PATCH carries the design's default enums; the per-step 200 pytest gets a sibling that PATCHes step 1 and step 5 with exactly the design's default values. (2) **MAJOR-D — Save-and-exit with a blank required numeric is a 400 that traps the seller.** A draft is incomplete by nature: `patch()` gains a `partial` mode used by Save-and-exit (and by the rail, (3)) that OMITS a field whose value is `""` when the API would refuse `""` for it (the required numerics — derive the list from `columns_for`/`OPTIONAL_NUMERIC`, never hand-typed: the drift pin extends to it); Continue keeps the full payload behind the design's own guards. Pins: vitest (blank `est` on step 1 and blank `price` on step 3 are omitted in partial mode and sent in full mode), a `logic.test.ts` case (Save-and-exit on step 1 with a blank year is 200 and exits), pytest (a partial step-1 payload without `est` is 200). The rejection arm stays as A-SL23 (3) ruled (the seller stays, with the message) — it now fires only on genuine server refusals. (3) **MAJOR-E — the step rail discards the step being typed.** One new A16 literal: with an adapter present, `steps[i].go` PATCHes the current step first (partial mode, the existing single rejection arm; a refusal keeps the seller on the current step with `wizErr`; steps 6 and 8 make no PATCH), then moves; the reference is untouched. `harness.ts` gains the `…/{id}?step=*` route the three rail-pressing captures (`screens.ts:160-162`) need; manifest unchanged vs 6b5da15 is the acceptance. A `logic.test.ts` case: type on step 5 → rail to 6 → the recorded PATCH is step 5's fields. (4) **INFO-J:** a vitest pins that every key of the design's `w` except `photos` belongs to exactly one step of `STEP_FIELDS`. **INFO-L:** the report's phrasing corrected ("a GET re-read, no PATCH"). **INFO-K:** record-only — the pin fails loudly under every perturbation tried; no formatter is configured under `frontend/`. (5) **The architectural fix — the seller half of `frontend/tests/listing-flows.spec.ts` lands in THIS round, not SL8** (A-SL23 (8) and A-SL24 (2) are amended accordingly; SL8 adds the admin half — Admin › Listings shows the real queue → decide → the seller's dashboard reflects it — and the seeded-listing Edit assertions the SL8 brief lists): value-assertion, `account-flows.spec.ts`'s shape, NO stub armed, in the `app` project's `testMatch` (+ `playwright-config.test.ts`'s pin): sign in as the seller persona → "Create a listing" → the dashboard/API shows a new draft → each of steps 1–5 and 7 filled through the design's own controls and Continue → each PATCH answered 200 (assert on the API's own row via `GET /api/seller/listings/{id}` after each step, not on the card) → step 6 uploads one photograph with a caption → step 8's preview shows the typed values and the photograph → Submit → `status` is `in_review` on the API and the dashboard row shows it → Save-and-exit and the step rail exercised once each. Three rounds passed every gate while no step could be saved because nothing pressed Continue against the real API; this spec is why round 5 will not be needed.

**A-SL27 addendum (2026-09-09 ~14:20 WITA; John's ruling, verbatim intent: "ensure zero-gap correction" on the round-3 list, with systematic-debugging + TDD).** INFO-K is FIXED at source, not recorded: `STEP_FIELDS` moves out of a TypeScript literal into one data file both sides read without parsing — `frontend/src/listings/step-fields.json`, imported by `seller.ts` and `json.load`ed by the pytest drift pin; the regex is gone; the pin still fails in both directions. The `partial`-mode omission list derives from the API side and is pinned the same way. Every finding: trace the data flow, fix at the source the trace names (Critical-C → `toWizardState`'s `""` for null; Major-D → `patch()` has no partial mode; Major-E → `steps[i].go` bypasses the save arm), one change per finding, RED first.
**Controller amendment A-SL28 (2026-09-09 ~15:20 WITA; ruling on round 4's NEEDS_CONTEXT — the photograph half of the flow spec).** The photograph proof runs LOCALLY, no skip, no new dependency: `moto[s3]` (already a dev dependency) mocks S3 in-process inside a TEST-ONLY launcher `tests/e2e/api_under_test.py` (refuses to start unless `ENVIRONMENT=test`; creates `S3_BUCKET`; then uvicorn as `targets.ts` runs it today); `targets.ts` runs the launcher and adds four dummy `S3_*` defaults under the same "process env wins" rule (pinned); under `PW_APP_URL` nothing changes. The photograph test runs for real locally and in CI; a loud skip remains only for a `PW_APP_URL` target answering 503 STORAGE_UNAVAILABLE. INFO-J wording ratified as pinned (`anon`/`revBand` exactly two, every other key exactly one). `revBand` true on the API (D11 hide-by-default, the spec) vs false in the design literal: recorded for John (D-SL28), no change — the stub's value is a legitimate state. Leftover drafts on the seller persona: accepted; SL8's admin half decides them.

**Controller amendment A-SL29 (2026-09-09 ~16:30 WITA; rulings on the round-4 re-review — NOT ALL ADDRESSED: 6/6 findings and 12/12 ruled points closed; new MAJOR-F, MINOR-2, MINOR-3, Info-1–10; closed in fix round 5 — the loop's last round).** (1) **MAJOR-F:** the wizard's Back button saves the step it leaves exactly as A16.18 made the rail do (partial mode, Continue's rejection arm, no PATCH on steps 6/8) — one A16 literal (A16.19), a `logic.test.ts` case (type on 5 → Back → the recorded PATCH is step 5's fields), the flow spec presses Back once with its save asserted on the API; pixel-free (no capture presses Back). (2) **MINOR-2:** `tests/e2e/api_under_test.py` joins BOTH gates its shape belongs to — the CI strict-mypy list and the coverage command (`--cov=tests/e2e`, in CI, CLAUDE.md's gate line and the plan's gate table, all pinned by `tests/test_docs.py`); (Info-8 folded in) its guard also refuses a non-loopback `DATABASE_URL`/`REDIS_URL` exactly as `scripts/reset_rate_limits.py` does, RED first. (3) **MINOR-3:** CLAUDE.md's A16 clause counts nineteen literal edits (A16.1–A16.18 with 11a/b) and, after (1), twenty — write the number the entry count derives and keep it derived. (4) **Info-1/Info-2:** the report's stale citations and the duplicate comment at the spec's line ~210 are corrected. (5) **Info-7:** the Info-J pin reads the design's `w` keys from `new Component({}).state.w` (the module exports `Component`) instead of a regex over the byte-locked literal — no source regex remains on either side. (6) **Record-only, with reasons:** Info-3 (`revBand` API true vs design false — D-SL28, John's), Info-4 (a required number cannot be cleared to null through Save-and-exit: the design's Continue guard already requires a value; partial mode saves what is filled), Info-5 (a NULL enum renders the design's default and the first save persists it — the ruled A-SL27 (1) behaviour), Info-6 (the three rail captures now await the step change; they pass at zero tolerance), Info-9 (a live `PW_APP_URL` run writes drafts through the rail, as the lifecycle test already does — accepted with the leftover-drafts ruling), Info-10 (the plan's duplicated A-SL28 paragraph — removed by this commit). (7) The re-review's three concern assessments are accepted: CI needs no change beyond (2); `revBand` is a declared, doubly-pinned state, not a fiction; no test depends on the persona's leftover drafts.

**Controller amendment A-SL30 (2026-09-09 ~16:50 WITA; adjudication after re-review 5 — the loop's breaker: 12/13 round-4 items and 6/7 A-SL29 points closed; nothing open is load-bearing; every open item is fixed at source in the NEXT dispatch (SL7b), not parked).** Task SL7 is COMPLETE on its own scope (the wizard and dashboard read and write the real API; the seller flow proof and the photograph proof run against the real API with no stub). Carried into SL7b as its FIRST steps, each RED first: (1) **MINOR-4 — no cross-runtime execution in a gate:** `tests/api/test_seller_listings.py`'s `design_initial_w()` and its Node subprocess are deleted; `DESIGN_INITIAL_W` is declared beside the module's existing `DESIGN_W` literal and the by-name assertions stand; the design ↔ `step-fields.json` proof lives ONLY in vitest (`logic.test.ts`: the four enum defaults and the key set), the `step-fields.json` ↔ `columns_for` proof ONLY in pytest — the backend gate needs no Node and the frontend gate needs no Python, pinned by a `tests/test_docs.py` assertion that no test under `tests/` spawns `node`. (2) **MINOR-5 — CLAUDE.md's A12 clause** counts eleven literal script edits (A12.1–A12.11) and the per-family literal-count pin in `tests/test_docs.py` widens to EVERY family named in CLAUDE.md's amendment paragraph (each family's spelled-out count derived from `design-amendments.ts`). (3) **Info-15 — leaving the wizard discards the step being typed (header navigation, Sign out):** one more A16 literal (A16.20): with an adapter present and the wizard open on a step with fields, `go()` (header navigation) saves the current step in partial mode through the existing single rejection arm before it navigates, and a refusal keeps the seller in the wizard with `wizErr` (the rail's rule); `signOut` attempts the same save and then signs out REGARDLESS of the answer (a session end is the seller's explicit act and is never held hostage to a save; the attempt is the most the chrome's "Saved automatically" can honestly offer) — both behaviours characterised in `logic.test.ts`, pixel-free (no capture leaves the wizard). (4) **Record-only:** Info-14 (step 1's Back renders disabled — a disabled control cannot be clicked, so its handler's PATCH is unreachable from the UI; "Back to edit" from step 8 is a GET re-read whose refusal keeps the seller on the preview with the message — the ruled arm), Info-16 (`--cov=tests/e2e` also measures the launcher's own test module — harmless; coverage of a test module is reported, not required), Info-13 (this commit removes the stale paragraph tail).

**Controller amendment A-SL31 (2026-09-09 ~18:55 WITA; ruling on SL7b's concern 1 — the wizard cannot save a seeded hospital, and on concerns 2 and 3).** (1) **The defect is real and it is the original complaint's own shape** ("none of the actual inputs are appearing… the UX must be true"): `OWNERSHIPS` is the design's four-option select vocabulary (`seller_listings.py:126`) while fifteen of the eighteen seeded hospitals carry the DESIGN'S OWN richer prose — "Three-doctor LLC", "Sole proprietor (S-corp)", "Four-doctor partnership" — the same register as the design's own fixture value "Four-doctor LLC" in `logic.js`. So the data is faithful and the vocabulary is narrow, and today a seller who opens Edit on a seeded hospital and presses Continue on step 1 is refused. **The ruling: a value the seller did not change is not re-validated.** `patch_step` compares each incoming enum field against the STORED value and skips the vocabulary check when they are identical (a no-op write); the check applies in full the moment the seller sends something different, so nothing is loosened for new input and no design option is invented. This covers `ownership`, `type`, `facility_type` and `bldg` alike, and is derived from the row, never a per-field allowlist. Tests: a seeded row with "Three-doctor LLC" saves step 1 unchanged (200) and keeps its stored prose; changing it to a value outside the four is still 422 with the envelope code the field already uses; changing it to one of the four succeeds; the same three cases for `type`. The seed data is NOT rewritten and the design's select is NOT widened. **(2) D-SL31 queued for John:** whether the eighteen hospitals' ownership prose should later be normalised to the design's four options (his data, his call) or the select widened by a ruled amendment — until he rules, the seeds keep their words and the rule above keeps the wizard true. (3) **Concern 2 ACCEPTED with its reason recorded:** `scripts/seed_listings.py` joining the Playwright fixture-reseed chain is required by the flow spec's own acceptance criterion, is idempotent, and cannot touch a pixel capture because `prepare()` stubs `/api/listings` for those specs; SL9's whole-branch review re-checks that no other worktree's run is affected. (4) **Concern 3 ACCEPTED as disclosed:** the contaminated intermediate pytest run (a source file edited while its coverage run was in flight) is not the number of record; the clean re-run is. Standing rule for every implementer from here: never edit a tracked file while a coverage run is measuring it — wait, then re-run.

**Controller amendment A-SL32 (2026-09-09 ~19:15 WITA; SL7b's review — spec 5 of 5, quality APPROVED, no Critical or Important, one Minor — and the implementation shape for A-SL31, which lands as SL8's FIRST steps).** (1) **Task SL7b is complete.** Its one Minor is a report-narrative undercount (it says seven recomputed `V3:<line>` citations; the diff shows nineteen rows moved — A10, A10.2, A12.1–A12.11 and A15.1–A15.3d, shifted by `go()`'s +4 lines and, for the two A10 rows, `signOut`'s +3): the numbers in the file are right and machine-verified, so the correction is to the report's prose, made in SL8's first commit. (2) **A-SL31 is implemented as SL8's first steps, RED first, before any Admin › Listings work.** `columns_for` gains the row it is validating so each of its four enum branches skips `_one_of` when the incoming value equals the row's OWN stored value — for `bldg` the comparison is made through `BLDG_IN`/`BLDG_OUT` so the wizard's word and the column's word are compared like for like — and every other validation is untouched. (3) **The flow spec stops dodging the defect.** The review found that `listing-flows.spec.ts` targets `abc_animal_hospital`, one of only three seeded hospitals whose ownership happens to satisfy the four-option select, while fifteen do not: the spec must edit a hospital whose stored ownership is OUTSIDE the vocabulary (a "Three-doctor LLC" row), and pytest gains the three cases A-SL31 named (unchanged saves 200 and keeps the stored prose; a different out-of-vocabulary value is still refused; a different in-vocabulary value succeeds) plus the same three for `type`. A test that steers around the broken case is not coverage — it is the defect's hiding place, and this is the second time on this branch that a fixture chosen for convenience concealed a real failure. (4) **Recorded, not changed:** the local default `DATABASE_URL`/`REDIS_URL` shared across worktrees unless a run overrides them is a pre-existing hazard this task neither created nor worsened (the seeder is idempotent and advisory-lock serialised); it stays on SL9's hardening list. (5) The reviewer's three judgements are accepted as written: `signOut`'s save is deliberately swallowed and the seller is signed out unconditionally, exactly as A-SL30 (3) ruled; header navigation surfaces its refusal and blocks; neither chain can double-navigate.
...
**Controller amendment A-SL33 (2026-09-09 ~20:45 WITA; rulings on the SL8 review — spec 6 of 7, quality NOT APPROVED: 1 Critical, 1 Important, 3 Minor, all closed in fix round 1).** (1) **CRITICAL — a published listing with no floor area crashes Browse's own rendering, and the third commit stepped around it instead of fixing it.** `logic.js` calls `p.sqft.toLocaleString()` unconditionally at six sites; no CHECK, no submit precondition and no admin decide branch requires `sqft`, so a listing can legally be published without it and take the buyer's page down. The plan's own line 260 names that very expression as the reason the publish CHECKs exist, and `sqft` was never in the list. **Fixed at source, in the database and at the boundary:** a new migration in the seller carve-out (`034`, the next free number — `030`–`033` are spent) extends `listing_publishable_ck` to require `sqft` alongside `state`, `market` and `area`, with the same comment style saying why; the migration first reports and refuses on any existing published row that violates it, rather than silently dropping the constraint (there must be none — prove it in the report); `submit`'s completeness check and the admin decide's first-publish branch answer the same requirement in the envelope, so a seller is told which field is missing rather than meeting a database error. (2) **The rule behind it — enumerate, do not assert.** Before the fix, list EVERY expression in the byte-locked design that dereferences a listing field without a guard (`.toLocaleString`, `.toFixed`, arithmetic, `.length`, property access on an object field), and for each say which CHECK, which serialiser default or which design guard makes it safe. The report carries that table. `sqft` is the one this review found; the table is what stops the next one. The reviewer's note that `facility_type` shares `ownership`'s exposure without its end-to-end coverage belongs in the same table with its answer. (3) **IMPORTANT — "Edit" and "Contact seller" are restored.** The brief says the actions are the design's own buttons, wired to the decide endpoint where a decision exists and left as the design's no-op where it does not; dropping two of them is a change to what the approved table renders and no amendment authorises it. Restore both as no-ops in the design's own shape, and pin the action set per status against `adminVals()`'s own rows so a future edit cannot quietly drop a button. (4) **MINORS, all at source:** the wrong file:line in the third commit's message is corrected in the report (the commit itself is not rewritten — it is not pushed, but rewriting history to fix prose is a worse trade); the "matches `admin/users.ts`" rationale is restated honestly (that module has never been wired to a live component, so it is not a precedent — the absence of a refresh after a decision is a deliberate gap, recorded for Rev 3, not a match to anything); and `facility_type` gains the end-to-end coverage `ownership` has. (5) The reviewer's other adjudications are accepted: the four test fixes and the per-run-unique city are legitimate test work, the shared-database repair follows the earlier ruling, and D-SL31 stays open for John.

**Controller amendment A-SL34 (2026-09-09 ~22:10 WITA; SL8 closed, and everything the branch has carried lands in SL9).** SL8's re-review: all eight ruled items and all four structural checks ADDRESSED, nothing open, one new Minor — the enumeration table omits `logic.js`'s `p.price / 1000` site and undercounts its own headline by one, cosmetic because that site is doubly safe (null arithmetic coerces, and `listing_submittable_ck` already guarantees a price). The flake was a genuine diagnosis, confirmed mechanically against the `014` precedent, not a retry dressed up as analysis. **SL9 therefore opens with the branch's whole carried list, each RED first:** (1) the enumeration table corrected and its headline recomputed; (2) **the hardening queue this branch has accumulated** — `patch_step` takes the row lock its precondition needs, exactly as `locked_row` does, so a concurrent PATCH cannot write a duplicate (SL5 re-review (a)); a test that isolates `claim_from_seed`'s `source = 'seed'` predicate by planting a `source = 'seller'` row and proving a write leaves it untouched (SL6 re-review (c)); and the spec amendment recording what the photographs, the nulls and the captions actually became (A-SL20's ruling superseded the four-photo cap and the specification still reads the old way). (3) **The merge is the task's real weight, not its documentation.** `main` is now 0.1.10 and 71 commits ahead: it carries families A18 and A19 from the lightbox branch, three new approved states, one re-based frozen screen, a re-pinned manifest and a widened per-family count pin, while this branch carries A16 and A17. Every family coexists in id order, every `find` must still match exactly once, the citations recompute, the counts derive, and `screens.ts` keeps main's states before this branch's. If a `find` collides or a frozen hash moves that this branch did not move, STOP — that is the leak detector doing its job. (4) **The version is the controller's at release**, and this branch is next in merge order after 0.1.10, so it becomes 0.1.11. (5) **D-SL31 stays open for John** (the seeded hospitals' ownership prose against the design's four-option select) and is named in the hand-back rather than decided. (6) The QA click-through list SL9 writes is for John, who does not run the app locally: the wizard end to end on a seeded hospital, a photograph re-described, a submission, the reviewer's decision, and the dashboard reflecting it.

**Controller amendment A-SL35 (2026-09-10 ~00:20 WITA; the whole-branch review — READY TO MERGE, no Critical — and the one Important fixed before the release).** The reviewer re-derived every total independently (nineteen families, 144 entries, 121 documentation rows, 49 approved states, all matching), proved the frozen-oracle claim by walking both sides of the merge (eleven of thirteen hashes differ from the fork point, and the union of two already-ruled commits on main accounts for exactly those eleven, so none of the movement is this branch's), and sampled eleven tests for honesty — including a concurrency test that creates real contention and an isolation test that reads the row version — finding none that passes for the wrong reason. **The Important, fixed before the merge:** the release merge silently dropped CLAUDE.md's prose paragraphs for the metro-dropdown and Give-button families while their machine-readable entries and every count stayed correct, and later paragraphs still cite those very families. No test noticed, because the per-family test only checks the markers that ARE present and the aggregate test only matches the summary sentence. **So the fix is both halves:** restore the two paragraphs from main's own text, and add the assertion that every family in the machine-readable ledger has a prose section in CLAUDE.md, so a future merge cannot drop one silently. A documentation ledger that can lose an entry without failing is not a ledger. **The three Minors stand as disclosed:** the early Playwright call that touched the shared development database (idempotent throughout, a pre-existing hazard this branch did not create), the commit-message typo left rather than rewritten, and migration 034's one-time abort path being verified operationally rather than by a gate. **D-SL31 remains John's**, named in the hand-back rather than decided.
...
**Controller amendment A-SL39 (2026-09-10 ~18:15 WITA; a Critical found while proving Task B10's panel fix, and the ruling that closes it — Task MP1, amendment family A25).** A published listing whose seller has not disclosed its location has no coordinates, and the Browse map throws when asked to place it. `migrations/016_listing.sql` declares `location_disclosed boolean NOT NULL DEFAULT false`, so **this is the default state of a new listing, not an edge case**; `app/api/listings.py`'s `serialise` correctly nulls `lat`/`lng` for it, and everything downstream carried the nulls to Leaflet. `toLatLng([null, null])` returns `null` — its array branch is guarded by `typeof a[0] !== "object"` and `typeof null === "object"` — and `_setPos` then reads `.lat` off that null. Measured in real Chromium: `Cannot read properties of null (reading 'lat')`. **One listing is enough**: the pin loop has no `try`, so every listing after it goes undrawn, and `LayerGroup.addLayer` has already stored the poisoned marker, which re-throws from inside Leaflet's own event loop on every later zoom. Under the Vite dev server the Playwright suite uses, Vue's development handler re-throws out of `flushJobs` and the whole screen stops responding; the production build logs instead, so QA and production lose pins and zoom rather than freezing. Not reachable on QA today — all seeds disclose their locations — but `frontend/tests/listing-flows.spec.ts` already publishes exactly such a listing and leaves it in the test database, so the landmine was armed in the suite and hidden only by a stub. **The ruling: a listing with no coordinates keeps its place in the results and does not get a pin.** Not drawn at `[0, 0]`, not at the metro centre, not at all — the approved design has no treatment for "somewhere in this metro" and inventing one is out of scope. It stays in the results list, the count, the sort and the filters, exactly as before; `driveCenter` falls back to the metro centre, which is that expression's own existing else-branch; and NO new copy, badge, banner or empty state is added. **A third leg the ruling did not name was found and closed in the same task:** `mosaicBbox` takes `Math.min`/`Math.max` over the map's community coordinates, where `null` coerces to `0` — measured, one unlocated Austin listing stretched the bounding box to **100,482,513 mosaic cells**, a hang rather than a throw. Two further latent copies are recorded and deliberately untouched under the bundle's dead-code rule: `marketVals`'s computed-but-unread `minLat/maxLat/minLng/maxLng`, and `mob.markers` (V3:3665), a fourth copy of the same hazard that no template consumes. **Open for John, recorded and NOT built:** should a buyer be told that a listing's location is undisclosed, and how? Today it keeps its place everywhere and simply has no pin.
