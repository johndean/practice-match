...
- Produces, `app/privacy/identity.py`:
  ...
  - `REGEX_CLASSES: dict[str, re.Pattern[str]]` — `phone`, `url`, `email`, `address`, `handle`, `zip`.
...
def test_a_regex_class_is_an_identifying_region_whether_or_not_it_matches_the_listing(
    ...
    that is not this practice's is still a phone number in a photograph of it."""
...
REGEX_CLASSES: dict[str, re.Pattern[str]] = {
    ...
    "address": re.compile(r"\b\d+\s+[\w'-]+(\s+[\w'-]+)*\s+(st|street|ave|avenue|rd|road|blvd|dr|drive|ln|lane|pkwy|parkway|hwy|highway|suite|ste)\b\.?", re.I),
    "handle": re.compile(r"(?<!\w)@\w{3,}"),
    "zip": re.compile(r"(?<!\d)\d{5}(?!\d)"),
}
#: A regex class that names a suite number with no street number in front of it.
REGEX_CLASSES["address"] = re.compile(
...
PROMPT = (
    ...
    " business cards, building directories and any other lettering. Give each one a bounding box in"
    " the image's own pixel coordinates. Do not guess: report only what is visible."
)
