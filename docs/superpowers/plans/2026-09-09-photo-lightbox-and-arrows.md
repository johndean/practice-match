...
**Controller amendment A-LB2 (2026-09-09 ~19:00 WITA; rulings on the L2 review — spec 10 of 11 met with the eleventh unverifiable from a diff, quality APPROVED, no Critical; one Important and three Minor, every one fixed at source in fix round 1).** (1) **Important — the flake is a real race, so it is fixed, not re-run away.** `atTop()` in `frontend/tests/harness.ts` pins the page scroll but not the docked panel's own internal `.rf-scroll` container, which Playwright's click-actionability can scroll under layout jitter; the pre-screenshot stabilisation gains the same treatment for that container (set its `scrollTop` to 0 and assert it, in `atTop`'s own shape so every state benefits, not just `browse-panel-lightbox`). A capture that still moves after this is a real regression, which is the point. (2) **Minor — the A19.2 and A19.10 rows in `LOCAL_AMENDMENTS.md`** gain the note Step 10 asked for: focus is deferred through a zero-delay `setTimeout` because live Chromium drops a synchronous `focus()` issued inside a capture-phase `focusout`, and the focus still lands — the rows are incomplete today, not wrong. (3) **Minor — the live-browser Tab-cycling confirmation becomes a GATE rather than a person's word:** an e2e assertion in the lightbox's own spec that Tab from the last control returns to the first and Shift+Tab from the first returns to the last, with focus never leaving the dialog. An unverifiable manual step is not evidence; a test is. (4) **Minor — the review package spanning two commits** is the controller's packaging (the plan commit rode along), not a defect in the work: recorded, nothing to change. (5) The reviewer's own independent re-derivation — 118 amendments applied byte for byte, `logic.js`, `App.vue` and `pseudo.css` reproduced, 48 citations valid, 95 rows matching, the manifest diff empty — stands as this task's structural proof.

**Controller amendment A-LB3 (2026-09-09 ~19:40 WITA; ruling on fix round 1's NEEDS_CONTEXT — the focus trap does not hold, and the implementer was right to stop).** The round closed the flake at source (`atTop()` now pins every `.rf-scroll` container, with a guard test whose RED was watched) and the two amendment rows' notes. Building the third finding's gate, it walked real Chromium and found A19.10's trap fails reproducibly: forward Tab off "Next photo" lands on `<body>` and then a real header button before self-correcting, and Shift+Tab from "Close photo" reaches the dialog container and never wraps. It refused to write an assertion against the broken behaviour — correctly: a test written to match a known defect is the defect's permanent alibi. **The ruling.** (1) **The mechanism changes: the `focusout` trap is REMOVED and Tab is handled where it happens, in the shared `keydown` closure A19.9 already owns.** The root cause is structural, not a tuning error — a null `relatedTarget` means both "the window blurred" (which must be ignored) and "focus left the trap's own tabbable set" (which must not), so the arm cannot tell its two cases apart and no threshold makes it. In its place: on `Tab` with the dialog open, the handler reads the dialog's three controls in DOM order (Close photo, Previous photo, Next photo — the container is `tabindex="-1"` and never in the cycle), and when focus is on the last it moves to the first and calls `preventDefault()`, when Shift+Tab and focus is on the first (or on the container itself) it moves to the last and calls `preventDefault()`. Deterministic, one code path, no special case. (2) **`aria-modal="true"` stays and becomes true.** The design's own interest modal has no trap and makes no such claim; the lightbox makes the claim, so it owes the behaviour — the alternative, dropping both, would leave assistive technology told the page behind is inert while Tab walks into it. (3) **The disproven prose goes at source:** A19.10's comment in `frontend/tests/design-amendments.ts` still says focus never reaches the page behind; that sentence is now false and is replaced by what the code does, alongside the row note the round already corrected. (4) **The gate the round could not write is now writable and required:** the e2e assertion that Tab from the last control returns to the first, Shift+Tab from the first returns to the last, and focus never leaves the dialog across a full cycle in both directions — and it must pass without a retry. (5) The amendment literal changes, so the design file is regenerated, `logic.js` re-ported, `App.vue` regenerated and the citations recomputed; the manifest must still show ZERO moved hashes, because a closed lightbox renders none of this.
...
**Controller record — branch complete (2026-09-09, Task L3).** Commits, in order: `61786f5`+`833ddb1` (plan committed to `main`); Task L1 — `052d16b` (A18, the two arrow reversals) + `5155d98` (the one frozen re-pin, `detail`: `21cb1d71`→`c641b766`, the other twelve unchanged); L1 review ruling `a6795ee` (A-LB1 — worktree Step 0: `npm ci` in `coming-soon/` too, reuse the shared compose stack); Task L2 — `d2c710c` (A19, twelve entries, three new states, manifest zero moves); L2 review ruling `2e9f46b` (A-LB2); fix round 1 `6eb3a8c`; fix round 1 ruling `5af78a3` (A-LB3 — the `focusout` trap removed, Tab handled in the shared `keydown` closure); fix round 2 `c51acff`; L2 re-review ALL ADDRESSED, one carried Minor (A19.2's JSDoc, present tense) deferred to L3; Task L3 — `dfbc59f` (the JSDoc reworded to past tense, outside any pinned `find`/`replace`), `6def326` (merge `main`, docs-only — the image-identifiability spec and plan, zero code conflict) and this commit (the closing record). Final counts: **seventeen families, 118 entries, 48 approved states**; the frozen manifest diff against `5155d98` is empty (zero moved hashes) both after L2 and again here. Final gate on the merged tree: backend `poetry run ruff` clean (53 files), `mypy --strict` clean, `pytest` 1209 passed / 100% branch coverage (512.9s); frontend `typecheck` clean, `build` clean, `vitest run --coverage` 883 passed across 44 files at 100% lines/branches/functions/statements; `test:visual:baselines` 98 passed (3.5m) then `test:e2e` (the `app` project: visual + DOM oracle + smoke) 144 passed (3.3m), no retry; `baseline-manifest.test.ts` green; `tests/test_docs.py` 80 passed. DEPLOY.md needed no edit (verified by grep, per the brief). Owed at hand-back, not this branch's to do: the version bump (lockstep, merge-order-dependent — 0.1.10 if this lands before the seller branch), the QA click-through on `qa.foundation.vin` after `scripts/deploy.sh QA .worktrees/feat-photo-lightbox`, the production deploy and its smoke, and John's ruling on D-A18.1/D-A18.2 (the two other arrows, left unchanged by default) and on rulings 1–10 above (the lightbox's composition, shipped as the controller's defaults).
