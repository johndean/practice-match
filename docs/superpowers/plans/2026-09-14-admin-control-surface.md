...
| 10.4 | **The pets 0.57 factor is a methodology note**, not a `dataset_registry` row. *(Not this plan's work; recorded so nobody adds a row here.)* **Superseded in part, 2026-09-15**, by Task PET-RATE-PROVENANCE: the factor is now the AVMA 2025 Sourcebook's 58.6 % with a named edition, a reference period and a `SOURCE VERIFIED / LICENCE-REDISTRIBUTION UNRESOLVED` licence position, so this ruling's own condition ("a licence and a vintage it does not have") no longer holds. `migrations/099` adds `avma_pet_rate` for the cited statistic and leaves `pet_ownership` blocked for the licensed feed; the methodology note ships too, in `app.api.market.PETS_CAVEAT`. See the spec's §10 question 4 for the full record. |
