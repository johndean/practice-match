...
| 10.4 | ~~**The pets 0.57 factor is a methodology note**, not a `dataset_registry` row.~~ **Superseded in its premise 2026-09-16 (A50)** — the factor is now sourced to the AVMA 2025 Sourcebook, so it HAS an edition, a reference period and a licence question, which is exactly what the ruling said it lacked. A `dataset_registry` row records them; the methodology note ships as well. *(Still not this plan's work.)* |
...
| **D4** | **The Settings tab carries NO badge** (`count: null`, so A36.3/A39.3a's `hasCount` `sc-if` unmounts the lozenge). Settings is not a queue. |
...
| **D13** | **A tab badge counted CLIENT-SIDE is ratified for the Data Sources tab and for NO other** (A38 review M5, carried here 2026-09-15 because the sibling task briefs it was raised against are no longer live documents and this plan is). `GET /api/admin/data-sources` answers the WHOLE registry unpaginated, so counting the rows the tab is already rendering is EXACT. Users, Listings and Requests are keyset-paged and cannot meet that condition: their badges are a `counts` object served BESIDE `items` (`admin_signups.COUNTS_SQL`'s pattern), which is what the Standing Rules sentence means. A41's Settings tab has no badge at all (D4), so the question does not arise there. |
...
**The family is FOUR entries — A41.1 (the tab row), A41.2 (the fifth `sets` entry), A41.3 (the state key), A41.4 (the `loadAdmin` line) — and no template edit at all.** A41.1 filters the tab array in the SCRIPT, in the design's own `.map()` chain, so the template is untouched: a second `sc-if` nested inside the tab `sc-for` would have been new template structure, and `hasCount`'s own `sc-if` (A36.3/A39.3a) stays exactly as those families wrote it.
...
// A36.3/A39.3a's `hasCount` sc-if unmounts the lozenge for a null. `shown` is the gate that
