...
| `app/disclosure/requests.py` (new) | The request lifecycle: create, list-mine, list-inbox, decide (approve/deny), revoke. All writes to the `request` table and the audit log live here; routes are thin wrappers around this module. |
...
This task does **not** touch `frontend/tests/design-amendments.ts`, add a `LOCAL_AMENDMENTS.md` row, or re-base any approved state — it is pure adapter wiring in the A16.1/A17.1 idiom (an adapter's presence changes what `App.vue`'s existing ternaries read; it adds no template, no CSS, no new element). Confirm this remains true at Step 4.
...
`logic.js`'s own `componentDidMount` (A16.1's own idiom) then reads `this.props.requests`/`this.props.sellerRequests` presence to decide whether to call `loadRequests()`/`reloadInbox()` (mirroring A16.17's `reloadListings`) instead of using the static `s.requests` fixture array — this is a SCRIPT-ONLY change inside `logic.js`, reached by no template, so it is not a `LOCAL_AMENDMENTS.md` entry any more than A16.1's own ternary is one (CLAUDE.md: "A40.3-A40.6 are SCRIPT-ONLY and paint nothing"). Map the served `status` values onto the design's own three: `APPROVED → "accepted"`, `DENIED → "declined"`, `REVOKED → "declined"` (the documented, low-risk default from "What needs a design ruling" above), `PENDING → "pending"`.
