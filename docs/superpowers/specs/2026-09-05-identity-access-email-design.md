...
**Automation tokens.** `api_token` created by an admin for a named purpose (`k6-qa`, `e2e-qa`, `deploy-verify`), one role, hashed at rest, ≤ 90 days, revocable; `Authorization: Bearer pm_<id>.<secret>`. They replace `API_SECRET_KEY`/`auth_stub.py`, which are deleted once CI secrets are switched (one-release overlap in which `require(perm)` accepts either).
