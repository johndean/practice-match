...
**RED → GREEN.** `visual.spec.ts` fails before the port is wired (no app / no match). The port is complete when every state passes. Linux baselines are generated by the CI `visual-baselines` dispatch job and committed (Rounds pattern); darwin baselines are generated locally for the development loop. CI enforces the suite on every push.
...
**`.github/workflows/quality.yml`** (mirrors Rounds): `gitleaks` (binary install, `.gitleaks.toml`) · `backend` (services `postgis/postgis:16-3.5`, `redis:7-alpine`; Poetry install; `python scripts/migrate.py`; `pytest -v`) · `frontend` (Node 22; `npm ci`; `npm run typecheck`; `npm run build`; Playwright chromium; `test:smoke`; `test:visual`; upload `playwright-report` on failure) · `visual-baselines` (`workflow_dispatch` with `update_baselines=true`; runs `test:visual:baselines` against the reference; uploads the snapshot directory as an artifact to commit). The workflow lives in the repo and therefore runs in both remotes; enabling Actions on the vin-swe internal repo is a John action if it is off.
...
- [ ] `npm run test:visual` passes against committed Linux baselines generated from the reference; tolerance recorded.
