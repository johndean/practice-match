...
qa.foundation.vin stops showing the design's nine anonymised Austin practices and shows **eighteen seeded demo hospitals** across Texas, California, New York, Colorado, Florida and Georgia, each at a real, mapable street address with a fake `555` phone number, opening hours and photographs John supplied. Browse Practices (V3), the map pins, the results rail and the detail screen read them from the API. Production is unaffected (Coming Soon).
