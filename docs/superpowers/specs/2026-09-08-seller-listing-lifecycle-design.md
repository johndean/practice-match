...
> **D18 amended (SL9, A-SL34; ruling A-SL20, John, 2026-09-09: "surface all images uploaded … render ALL images").** The cap above **no longer holds** — this paragraph is left standing for the history, not the behaviour. What shipped instead, in `app/api/seller_listings.py`:
>
> - **No cap.** `upload_photo` stores every photograph a seller sends; the design's six tile slots are what the wizard can render with one of its own fixed captions, not a limit on how many a listing may hold. `409 PHOTO_LIMIT` and `MAX_PHOTOS` are gone from the write path; a hospital with nine photographs shows nine.
> - **Two homes for a caption, one contract over both (`app/api/listings.py::photo_captions`).** A SEED row's photographs are paths in `listing.photos`, described positionally by the parallel `listing.photo_captions` column (`migrations/090_listing_photo_captions.sql`) — written once by the seeder from the supplier's own filenames (A-L11), and re-writable one photograph at a time by `PATCH /api/seller/listings/{id}/photos/{n}` (SL7b, A-SL25 (10)), the click-to-caption route a seed entry needs because it names no `listing_asset` row for the ordinary `PATCH .../assets/{id}` caption route to reach. A SELLER-uploaded photograph is an asset uuid in `listing.photos`, described by `listing_asset.caption`, set at upload and re-writable through that same asset route. `photo_captions()` (`app/api/listings.py:193`) returns exactly one string per photograph on both paths — the seed column padded with `""` or truncated to `len(photos)` if it is ever short or long, a missing or explicit `null` at a position becoming `""` either way — so the buyer-facing `serialise` and the seller's own `photo_tiles` (`app/api/seller_listings.py:335`) never have to ask which home a given position's caption came from.
> - **A `null` entry in `listing.photos` is an empty seed slot (A-L10), not a missing photograph — and it is never dropped.** The seeder writes `null` at a position when a hospital's own photo folder was too thin to fill the design's six-slot inventory; the position is still spent (the next real photograph keeps ITS OWN index into `photo_captions`), so `photo_tiles` skips a `null` position when building step 6's tiles but the list is never compacted — compacting would slide every later photograph's caption out of step with it. Thirty-five of the 195 seeded slots are `null` this way (SL7 review).
> - **A caption's fallback chain**, for a photograph the seller has never touched: the seller's OWN positional recaption (`listing.photo_captions[n]`) first, then the seed inventory's own caption (`seeds/hospitals/photos/index.json`, read once per process by `seed_captions()`), then an empty string — never the raw filename (superseding this task's own original D26 wording, A-SL22 (2)): a filename like `DSC_0431.jpg` describes nothing a buyer can use, and the design already has a truthful name for an undescribed photograph in its own fixed per-slot caption, which the wizard fills in past the sixth photograph as "Photo N".
>
> Tests: `tests/api/test_listing_assets.py`'s upload suite (no fifth-photo refusal case remains — a ninth upload is asserted to succeed), `photo_captions()`'s four pins (longer and shorter than `photos`, an owned position, an explicit interior `null`), and `photo_tiles`'s own tests for a seed entry, an asset entry, and a `null`-skipped position that still advances the caption index.
