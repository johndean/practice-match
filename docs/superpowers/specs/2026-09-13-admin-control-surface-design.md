...
6. A refused load and an empty table look the same on the Data Sources tab (A38 review, M7). The rejection arm empties the rows AND the badge, by A17.1's rule — a reviewer of the legal gate is never shown five datasets the platform does not hold — so "nothing is outstanding" and "I could not read the registry" render identically, and the absent badge is the only tell, absent during loading too. V3 has NO element for a load that failed, on this tab or on Listings, so nothing was invented in A38. — **Recommendation: one shared failure line for every admin tab**, composed from the design's own footnote type (12 px, `--color-steel`, the same paragraph the Data Sources footnote already uses), rendered in place of the table on a refusal with a retry; it is one amendment family for all four tabs, not four. Same shape as A17.1's Listings tab, so it closes a pattern rather than a one-off.
