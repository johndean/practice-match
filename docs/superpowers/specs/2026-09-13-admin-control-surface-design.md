...
4. **The pets 0.57 factor: a registry row or a methodology note?** — **A METHODOLOGY NOTE** in the layer's own caveat. It is a modelled factor, not a dataset; a `dataset_registry` row would imply a licence and a vintage it does not have.
   ...
   **Superseded in part on 2026-09-15 by Task PET-RATE-PROVENANCE (John's requirement of that day, its controller ruling 4), and the premise is what changed rather than the judgement.** This ruling was right about `0.57`, which had no publisher, no edition and no licence position to state. The factor that replaces it has all three — AVMA, 2025 *Pet Ownership and Demographics Sourcebook*, reference period 2025, `SOURCE VERIFIED / LICENCE-REDISTRIBUTION UNRESOLVED` — so "a licence and a vintage it does not have" no longer describes it, and John's own requirement asks for that status "recorded on the `dataset_registry` row and visible on the admin Data Sources tab". `migrations/099_avma_pet_rate_registry.sql` adds `avma_pet_rate` (`unresolved`) for the cited STATISTIC and leaves `pet_ownership` (`blocked`) for the per-geography licensed FEED, which is the separation this ruling's single row could not make. **The METHODOLOGY NOTE this ruling asked for is also shipped and is not replaced**: `app.api.market.PETS_CAVEAT` carries it on the layer, composed from the same provenance record. Reversible in one line if the controller reads it otherwise; recorded here so it is not found as an unexplained row.
