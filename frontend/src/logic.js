...
class Component extends DCLogic {
  state = {
    ...
    w: { name: "", type: "Small animal", est: "", street: "", city: "", zip: "", phone: "", anon: true, price: "", rev: "", revBand: false, docs: "", rooms: "", sqft: "", bldg: "Included", facility: "", desc: "", photos: 0, ownership: "Sole proprietor", hours: "", facilityType: "Standalone", docsLocked: true, showIdentifiable: false },
  ...
  // A31 (D-C50 as revised): the metro-wide summary the snapshot strip's AREA mode reads.
  ...
  setMarket = (e) => {
    const v = e && e.target ? e.target.value : e;
    // A31: the summary is METRO-wide and carries no bbox, so a metro change asks for it at
  ...
  // A31 (D-C50 as revised): the design's own metro summary - one row per shaded layer,
    ...
    return {
      ...
      stripCards: (() => {
        ...
        .map((k) => {
          ...
          // A31.13 (fix round 1): the bucket ITSELF, not a re-derived index. `income`'s ramp
          ...
          return {
            ...
            // A31.12 (fix round 1): the caption over a figure is THAT FIGURE's own
  ...
  filtered() {
    const f = this.state.f;
    const market = this.state.market || "Austin, TX";
    const asked = [["doctors", "docs"], ["price", "price"], ["est", "est"], ["sqft", "sqft"], ["revenue", "rev"], ["ownership", "ownership"]];
    return P.filter((p) => {
  ...
  openDraft(id, d, err) {
    this.setState({ sellerView: "wizard", step: 1, wizSubmitted: false, creating: false, wizErr: err || "", editingId: id, wizAssets: (d && d.assets) || [], w: Object.assign({ name: "", type: "Small animal", est: "", street: "", city: "", zip: "", phone: "", anon: true, price: "", rev: "", revBand: false, docs: "", rooms: "", sqft: "", bldg: "Included", facility: "", desc: "", photos: 0, ownership: "Sole proprietor", hours: "", facilityType: "Standalone", docsLocked: true, showIdentifiable: false }, (d && d.w) || {}) });
  }
  ...
  sellerVals() {
    ...
    return {
      ...
      inbox: inbox.map((r) => {
        ...
        const label = r.status === "pending" ? "New" : r.status === "accepted" ? "Engaged" : r.status === "revoked" ? "Withdrawn" : "Declined";
        ...
        return {
          ...
          resolvedNote: r.status === "accepted" ? (r.grantedLabel || "You released the financial packet and floor plan to this buyer.") : r.status === "revoked" ? "You withdrew this buyer's access. The buyer no longer sees the financial packet or floor plan." : "You declined this request. The buyer was told you are not engaging further.",
          ...
          revoke: () => (this.props.sellerRequests ? this.props.sellerRequests.revoke(r.id).then(() => this.reloadInbox(), () => this.reloadInbox()) : this.setState((st) => ({ requests: st.requests.map((x) => (x.id === r.id ? Object.assign({}, x, { status: "revoked", reply: "Access withdrawn. The financial packet and floor plan are locked again." }) : x)) }))),
          accept: () => (this.props.sellerRequests ? this.props.sellerRequests.chooseAccess(r.buyer, r.granted || [], "Share more").then((set) => (set === null ? null : this.props.sellerRequests.decide(r.id, "approve", set))).then((d) => (d ? this.reloadInbox() : null), () => this.reloadInbox()) : this.setState((st) => ({ requests: st.requests.map((x) => (x.id === r.id ? Object.assign({}, x, { status: "accepted", reply: "Happy to share more. Financial packet unlocked." }) : x)) }))),
  ...
  wizardVals() {
    ...
    const byStep = {
      ...
      2: { blurb: "Buyers search by location. You choose how precisely yours is shown.", fields: [text("street", "Street address", "1204 Cypress Creek Rd", "Never shown to buyers until you approve a request."), text("city", "City or community", "Cedar Park"), text("zip", "ZIP code", "78613", "Used to place your practice on the map and to attach community data."), text("phone", "Practice telephone", "(512) 555-0100", "Never shown to buyers until you approve a request.")], toggles: [{ key: "anon", label: "Show only the community, not the street address", help: "Recommended while you are still operating. Buyers see \u201CCedar Park, TX\u201D and an approximate map pin." }] },
      ...
      7: { blurb: "You decide what an approved buyer sees before you have spoken to them.", fields: [], toggles: [
        ...
        { key: "showIdentifiable", label: "Identifiable image content", help: w.showIdentifiable ? "Shown to buyers — signage, logos and names may be visible" : "Not shown to buyers — recommended" }
      ] }
    };
    ...
    const tileName = (a, i) => a.name || (slots[i] ? slots[i].caption : "Photo " + (i + 1));
    const uploads = this.props.listings || (s.wizAssets && s.wizAssets.length)
      ? (s.wizAssets || []).map((a, i) => ({ kind: a.kind, name: tileName(a, i), src: a.src || null, hasSrc: !!a.src, noSrc: !a.src, state: a.state || "processing", pill: ({ processing: "Processing…", review: "Review", confirmed: "Confirmed", failed: "Failed" })[a.state || "processing"], open: this.openPhotoReview && this.openPhotoReview(a.id), cover: a.kind === "Photo" && i === 0, canRemove: a.kind !== "Photo" || ownPhotos, canCover: a.kind === "Photo" && ownPhotos && i > 0, remove: (a.kind === "Photo" && !ownPhotos) || !s.editingId ? null : () => this.props.listings.confirmRemove(a.kind, tileName(a, i)).then((yes) => (yes ? this.props.listings.remove(s.editingId, a.id) : null)).then((d) => (d ? this.setState({ wizAssets: d.assets, wizErr: "" }) : null), (e) => this.setState({ wizErr: (e && e.message) || "That could not be removed." })), makeCover: a.kind !== "Photo" || !ownPhotos || i === 0 || !s.editingId ? null : () => this.props.listings.reorder(s.editingId, [a.id].concat(photoIds.filter((x) => x !== a.id))).then((d) => this.setState({ wizAssets: d.assets, wizErr: "" }), (e) => this.setState({ wizErr: (e && e.message) || "That could not be saved." })), describe: a.kind !== "Photo" || !s.editingId ? null : () => this.props.listings.describe().then((t) => (t === null ? null : (a.source === "asset" ? this.props.listings.caption(s.editingId, a.id, t) : this.props.listings.describe(s.editingId, a.position, t)))).then((d) => (d ? this.setState({ wizAssets: d.assets, wizErr: "" }) : null), (e) => this.setState({ wizErr: (e && e.message) || "That could not be saved." })) }))
    ...
    return {
      ...
      toggles: cfg.toggles.map((t) => ({ key: t.key, label: t.label, help: t.help, on: !!w[t.key], toggle: this.setW(t.key) })),
      hasUpload: step === 6,
      uploads,
      addPhoto: () => {
  ...
  detail() {
    ...
    const bldg = p.bldg === "Included" ? "Included in sale" : p.bldg === "Separate" ? "Available separately" : p.bldg === "Leased" ? "Leased — assignable" : null;
    const docIcon = (open) =>
    ...
    return {
      ...
      priceLabel: this.money(p.price),
      priceNote: "Practice only." + (bldg ? " " + bldg.toLowerCase() + "." : ""),
      ...
      canRequest: !sent,
      alreadySent: sent,
      sentLabel: unlocked ? "Seller accepted your request" : req && req.status === "revoked" ? "Seller withdrew your access" : req && req.status === "declined" ? "Seller declined this request" : "Request sent — awaiting the seller",
      ...
      sections: [
        ...
        },
        {
          ...
          rows: (bldg ? [{ k: "Building status", v: bldg }] : []).concat(p.facilityType ? [{ k: "Facility type", v: p.facilityType }] : []).concat([
    ...
    return {
      ...
      signIn: () => {
        ...
        return this.props.auth.signIn(s.email, s.pw).then(
          (me) => { this.setState({ screen: "browse", formError: "", auth: true, email: me.email, me: { name: me.name, role: me.role, initials: me.initials } }); return this.loadAdmin(); },
      ...
      reqList: this.myReqs().filter((r) => P.some((x) => x.id === r.pid)).map((r) => {
        const p = P.filter((x) => x.id === r.pid)[0];
        const label = r.status === "pending" ? "Awaiting seller" : r.status === "accepted" ? "Seller engaged" : r.status === "revoked" ? "Access withdrawn" : "Declined";
        const tone = r.status === "pending" ? ["#003a70", "#deecf7", "#deecf7"] : r.status === "accepted" ? ["#ffffff", "#003a70", "#003a70"] : ["#494949", "#ffffff", "#494949"];
        return {
          title: p.type + " practice — " + p.area,
          ...
          hint: r.status === "pending" ? "The seller has not responded yet. Nothing further is disclosed until they do." :
                ...
                r.status === "revoked" ? "The seller withdrew your access. The financial packet and floor plan are locked again." :
