...
class Component extends DCLogic {
  ...
  sellerVals() {
    ...
    return {
      ...
      inbox: inbox.map((r) => {
        ...
        const label = r.status === "pending" ? "New" : r.status === "accepted" ? "Engaged" : r.status === "revoked" ? "Revoked" : "Declined";
        ...
        return {
          ...
          resolvedNote: r.status === "accepted" ? "You released the financial packet and floor plan to this buyer." : r.status === "revoked" ? "You revoked this buyer's access. The buyer no longer sees the financial packet or floor plan." : "You declined this request. The buyer was told you are not engaging further.",
  ...
  detail() {
    ...
    return {
      ...
      canRequest: !sent,
      alreadySent: sent,
      sentLabel: unlocked ? "Seller accepted your request" : req && req.status === "revoked" ? "Seller revoked your access" : req && req.status === "declined" ? "Seller declined this request" : "Request sent — awaiting the seller",
  ...
  renderVals() {
    ...
    return {
      ...
      reqList: this.myReqs().map((r) => {
        const p = P.filter((x) => x.id === r.pid)[0] || P[0];
        const label = r.status === "pending" ? "Awaiting seller" : r.status === "accepted" ? "Seller engaged" : r.status === "revoked" ? "Access revoked" : "Declined";
        const tone = r.status === "pending" ? ["#003a70", "#deecf7", "#deecf7"] : r.status === "accepted" ? ["#ffffff", "#003a70", "#003a70"] : ["#494949", "#ffffff", "#494949"];
        return {
          title: p.type + " practice — " + p.area,
