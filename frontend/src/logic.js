...
class Component extends DCLogic {
  ...
  wizardVals() {
    ...
    const uploads = this.props.listings || (s.wizAssets && s.wizAssets.length)
      ? (s.wizAssets || []).map((a, i) => ({ kind: a.kind, name: a.name || (slots[i] ? slots[i].caption : "Photo " + (i + 1)), src: a.src || null, hasSrc: !!a.src, noSrc: !a.src, state: a.state || "processing", pill: ({ processing: "Processing…", review: "Review", confirmed: "Confirmed", failed: "Failed" })[a.state || "processing"], open: this.openPhotoReview && this.openPhotoReview(a.id), cover: a.kind === "Photo" && i === 0, canRemove: a.kind !== "Photo" || ownPhotos, canCover: a.kind === "Photo" && ownPhotos && i > 0, remove: (a.kind === "Photo" && !ownPhotos) || !s.editingId ? null : () => this.props.listings.confirmRemove(a.kind, a.name).then((yes) => (yes ? this.props.listings.remove(s.editingId, a.id) : null)).then((d) => (d ? this.setState({ wizAssets: d.assets, wizErr: "" }) : null), (e) => this.setState({ wizErr: (e && e.message) || "That could not be removed." })), makeCover: a.kind !== "Photo" || !ownPhotos || i === 0 || !s.editingId ? null : () => this.props.listings.reorder(s.editingId, [a.id].concat(photoIds.filter((x) => x !== a.id))).then((d) => this.setState({ wizAssets: d.assets, wizErr: "" }), (e) => this.setState({ wizErr: (e && e.message) || "That could not be saved." })), describe: a.kind !== "Photo" || !s.editingId ? null : () => this.props.listings.describe().then((t) => (t === null ? null : (a.source === "asset" ? this.props.listings.caption(s.editingId, a.id, t) : this.props.listings.describe(s.editingId, a.position, t)))).then((d) => (d ? this.setState({ wizAssets: d.assets, wizErr: "" }) : null), (e) => this.setState({ wizErr: (e && e.message) || "That could not be saved." })) }))
