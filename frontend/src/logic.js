...
class Component extends DCLogic {
  ...
  wizardVals() {
    ...
    const uploads = this.props.listings || (s.wizAssets && s.wizAssets.length)
      ? (s.wizAssets || []).map((a, i) => ({ kind: a.kind, name: a.name || (slots[i] ? slots[i].caption : "Photo " + (i + 1)), src: a.src || null, hasSrc: !!a.src, noSrc: !a.src, state: a.state || "processing", pill: ({ processing: "Processing…", review: "Review", confirmed: "Confirmed", failed: "Failed" })[a.state || "processing"], open: this.openPhotoReview && this.openPhotoReview(a.id), describe: a.kind !== "Photo" || !s.editingId ? null : () => (a.source === "asset" ? this.props.listings.caption(s.editingId, a.id, this.props.listings.describe()) : this.props.listings.describe(s.editingId, a.position, this.props.listings.describe())).then((d) => this.setState({ wizAssets: d.assets, wizErr: "" }), (e) => this.setState({ wizErr: (e && e.message) || "That could not be saved." })) }))
