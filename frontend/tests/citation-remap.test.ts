 ...
 * deletions took ten lines out above `areas: areaFc,` (V3:2542 -> 2532; then 2535 once A31.14g
 * put three comment lines into `metaSource`'s head above it) while its earlier edits
 * put three lines in above A24.7 (919 -> 922) and A3 (858/872 -> 861/875). Each number below was
...
describe('the citation re-mapper', () => {
  ...
  it('moves A24.4 and A25.3 by exactly what an insertion above them inserted', () => {
    ...
    expect(before[2534]).toContain('areas: areaFc,');
    expect(before[2535]).toContain('communities: comms.filter');
    for (const inserted of [1, 7, 400]) {
      ...
      expect(cited('A24.4'), `A24.4 after ${inserted} inserted line(s)`).toBe(2535 + inserted);
      expect(cited('A25.3'), `A25.3 after ${inserted} inserted line(s)`).toBe(2536 + inserted);
    }
  });
  ...
  it('a pin for an entry with no distinctive anchor of its own writes the pinned line as-is', () => {
    ...
    expect(unresolved).toEqual([]);
    expect(moves).toEqual([{ id: 'A98.2', from: 1, to: 2538, rung: 'pin' }]);
    expect(next).toBe(row('A98.2', 'V3:2538'));
  });
  ...
  it('a pin more than one line from every one of the entry\'s own anchors is refused, not silently moved', () => {
    ...
    // make the printed move log ("to: 2532") disagree with the file it wrote, which is the exact
    ...
    expect(unresolved[0]).toMatch(/^A24\.7: the pin resolves to V3:2535, which is not within ±1 of any of this entry's own anchors \(922\) — check the pin's anchor and offset$/);
  });
