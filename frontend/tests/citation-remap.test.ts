...
describe('the citation re-mapper', () => {
  ...
  it('moves A24.4 and A25.3 by exactly what an insertion above them inserted', () => {
    ...
    expect(before[2531]).toContain('areas: areaFc,');
    expect(before[2532]).toContain('communities: comms.filter');
    for (const inserted of [1, 7, 400]) {
      ...
      expect(cited('A24.4'), `A24.4 after ${inserted} inserted line(s)`).toBe(2532 + inserted);
      expect(cited('A25.3'), `A25.3 after ${inserted} inserted line(s)`).toBe(2533 + inserted);
    }
  });
  ...
  it('a pin for an entry with no distinctive anchor of its own writes the pinned line as-is', () => {
    ...
    expect(unresolved).toEqual([]);
    expect(moves).toEqual([{ id: 'A98.2', from: 1, to: 2535, rung: 'pin' }]);
    expect(next).toBe(row('A98.2', 'V3:2535'));
  });
  ...
  it('a pin more than one line from every one of the entry\'s own anchors is refused, not silently moved', () => {
    ...
    expect(unresolved[0]).toMatch(/^A24\.7: the pin resolves to V3:2532, which is not within ±1 of any of this entry's own anchors \(922\) — check the pin's anchor and offset$/);
  });
