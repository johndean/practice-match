...
if [[ "$mode" == "coming_soon" ]]; then
  ...
  # ...and the buyer's own access-request routes (per-buyer disclosure plan, Task 5): a member
  ...
  # Beside it, the buyer's own access-request routes (per-buyer disclosure plan, Task 5): guarded
  # by request.read_own, so an anonymous caller gets the same generic 401 rather than another
  # buyer's requests.
  code=$(curl -sS -o /dev/null -w '%{http_code}' --max-time 20 "$BASE/api/requests/mine")
