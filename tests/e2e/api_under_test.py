...
`ENVIRONMENT` is exactly `test` AND both `DATABASE_URL` and `REDIS_URL` name a service on this
machine — `scripts/reset_rate_limits.py`'s two conditions, the second applied to both services
(A-SL29 (2), on the round-4 re-review's Info-8: `ENVIRONMENT=test` beside a real `DATABASE_URL`
would have served production data with a mock bucket). A launcher whose only safety is where it
happens to be invoked from is one edit away from running somewhere else. Two more refusals guard the
mock itself: every one of the four settings must be present (an unconfigured store is the very 503
this exists to remove), and the endpoint must be an AWS-shaped host, because moto intercepts by
request URL and a Railway-shaped one would escape to the network with the dummy credentials
(A-SL16 M4 — the first time that fixture was written, it did). Each refusal is a non-zero exit and
ONE line on stderr that never repeats a value from the environment.

Its coverage is measured — `--cov=tests/e2e` is in the backend gate wherever the gate is spelled,
and CI's strict mypy step names this file (A-SL29 (2), MINOR-2).
...
#: The two services the api under test talks to; both must be on this machine.
LOCAL_SERVICES = ("DATABASE_URL", "REDIS_URL")
#: The only hosts either may name — `scripts/reset_rate_limits.py`'s own set.
...
def is_local(url: str) -> bool:
    """`scripts/reset_rate_limits.py::is_local_redis`, rule for rule: the parsed host — lower-cased,
    credentials and port stripped — is one of the three loopback spellings; no host at all, or a URL
    `urlsplit` cannot parse, is not a host this can show to be local. Restated rather than imported
    (`tests/e2e/test_api_under_test.py` holds the two equal on a corpus): CI's strict mypy step checks
    that script as a bare file, and an import here would make it see the same file under two module
    names."""
    try:
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    for name in LOCAL_SERVICES:
        if not is_local(os.environ.get(name, "")):
            return refuse(f"{name} must name a service on this machine ({', '.join(sorted(LOCAL_HOSTS))}); a mock bucket beside real data is what this launcher must never serve")
